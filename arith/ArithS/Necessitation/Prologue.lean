...
/-! ## 14. The `exs` child: `insert (substs1 t p) s` (`DESIGN_fragments.md` §4.6)

Route (2026-09-15): (1) the walk of the singleton vector `⟨t⟩` (`descVecAux`: the vector object at `0`, `t`'s dossier
at `1`, `adjFact w T̂ 𝟎`), the re-indexed term `lenT` (`tlenFact (bnum |t|) T̂`), then `lenObj`: `eqTotal [bnum |t|]`
(the LENGTH OBJECT `l = &0` with `l = bnum |t|`), `eqSymm`, the re-indexed cert row `congTLenNum` (`tlenFact l T̂`) and
the prologue row `leOfEqP` (159; `leFact l (bnum |t|)`) — `nodeExs_ok`'s `htl`/`hlet`; (2) a fresh walk of
`substs1 t p` and the re-indexed `certSubst` at the singleton vector (`subFPre_single`) between the parent's dossier of
`p` (the body of the member `∃p`) and the fresh one, then `substsSubsts1` (frag2 row 149): `substs1Fact PT T̂ P`;
(3) `proIns_ok` REUSED on `(s, substs1 t p)`: the child at `0`, `insFact cp PT S`, `eqFactB cp s''`. -/

section exsChild

/-! ### 14.1 The reading of the prologue row `leOfEqP` (159) -/

lemma ProTable.leOfEqP {tbl : V} (h : ProTable tbl) :
    ((pIdx_leOfEqP : ℕ) : V) < len tbl ∧
    rowM tbl.[((pIdx_leOfEqP : ℕ) : V)] = ((2 : ℕ) : V) ∧
    rowB tbl.[((pIdx_leOfEqP : ℕ) : V)] = impChainV LAct (vecOf row_leOfEqP_as) row_leOfEqP_c := by
  have hlt : pIdx_leOfEqP < proRows.length := by
    rw [proRows_length]; simp only [proRowCount, proBase, pIdx_leOfEqP, topRowCount, proExtraRowCount]; omega
  have hr := h.2 pIdx_leOfEqP hlt
  have e' : proRows[pIdx_leOfEqP]? = proExtraRows[4]? := by
    have h1 : pIdx_leOfEqP < (topRows ++ proExtraRows).length := by
      rw [List.length_append]; simp only [pIdx_leOfEqP, topRows_length, proExtraRows_length, topRowCount, proExtraRowCount]; omega
    have h2 : topRows.length ≤ pIdx_leOfEqP := by rw [topRows_length]; simp only [pIdx_leOfEqP, topRowCount]; omega
    unfold proRows
    rw [List.getElem?_append_left h1, List.getElem?_append_right h2]
    rfl
  have e : proRows[pIdx_leOfEqP] = proExtraRows[4] := by
    rw [List.getElem?_eq_getElem hlt, List.getElem?_eq_getElem (by decide : 4 < proExtraRows.length)] at e'
    exact Option.some.inj e'
  rw [e] at hr
  refine ⟨lt_of_lt_of_le (by exact_mod_cast hlt) (proRows_length ▸ h.1), hr.1, ?_⟩
  rw [impChainV_vecOf, hr.2]
  exact quote_row_leOfEqP

/-! ### 14.2 The walk of the singleton vector `⟨t⟩` -/

/-- The walk of `t ∷ 0` at arity `0`: the vector object at `0`, `t`'s dossier at `1`. -/
noncomputable def vecWalk (Ww t : V) : V := π₂ (descVecAux Ww 0 (descTVec Ww 0 1 (t ∷ 0)) 1)
/-- Its eigenvariable count (`≤ 2|t|`). -/
noncomputable def vecCw (Ww t : V) : V := π₁ (descVecAux Ww 0 (descTVec Ww 0 1 (t ∷ 0)) 1)

lemma listSum_termLenVec_single {t : V} (ht : IsSemiterm LAct 0 t) :
    listSum (termLenVec LAct 1 (t ∷ (0 : V))) = termLen LAct t := by
  have := termLenVec_cons (L := LAct) (k := (0 : V)) (t := t) (ts := 0) ht.isUTerm (by simp)
  rw [zero_add] at this
  rw [this, listSum_adjoin, termLenVec_nil, listSum_nil]; simp

/-- **The vector walk is applicable**: Horn-only, `vecCw ≤ 2|t|`, the vector's dossier at `0`. -/
theorem vecWalk_ok {tbl N t E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl) (ht : IsSemiterm LAct 0 t)
    (hE : 2 * termLen LAct t + 10 ≤ E) (hΓ : IsFormulaSet LAct Γ) :
    ListOK tbl E ((8 : ℕ) : V) Γ (vecWalk walkPieces t) ∧ NoDrop (vecWalk walkPieces t) ∧ HornOnly (vecWalk walkPieces t) ∧
    shiftsV (vecWalk walkPieces t) = vecCw walkPieces t ∧ vecCw walkPieces t ≤ 2 * termLen LAct t ∧
    DossV walkPieces (finalCtx Γ (vecWalk walkPieces t)) 0 1 (t ∷ 0) 1 0 := by
  have hW := hP.walkTable
  have hw : IsSemitermVec LAct 1 0 (t ∷ (0 : V)) := by simp [ht]
  have hS : listSum (termLenVec LAct 1 (t ∷ (0 : V))) = termLen LAct t := listSum_termLenVec_single ht
  have htl : takeLast (t ∷ (0 : V)) 1 = t ∷ 0 := by have := takeLast_len_self (t ∷ (0 : V)); rwa [hw.lh] at this
  have hEw : 2 * 0 + 2 * 1 + 2 * listSum (termLenVec LAct 1 (t ∷ (0 : V))) + 8 ≤ E := by
    rw [hS]; exact le_trans (le_of_eq (by ring)) hE
  obtain ⟨_, hcu, hVF⟩ := descVecAux_ok' htbl hW rfl hw hEw 1 le_rfl
  rw [htl, hS] at hcu
  obtain ⟨okU, ndU, shU, _⟩ := hVF Γ hΓ
  have hoU : HornOnly (vecWalk walkPieces t) :=
    hornOnly_descVecAux rfl hw (fun i hi ↦ by have := hornOnly_describeT rfl 0 _ (hw.nth hi); rwa [describeT] at this) 1 le_rfl
  exact ⟨okU, ndU, hoU, shU, hcu, dossV_of_walk ndU⟩

/-! ### 14.3 The length object `l = bnum |t|` -/

/-- `eqTotal [bnum lt]` (`&0 = bnum lt`), `eqSymm [&0, bnum lt]`, the re-indexed cert row `congTLenNum [T̂, bnum lt, &0]`
(`tlenFact &0 T̂`, `T̂ = &(it₀ + 1)` after the eigenvariable), `leOfEqP [&0, bnum lt]` (`leFact &0 (bnum lt)`). -/
noncomputable def lenObj (Wc W lt it₀ : V) : V :=
  appendV ?[mkStep W 40 ?[bnum lt]]
    (appendV ?[mkStep W 42 ?[^&0, bnum lt]]
      (appendV (reidxL ?[mkStep Wc 146 ?[^&(it₀ + 1), bnum lt, ^&0]])
        ?[mkStep W 159 ?[^&0, bnum lt]]))

noncomputable def lenObjDef : 𝚺₁.Semisentence 5 := .mkSigma
  “y Wc W lt it. ∃ bn, !bnumGraph bn lt ∧ ∃ z0, !qqFvarDef z0 0 ∧ ∃ it1, it1 = it + 1 ∧ ∃ zi, !qqFvarDef zi it1 ∧
    ∃ e₁, !adjoinDef e₁ bn 0 ∧ ∃ s₁, !mkStepDef s₁ W 40 e₁ ∧ ∃ l₁, !adjoinDef l₁ s₁ 0 ∧
    ∃ e₂, !adjoinDef e₂ z0 e₁ ∧ ∃ s₂, !mkStepDef s₂ W 42 e₂ ∧ ∃ l₂, !adjoinDef l₂ s₂ 0 ∧
    ∃ e₃', !adjoinDef e₃' z0 0 ∧ ∃ e₃'', !adjoinDef e₃'' bn e₃' ∧ ∃ e₃, !adjoinDef e₃ zi e₃'' ∧ ∃ s₃, !mkStepDef s₃ Wc 146 e₃ ∧
    ∃ l₃', !adjoinDef l₃' s₃ 0 ∧ ∃ l₃, !reidxLDef l₃ l₃' ∧
    ∃ s₄, !mkStepDef s₄ W 159 e₂ ∧ ∃ l₄, !adjoinDef l₄ s₄ 0 ∧
    ∃ r₃, !appendVDef r₃ l₃ l₄ ∧ ∃ r₂, !appendVDef r₂ l₂ r₃ ∧ !appendVDef y l₁ r₂”

set_option maxHeartbeats 1000000 in
instance lenObj_defined : 𝚺₁-Function₄ (lenObj : V → V → V → V → V) via lenObjDef := .mk fun v ↦ by
  simp [lenObjDef, lenObj, bnum.defined.iff, mkStep_defined.iff, reidxL_defined.iff, appendV_defined.iff, numeral_eq_natCast]
instance lenObj_definable : 𝚺₁-Function₄ (lenObj : V → V → V → V → V) := lenObj_defined.to_definable

set_option maxHeartbeats 2000000 in
/-- **`lenObj` is applicable**: one eigenvariable (`l = &0`), four steps, `tlenFact l T̂` and `leFact l (bnum lt)`. -/
theorem lenObj_ok {tbl N Wc W lt it₀ E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl) (hWc : Wc = certPieces)
    (hWp : W = proPieces) (hΓ : IsFormulaSet LAct Γ) (hbE : termLen LAct (bnum lt) ≤ E) (hit : it₀ + 2 ≤ E)
    (htl : neg LAct (tlenFact (bnum lt) (^&it₀)) ∈ Γ) :
    ListOK tbl E ((8 : ℕ) : V) Γ (lenObj Wc W lt it₀) ∧ NoDrop' (lenObj Wc W lt it₀) ∧
    shiftsV (lenObj Wc W lt it₀) = 1 ∧ len (lenObj Wc W lt it₀) = 4 ∧
    neg LAct (tlenFact (^&0) (^&(it₀ + 1))) ∈ finalCtx Γ (lenObj Wc W lt it₀) ∧
    neg LAct (leFact (^&0) (bnum lt)) ∈ finalCtx Γ (lenObj Wc W lt it₀) ∧
    ∀ Q Dd : V, SizeOK Q Dd (lenObj Wc W lt it₀) := by
  have hL := hP.layoutTable
  have hC := hP.certTable
  have htblC := hP.tableOK_certView htbl
  have e40 : ∀ ev : V, mkStep proPieces (40 : V) ev = mkStep layoutPieces (40 : V) ev := fun ev ↦ by
  ...
  have e42 : ∀ ev : V, mkStep proPieces (42 : V) ev = mkStep layoutPieces (42 : V) ev := fun ev ↦ by
    have := mkStep_pro_layout 42 (by decide) ev; simpa using this
  have hE1 : (1 : V) ≤ E := le_trans (by norm_num) (le_trans le_add_self hit)
  have hb : IsSemiterm LAct 0 (bnum lt) := isSemiterm_bnum0 lt
  have hf0 : IsSemiterm LAct 0 (^&0 : V) := by simp
  have hf0E : termLen LAct (^&0 : V) ≤ E := termLen_fvar0_le hE1
  have hitE : it₀ + 1 + 1 ≤ E := by rw [add_assoc, one_add_one_eq_two]; exact hit
  -- step 1: the length object
  obtain ⟨ok₁, tg₁, cx₁⟩ := lok_eqTotal htbl hL rfl hΓ hb hbE
  rw [← e40, ← hWp] at ok₁ tg₁ cx₁
  rw [Nat.cast_zero, termShift_bnum] at cx₁
  have h1nd : NoDrop' (?[mkStep W 40 ?[bnum lt]] : V) := noDrop'_single (Or.inr (Or.inr (Or.inl tg₁)))
  have h1sh : shiftsV (?[mkStep W 40 ?[bnum lt]] : V) = 1 := shiftsV_single_tag2 tg₁
  obtain ⟨Γ₁, hΓ₁⟩ : ∃ Γ', Γ' = insert (neg LAct (eqFactB (^&0) (bnum lt))) (setShift LAct Γ) := ⟨_, rfl⟩
  have hΓ₁f : IsFormulaSet LAct Γ₁ := by rw [hΓ₁, ← cx₁]; exact isFormulaSet_ctxAfter 8 htbl ok₁
  have htl₁ : neg LAct (tlenFact (bnum lt) (^&(it₀ + 1))) ∈ Γ₁ := by
    have := tlenFact_transport h1nd htl
    rw [h1sh, finalCtx_single, cx₁] at this
    rw [hΓ₁]; exact this
  have heq₁ : neg LAct (eqFactB (^&0) (bnum lt)) ∈ Γ₁ := by rw [hΓ₁]; exact memInsSelf _ _
  -- step 2: `eqSymm`
  obtain ⟨ok₂, tg₂, cx₂⟩ := lok_eqSymm htbl hL rfl hΓ₁f hf0 hf0E hb hbE heq₁
  rw [← e42, ← hWp] at ok₂ tg₂ cx₂
  have hΓ₂f : IsFormulaSet LAct (insert (neg LAct (eqFactB (bnum lt) (^&0))) Γ₁) := by
    rw [← cx₂]; exact isFormulaSet_ctxAfter 8 htbl ok₂
  -- step 3: `congTLenNum` (a cert row, re-indexed)
  obtain ⟨ok₃, tg₃, cx₃⟩ := cok_congTLenNum htblC hC hWc hΓ₂f (by simp) (termLen_fvar_le' hitE) hb hbE hf0 hf0E
    (memInsSelf _ _) (memIns htl₁)
  have hΓ₃f : IsFormulaSet LAct (insert (neg LAct (tlenFact (^&0) (^&(it₀ + 1)))) (insert (neg LAct (eqFactB (bnum lt) (^&0))) Γ₁)) := by
    rw [← cx₃]; exact isFormulaSet_ctxAfter 8 htblC ok₃
  -- step 4: `leOfEqP`
  have hrow := hP.leOfEqP
  obtain ⟨ok₄, tg₄, cx₄⟩ := pok_leOfEqP htbl hWp hrow.1 ⟨hrow.2.1, hrow.2.2⟩ hΓ₃f hf0 hf0E hb hbE (memIns (memIns heq₁))
  -- assembly
  unfold lenObj
  refine ⟨?_, noDrop'_appendV h1nd (noDrop'_appendV (noDrop'_single (Or.inl tg₂))
      (noDrop'_appendV (noDrop'_reidxL (noDrop'_single (Or.inl tg₃))) (noDrop'_single (Or.inl tg₄)))), ?_, ?_, ?_, ?_, ?_⟩
  · refine listOK_appendV (listOK_single ok₁) ?_
    rw [finalCtx_single, cx₁, ← hΓ₁]
    refine listOK_appendV (listOK_single ok₂) ?_
    rw [finalCtx_single, cx₂]
    refine listOK_appendV (listOK_reidxL hP (listOK_single ok₃)) ?_
    rw [finalCtx_reidxL, finalCtx_single, cx₃]
    exact listOK_single ok₄
  · rw [shiftsV_appendV, shiftsV_appendV, shiftsV_appendV, h1sh, shiftsV_single_tag0 tg₂, shiftsV_reidxL,
      shiftsV_single_tag0 tg₃, shiftsV_single_tag0 tg₄]
    simp
  · rw [len_appendV, len_appendV, len_appendV, len_reidxL, len_vec1, len_vec1, len_vec1, len_vec1]; norm_num
  · rw [finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_reidxL]
    simp only [finalCtx_single]
    rw [cx₁, ← hΓ₁, cx₂, cx₃, cx₄]
    exact memIns (memInsSelf _ _)
  · rw [finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_reidxL]
    simp only [finalCtx_single]
    rw [cx₁, ← hΓ₁, cx₂, cx₃, cx₄]
    exact memInsSelf _ _
  · intro Q Dd
    exact sizeOK_appendV (sizeOK_single (stepSizeOK_of_tag (Or.inr (Or.inr tg₁))))
      (sizeOK_appendV (sizeOK_single (stepSizeOK_of_tag (Or.inl tg₂)))
        (sizeOK_appendV (sizeOK_reidxL (sizeOK_single (stepSizeOK_of_tag (Or.inl tg₃))))
          (sizeOK_single (stepSizeOK_of_tag (Or.inl tg₄)))))

/-! ### 14.4 The certification prefix `exsCert` -/

/-- The shifts of the term `lenT` (at offset `1`). -/
noncomputable def exsSl (Wc T t : V) : V := shiftsV (lenT Wc T 0 t 1)
/-- The shifts of the walk of `substs1 t p`. -/
noncomputable def exsC3 (Ww t p : V) : V := descCountF Ww 0 (substs1 LAct t p)
/-- The index of the vector object `⟨t⟩` when `certSubst` starts (`T̂` is at `exsIw + 1`). -/
noncomputable def exsIw (Ww Wc T t p : V) : V := exsSl Wc T t + 1 + exsC3 Ww t p
/-- The index of the parent's dossier of `p` (the body of the member `∃p`) when `certSubst` starts. -/
noncomputable def exsIp (Ww Wc T s p t i : V) : V :=
  memTop Ww Wc T s (^∃ p) (i + vecCw Ww t + exsSl Wc T t + 1 + exsC3 Ww t p) + 1
/-- The shifts of the substitution pass. -/
noncomputable def exsSc (Ww Wc T s p t i : V) : V :=
  shiftsV (certSubst Wc Ww 1 0 (t ∷ 0) (exsIw Ww Wc T t p) p (exsIp Ww Wc T s p t i) 0)
/-- The shifts of the whole certification prefix. -/
noncomputable def exsSig (Ww Wc T s p t i : V) : V :=
  vecCw Ww t + exsSl Wc T t + 1 + exsC3 Ww t p + exsSc Ww Wc T s p t i

/-- **`exsCert`**: the vector walk of `⟨t⟩`, the re-indexed term `lenT`, `lenObj`, the walk of `substs1 t p`, the
re-indexed `certSubst` (`substFact PT ⟨t⟩ P`), `substsSubsts1 [P, T̂, ⟨t⟩, PT]` (frag2 row 149). -/
noncomputable def exsCert (Ww Wc W T s p t i : V) : V :=
  appendV (vecWalk Ww t)
    (appendV (reidxL (lenT Wc T 0 t 1))
      (appendV (lenObj Wc W (termLen LAct t) (1 + exsSl Wc T t))
        (appendV (describeF Ww 0 (substs1 LAct t p))
          (appendV (reidxL (certSubst Wc Ww 1 0 (t ∷ 0) (exsIw Ww Wc T t p) p (exsIp Ww Wc T s p t i) 0))
            ?[mkStep W 149 ?[^&(exsIp Ww Wc T s p t i + exsSc Ww Wc T s p t i), ^&(exsIw Ww Wc T t p + exsSc Ww Wc T s p t i + 1),
                ^&(exsIw Ww Wc T t p + exsSc Ww Wc T s p t i), ^&(exsSc Ww Wc T s p t i)]]))))

noncomputable def exsCertDef : 𝚺₁.Semisentence 9 := .mkSigma
  “y Ww Wc W T s p t i. ∃ tv, !adjoinDef tv t 0 ∧ ∃ dv, !descTVecDef dv Ww 0 1 tv ∧ ∃ VW, !descVecAuxDef VW Ww 0 dv 1 ∧
    ∃ cv, !pi₁Def cv VW ∧ ∃ Lv, !pi₂Def Lv VW ∧
    ∃ Lt, !lenTDef Lt Wc T 0 t 1 ∧ ∃ Rt, !reidxLDef Rt Lt ∧ ∃ sl, !shiftsVDef sl Lt ∧
    ∃ lt, !(termLenGraph LAct) lt t ∧ ∃ it₀, it₀ = 1 + sl ∧ ∃ Lo, !lenObjDef Lo Wc W lt it₀ ∧
    ∃ pt, !(substs1Graph LAct) pt t p ∧ ∃ Lp, !describeFDef Lp Ww 0 pt ∧ ∃ c₃, !descCountFDef c₃ Ww 0 pt ∧
    ∃ iw, iw = sl + 1 + c₃ ∧
    ∃ r, !qqExsDef r p ∧ ∃ xs, !memberListDef xs s ∧ ∃ k, !lenDef k xs ∧ ∃ os, !offVecDef os xs Ww Wc T ∧
    ∃ a, !idxOfDef a xs r ∧ ∃ oa, !nthDef oa os a ∧ ∃ i', i' = i + cv + sl + 1 + c₃ ∧ ∃ ip, ip = i' + (2 * k + 1 + oa) + 1 ∧
    ∃ Ls, !certSubstDef Ls Wc Ww 1 0 tv iw p ip 0 ∧ ∃ Rs, !reidxLDef Rs Ls ∧ ∃ sc, !shiftsVDef sc Ls ∧
    ∃ a₁, a₁ = ip + sc ∧ ∃ z₁, !qqFvarDef z₁ a₁ ∧ ∃ a₂, a₂ = iw + sc + 1 ∧ ∃ z₂, !qqFvarDef z₂ a₂ ∧
    ∃ a₃, a₃ = iw + sc ∧ ∃ z₃, !qqFvarDef z₃ a₃ ∧ ∃ z₄, !qqFvarDef z₄ sc ∧
    ∃ e₁, !adjoinDef e₁ z₄ 0 ∧ ∃ e₂, !adjoinDef e₂ z₃ e₁ ∧ ∃ e₃, !adjoinDef e₃ z₂ e₂ ∧ ∃ e₄, !adjoinDef e₄ z₁ e₃ ∧
    ∃ st, !mkStepDef st W 149 e₄ ∧ ∃ l, !adjoinDef l st 0 ∧
    ∃ r₅, !appendVDef r₅ Rs l ∧ ∃ r₄, !appendVDef r₄ Lp r₅ ∧ ∃ r₃, !appendVDef r₃ Lo r₄ ∧ ∃ r₂, !appendVDef r₂ Rt r₃ ∧
    !appendVDef y Lv r₂”

set_option maxHeartbeats 2000000 in
instance exsCert_defined :
    𝚺₁.DefinedFunction (fun v : Fin 8 → V ↦ exsCert (v 0) (v 1) (v 2) (v 3) (v 4) (v 5) (v 6) (v 7)) exsCertDef := .mk
  fun v ↦ by
    simp [exsCertDef, exsCert, vecWalk, vecCw, exsSl, exsC3, exsIw, exsIp, exsSc, memTop, mTop, descTVec_defined.iff,
      descVecAux_defined.iff, lenT_defined.iff, reidxL_defined.iff, shiftsV_defined.iff, termLen.defined.iff,
      lenObj_defined.iff, substs1.defined.iff, describeF_defined.iff, descCountF_defined.iff, memberList_defined.iff,
      offVec_defined.iff, idxOf_defined.iff, certSubst_defined.iff, mkStep_defined.iff, appendV_defined.iff,
      numeral_eq_natCast]

/-- With `r = ∃p ∈ s`, the layout gives `exsFact &ir &(ir + 1)`, the dossier of `p` (arity 1) at `ir + 1` and
`memFact &ir &is` — the `nodeExs_ok` hypotheses `hexs`/`hmr` and the `certSubst` input. -/
theorem layout_exs {tbl N Wc T s p Γ i : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (hp : IsSemiformula LAct 1 p) (hr : (^∃ p) ∈ s) (hLay : Layout walkPieces Wc T Γ s i) :
    neg LAct (exsFact (^&(memTop walkPieces Wc T s (^∃ p) i)) (^&(memTop walkPieces Wc T s (^∃ p) i + 1))) ∈ Γ ∧
    DossF walkPieces Γ 1 p (memTop walkPieces Wc T s (^∃ p) i + 1) ∧
    neg LAct (memFact (^&(memTop walkPieces Wc T s (^∃ p) i)) (^&(i + (len (memberList s) + 1)))) ∈ Γ := by
  obtain ⟨hD, _, _, hm⟩ := hLay.member hr
  obtain ⟨h1, _, hp'⟩ := dossF_exs htbl hP.walkTable rfl (n := 0) (by rw [zero_add]; exact hp) hD
  rw [zero_add] at hp'
  exact ⟨h1, hp', hm⟩

set_option maxHeartbeats 4000000 in
/-- **The certification prefix is applicable** (cap `9`): afterwards the parent is at `i + exsSig`, the fresh dossier
of `PT = substs1 t p` is at `exsSc`, `T̂ = &(exsIw + exsSc + 1)` with `tPiFact 𝟎 T̂`, the length object
`l = &(exsC3 + exsSc)` with `tlenFact l T̂` and `leFact l (bnum |t|)`, and `substs1Fact PT T̂ P`. -/
theorem exsCert_ok {tbl N N' B' Wc W T s p t i D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 1 p) (hr : (^∃ p) ∈ s) (ht : IsSemiterm LAct 0 t)
    (hsD : setLen LAct s ≤ D) (htD : termLen LAct t ≤ D) (hptD : formulaLen LAct (substs1 LAct t p) ≤ D)
    (hE : 13 * D + 18 * ‖D‖ + 12 ≤ E)
    (hEQ : 2 * ((1 + D) * (1 + D + D)) * (D + 1) + 4 * D + 11 ≤ E)
    (hiE : i + 2 * ((1 + D) * (1 + D + D)) * D + 40 * D + 20 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout walkPieces Wc T Γ s i) :
    ListOK tbl E ((9 : ℕ) : V) Γ (exsCert walkPieces Wc W T s p t i) ∧ NoDrop' (exsCert walkPieces Wc W T s p t i) ∧
    shiftsV (exsCert walkPieces Wc W T s p t i) = exsSig walkPieces Wc T s p t i ∧
    exsSc walkPieces Wc T s p t i ≤ 2 * ((1 + D) * (1 + D + D)) * D ∧
    exsSig walkPieces Wc T s p t i ≤ 2 * ((1 + D) * (1 + D + D)) * D + 6 * D + 1 ∧
    Layout walkPieces Wc T (finalCtx Γ (exsCert walkPieces Wc W T s p t i)) s (i + exsSig walkPieces Wc T s p t i) ∧
    DossF walkPieces (finalCtx Γ (exsCert walkPieces Wc W T s p t i)) 0 (substs1 LAct t p) (exsSc walkPieces Wc T s p t i) ∧
    neg LAct (tPiFact (𝟎 : V) (^&(exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i + 1))) ∈
      finalCtx Γ (exsCert walkPieces Wc W T s p t i) ∧
    neg LAct (tlenFact (^&(exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i))
      (^&(exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i + 1))) ∈ finalCtx Γ (exsCert walkPieces Wc W T s p t i) ∧
    neg LAct (leFact (^&(exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i)) (bnum (termLen LAct t))) ∈
      finalCtx Γ (exsCert walkPieces Wc W T s p t i) ∧
    neg LAct (substs1Fact (^&(exsSc walkPieces Wc T s p t i)) (^&(exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i + 1))
      (^&(memTop walkPieces Wc T s (^∃ p) (i + exsSig walkPieces Wc T s p t i) + 1))) ∈
      finalCtx Γ (exsCert walkPieces Wc W T s p t i) ∧
    ∀ B : V, SizeOK (layQ B B' D) (layD N' B' D) (exsCert walkPieces Wc W T s p t i) := by
  have hW := hP.walkTable
  have hC := hP.certTable
  have htblC := hP.tableOK_certView htbl
  have hF2 := hP.frag2Table
  have h89 : ((8 : ℕ) : V) ≤ ((9 : ℕ) : V) := by exact_mod_cast (by decide : (8 : ℕ) ≤ 9)
  have e149 : ∀ ev : V, mkStep proPieces (149 : V) ev = mkStep frag2Pieces (149 : V) ev := fun ev ↦ by
    have := mkStep_pro_frag2 149 (by decide) ev; simpa using this
  have hw : IsSemitermVec LAct 1 0 (t ∷ (0 : V)) := by simp [ht]
  have hpt : IsSemiformula LAct 0 (substs1 LAct t p) := hp.substs1 ht
  have hpD : formulaLen LAct p ≤ D := by
    have h1 := formulaLen_exs (L := LAct) hp.isUFormula
    have h2 := formulaLen_le_setLen_of_mem (L := LAct) hr
    rw [h1] at h2
    exact le_trans le_self_add (le_trans h2 hsD)
  have hQ : (1 + formulaLen LAct p) * (1 + formulaLen LAct p + termLen LAct t) ≤ (1 + D) * (1 + D + D) :=
    mul_le_mul (add_le_add le_rfl hpD) (add_le_add (add_le_add le_rfl hpD) htD) zero_le zero_le
  have hQD2 : 2 * ((1 + formulaLen LAct p) * (1 + formulaLen LAct p + termLen LAct t)) * formulaLen LAct p ≤
      2 * ((1 + D) * (1 + D + D)) * D := mul_le_mul (mul_le_mul_of_nonneg_left hQ zero_le) hpD zero_le zero_le
  have hD13 : 2 * D + 10 ≤ E :=
    le_trans (add_le_add (le_trans (mul_le_mul_of_nonneg_right (by norm_num) zero_le) le_self_add) (by norm_num)) hE
  have hE18 : 18 * ‖D‖ + 7 ≤ E :=
    le_trans (add_le_add (le_add_self : 18 * ‖D‖ ≤ 13 * D + 18 * ‖D‖) (by norm_num : (7 : V) ≤ 12)) hE
  have hbE : termLen LAct (bnum (termLen LAct t)) ≤ E := termLen_bnum_le_bkE htD hE18
  have h0 : (t ∷ (0 : V)).[1 - (0 + 1)] = t := by rw [zero_add, tsub_self]; simp
  -- (1) the vector walk
  obtain ⟨vok, vnd, vho, vsh, hcv, vD⟩ := vecWalk_ok htbl hP ht
    (by calc 2 * termLen LAct t + 10 ≤ 2 * D + 10 := add_le_add (mul_le_mul_of_nonneg_left htD zero_le) le_rfl
        _ ≤ E := hD13) hΓ
  have hcv' : vecCw walkPieces t ≤ 2 * D := le_trans hcv (mul_le_mul_of_nonneg_left htD zero_le)
  obtain ⟨Γ₁, hΓ₁⟩ : ∃ Γ', Γ' = finalCtx Γ (vecWalk walkPieces t) := ⟨_, rfl⟩
  have hΓ₁f : IsFormulaSet LAct Γ₁ := by rw [hΓ₁]; exact finalCtx_isFormulaSet 8 htbl hΓ vok
  have vD₁ : DossV walkPieces Γ₁ 0 1 (t ∷ 0) 1 0 := by rw [hΓ₁]; exact vD
  have hLay₁ : Layout walkPieces Wc T Γ₁ s (i + vecCw walkPieces t) := by
    rw [hΓ₁]; have := hLay.transport vnd.noDrop'; rwa [vsh] at this
  have vD₁' : DossV walkPieces Γ₁ 0 1 (t ∷ 0) (0 + 1) 0 := by rw [zero_add]; exact vD₁
  obtain ⟨_, _, hDt₁, _⟩ := dossV_succ htbl hW rfl hw (j := 0) (by rw [zero_add]) vD₁'
  rw [h0, zero_add] at hDt₁
  -- (2) the term lengths (a cert list, re-indexed)
  have hTL := lenTGraph_ok htblC hC htblN rfl hWc 0 t ht
  unfold TLenOK at hTL
  obtain ⟨lok, lfact⟩ := hTL 1 _ E Γ₁ (lenT_graph ht)
    ⟨by calc 2 * 0 + 13 * termLen LAct t + 8 ≤ 2 * 0 + 13 * D + 8 := add_le_add (add_le_add le_rfl (mul_le_mul_of_nonneg_left htD zero_le)) le_rfl
        _ ≤ 13 * D + 18 * ‖D‖ + 12 := by rw [mul_zero, zero_add]; exact add_le_add le_self_add (by norm_num)
        _ ≤ E := hE,
      by calc 1 + 5 * termLen LAct t + 2 ≤ 1 + 5 * D + 2 := add_le_add (add_le_add le_rfl (mul_le_mul_of_nonneg_left htD zero_le)) le_rfl
        _ ≤ 13 * D + 18 * ‖D‖ + 12 := by
          calc 1 + 5 * D + 2 = 5 * D + 3 := by ring
            _ ≤ 13 * D + 18 * ‖D‖ + 12 := add_le_add (le_trans (mul_le_mul_of_nonneg_right (by norm_num) zero_le) le_self_add) (by norm_num)
        _ ≤ E := hE, hΓ₁f⟩ hDt₁
  obtain ⟨lnd, lsh⟩ := lenTGraph_noDrop_shifts hWc T 0 t ht 1 _ (lenT_graph ht)
  have hsl : shiftsV (lenT Wc T 0 t 1) = exsSl Wc T t := rfl
  have hsl' : exsSl Wc T t ≤ 2 * D := by
    rw [← hsl]; exact le_trans le_self_add (le_trans lsh (mul_le_mul_of_nonneg_left htD zero_le))
  rw [hsl] at lfact
  obtain ⟨Γ₂, hΓ₂⟩ : ∃ Γ', Γ' = finalCtx Γ₁ (reidxL (lenT Wc T 0 t 1)) := ⟨_, rfl⟩
  have hΓ₂f : IsFormulaSet LAct Γ₂ := by rw [hΓ₂]; exact finalCtx_isFormulaSet 8 htbl hΓ₁f (listOK_reidxL hP lok)
  have htl₂ : neg LAct (tlenFact (bnum (termLen LAct t)) (^&(1 + exsSl Wc T t))) ∈ Γ₂ := by rw [hΓ₂, finalCtx_reidxL]; exact lfact
  have vD₂ : DossV walkPieces Γ₂ 0 1 (t ∷ 0) 1 (0 + exsSl Wc T t) := by
    rw [hΓ₂]; have := dossV_transport' (noDrop'_reidxL lnd) vD₁; rwa [shiftsV_reidxL, hsl] at this
  have hLay₂ : Layout walkPieces Wc T Γ₂ s (i + vecCw walkPieces t + exsSl Wc T t) := by
    rw [hΓ₂]; have := hLay₁.transport (noDrop'_reidxL lnd); rwa [shiftsV_reidxL, hsl] at this
  -- (3) the length object
  obtain ⟨ook, ond, osh, _, otl, ole, osz⟩ := lenObj_ok htbl hP hWc hWp hΓ₂f hbE
    (by calc 1 + exsSl Wc T t + 2 ≤ 1 + 2 * D + 2 := add_le_add (add_le_add le_rfl hsl') le_rfl
        _ ≤ 2 * D + 10 := by rw [add_comm 1 (2 * D), add_assoc]; exact add_le_add le_rfl (by norm_num)
        _ ≤ E := hD13) htl₂
  obtain ⟨Γ₃, hΓ₃⟩ : ∃ Γ', Γ' = finalCtx Γ₂ (lenObj Wc W (termLen LAct t) (1 + exsSl Wc T t)) := ⟨_, rfl⟩
  have hΓ₃f : IsFormulaSet LAct Γ₃ := by rw [hΓ₃]; exact finalCtx_isFormulaSet 8 htbl hΓ₂f ook
  have otl₃ : neg LAct (tlenFact (^&0) (^&(1 + exsSl Wc T t + 1))) ∈ Γ₃ := by rw [hΓ₃]; exact otl
  have ole₃ : neg LAct (leFact (^&0) (bnum (termLen LAct t))) ∈ Γ₃ := by rw [hΓ₃]; exact ole
  have vD₃ : DossV walkPieces Γ₃ 0 1 (t ∷ 0) 1 (0 + exsSl Wc T t + 1) := by
    rw [hΓ₃]; have := dossV_transport' ond vD₂; rwa [osh] at this
  have hLay₃ : Layout walkPieces Wc T Γ₃ s (i + vecCw walkPieces t + exsSl Wc T t + 1) := by
    rw [hΓ₃]; have := hLay₂.transport ond; rwa [osh] at this
  -- (4) the walk of `substs1 t p`
  have hptD' : 2 * 0 + 2 * formulaLen LAct (substs1 LAct t p) + 8 ≤ E := by
    calc 2 * 0 + 2 * formulaLen LAct (substs1 LAct t p) + 8 ≤ 2 * 0 + 2 * D + 8 := add_le_add (add_le_add le_rfl (mul_le_mul_of_nonneg_left hptD zero_le)) le_rfl
      _ ≤ 2 * D + 10 := by rw [mul_zero, zero_add]; exact add_le_add le_rfl (by norm_num)
      _ ≤ E := hD13
  obtain ⟨pok', pnd, psh, pcnt, _⟩ := describeF_ok htbl hW hpt hptD' hΓ₃f
  have hc₃ : descCountF walkPieces 0 (substs1 LAct t p) = exsC3 walkPieces t p := rfl
  have hc₃' : exsC3 walkPieces t p ≤ 2 * D := by
    rw [← hc₃]; exact le_trans le_self_add (le_trans pcnt (mul_le_mul_of_nonneg_left hptD zero_le))
  rw [hc₃] at psh
  obtain ⟨Γ₄, hΓ₄⟩ : ∃ Γ', Γ' = finalCtx Γ₃ (describeF walkPieces 0 (substs1 LAct t p)) := ⟨_, rfl⟩
  have hΓ₄f : IsFormulaSet LAct Γ₄ := by rw [hΓ₄]; exact finalCtx_isFormulaSet 8 htbl hΓ₃f pok'
  have hDpt₄ : DossF walkPieces Γ₄ 0 (substs1 LAct t p) 0 := by rw [hΓ₄]; exact dossF_of_walk pnd
  have vD₄ : DossV walkPieces Γ₄ 0 1 (t ∷ 0) 1 (exsIw walkPieces Wc T t p) := by
    rw [hΓ₄]; have := dossV_transport pnd vD₃; rw [psh, zero_add] at this; exact this
  have hLay₄ : Layout walkPieces Wc T Γ₄ s (i + vecCw walkPieces t + exsSl Wc T t + 1 + exsC3 walkPieces t p) := by
    rw [hΓ₄]; have := hLay₃.transport pnd.noDrop'; rwa [psh] at this
  have otl₄ : neg LAct (tlenFact (^&(0 + exsC3 walkPieces t p)) (^&(1 + exsSl Wc T t + 1 + exsC3 walkPieces t p))) ∈ Γ₄ := by
    rw [hΓ₄]
    have := mem_finalCtx_of_mem pnd otl₃
    rwa [psh, shiftIterV_neg (isFormula_tlenFact (by simp) (by simp)), shiftIterV_tlenFact (by simp) (by simp),
      termShiftIterV_fvar, termShiftIterV_fvar] at this
  have ole₄ : neg LAct (leFact (^&(0 + exsC3 walkPieces t p)) (bnum (termLen LAct t))) ∈ Γ₄ := by
    rw [hΓ₄]
    have := mem_finalCtx_of_mem pnd ole₃
    rwa [psh, shiftIterV_neg (isFormula_leFact (by simp) (isSemiterm_bnum0 _)), shiftIterV_leFact (by simp) (isSemiterm_bnum0 _),
      termShiftIterV_fvar, termShiftIterV_bnum'] at this
  obtain ⟨_, hDp₄, _⟩ := layout_exs htbl hP hp hr hLay₄
  have hip : memTop walkPieces Wc T s (^∃ p) (i + vecCw walkPieces t + exsSl Wc T t + 1 + exsC3 walkPieces t p) + 1 =
      exsIp walkPieces Wc T s p t i := rfl
  rw [hip] at hDp₄
  -- (5) the substitution pass
  have hmt : exsIp walkPieces Wc T s p t i ≤ i + 12 * D + 3 := by
    unfold exsIp
    refine le_trans (add_le_add (memTop_le htbl hW hWc T hs hr hsD) le_rfl) ?_
    calc i + vecCw walkPieces t + exsSl Wc T t + 1 + exsC3 walkPieces t p + 6 * D + 1 + 1
        ≤ i + 2 * D + 2 * D + 1 + 2 * D + 6 * D + 1 + 1 :=
          add_le_add (add_le_add (add_le_add (add_le_add (add_le_add (add_le_add (add_le_add le_rfl hcv') hsl') le_rfl) hc₃') le_rfl) le_rfl) le_rfl
      _ = i + 12 * D + 3 := by ring
  have hiw : exsIw walkPieces Wc T t p ≤ 4 * D + 1 := by
    unfold exsIw
    calc exsSl Wc T t + 1 + exsC3 walkPieces t p ≤ 2 * D + 1 + 2 * D := add_le_add (add_le_add hsl' le_rfl) hc₃'
      _ = 4 * D + 1 := by ring
  have hPre : SubFPre E ((1 + formulaLen LAct p) * (1 + formulaLen LAct p + termLen LAct t)) 1 0 (t ∷ 0) p
      (exsIp walkPieces Wc T s p t i) 0 (exsIw walkPieces Wc T t p) Γ₄ := by
    refine subFPre_single ht (le_refl _) ?_ ?_ ?_ ?_ hΓ₄f
    · calc 4 * formulaLen LAct p + 2 * ((1 + formulaLen LAct p) * (1 + formulaLen LAct p + termLen LAct t)) + 11
          ≤ 4 * D + 2 * ((1 + D) * (1 + D + D)) + 11 := add_le_add (add_le_add (mul_le_mul_of_nonneg_left hpD zero_le) (mul_le_mul_of_nonneg_left hQ zero_le)) le_rfl
        _ ≤ 4 * D + 2 * ((1 + D) * (1 + D + D)) * (D + 1) + 11 := add_le_add (add_le_add le_rfl (le_mul_of_one_le_right zero_le le_add_self)) le_rfl
        _ = 2 * ((1 + D) * (1 + D + D)) * (D + 1) + 4 * D + 11 := by ring
        _ ≤ E := hEQ
    · calc exsIp walkPieces Wc T s p t i + 2 * formulaLen LAct p * ((1 + formulaLen LAct p) * (1 + formulaLen LAct p + termLen LAct t) + 1) + 1
          ≤ (i + 12 * D + 3) + 2 * D * ((1 + D) * (1 + D + D) + 1) + 1 :=
            add_le_add (add_le_add hmt (mul_le_mul (mul_le_mul_of_nonneg_left hpD zero_le) (add_le_add hQ le_rfl) zero_le zero_le)) le_rfl
        _ = i + 2 * ((1 + D) * (1 + D + D)) * D + 14 * D + 4 := by ring
        _ ≤ i + 2 * ((1 + D) * (1 + D + D)) * D + 40 * D + 20 :=
            add_le_add (add_le_add le_rfl (mul_le_mul_of_nonneg_right (by norm_num) zero_le)) (by norm_num)
        _ ≤ E := hiE
    · show 0 + 2 * formulaLen LAct (subst LAct (t ∷ 0) p) + 2 * ((1 + formulaLen LAct p) * (1 + formulaLen LAct p + termLen LAct t)) * formulaLen LAct p + 1 ≤ E
      calc 0 + 2 * formulaLen LAct (subst LAct (t ∷ 0) p) + 2 * ((1 + formulaLen LAct p) * (1 + formulaLen LAct p + termLen LAct t)) * formulaLen LAct p + 1
          ≤ 0 + 2 * D + 2 * ((1 + D) * (1 + D + D)) * D + 1 :=
            add_le_add (add_le_add (add_le_add le_rfl (mul_le_mul_of_nonneg_left hptD zero_le)) hQD2) le_rfl
        _ = 0 + 2 * ((1 + D) * (1 + D + D)) * D + 2 * D + 1 := by ring
        _ ≤ i + 2 * ((1 + D) * (1 + D + D)) * D + 40 * D + 20 :=
            add_le_add (add_le_add (add_le_add zero_le le_rfl) (mul_le_mul_of_nonneg_right (by norm_num) zero_le)) (by norm_num)
        _ ≤ E := hiE
    · calc exsIw walkPieces Wc T t p + 2 * ((1 + formulaLen LAct p) * (1 + formulaLen LAct p + termLen LAct t)) * (formulaLen LAct p + 1) + 2
          ≤ (4 * D + 1) + 2 * ((1 + D) * (1 + D + D)) * (D + 1) + 2 :=
            add_le_add (add_le_add hiw (mul_le_mul (mul_le_mul_of_nonneg_left hQ zero_le) (add_le_add hpD le_rfl) zero_le zero_le)) le_rfl
        _ = 2 * ((1 + D) * (1 + D + D)) * (D + 1) + 4 * D + 3 := by ring
        _ ≤ 2 * ((1 + D) * (1 + D + D)) * (D + 1) + 4 * D + 11 := add_le_add le_rfl (by norm_num)
        _ ≤ E := hEQ
  have hLcap := qWalkP_single_cap (Wd := walkPieces) (t := t) (B := termLen LAct t) (r := p) ht (le_refl _)
  obtain ⟨cok, cnd, cho, csh, _, cfact⟩ := certSubst_ok htblC hC rfl hWc hw hp hPre hLcap hDp₄ hDpt₄ vD₄
  have hsc : shiftsV (certSubst Wc walkPieces 1 0 (t ∷ 0) (exsIw walkPieces Wc T t p) p (exsIp walkPieces Wc T s p t i) 0) =
      exsSc walkPieces Wc T s p t i := rfl
  rw [hsc] at csh cfact
  have hsc' : exsSc walkPieces Wc T s p t i ≤ 2 * ((1 + D) * (1 + D + D)) * D := le_trans csh hQD2
  obtain ⟨Γ₅, hΓ₅⟩ : ∃ Γ', Γ' = finalCtx Γ₄ (reidxL (certSubst Wc walkPieces 1 0 (t ∷ 0) (exsIw walkPieces Wc T t p) p (exsIp walkPieces Wc T s p t i) 0)) := ⟨_, rfl⟩
  have hΓ₅f : IsFormulaSet LAct Γ₅ := by rw [hΓ₅]; exact finalCtx_isFormulaSet 9 htbl hΓ₄f (listOK_reidxL hP cok)
  have cnd' : NoDrop' (reidxL (certSubst Wc walkPieces 1 0 (t ∷ 0) (exsIw walkPieces Wc T t p) p (exsIp walkPieces Wc T s p t i) 0)) :=
    noDrop'_reidxL cnd
  have csh' : shiftsV (reidxL (certSubst Wc walkPieces 1 0 (t ∷ 0) (exsIw walkPieces Wc T t p) p (exsIp walkPieces Wc T s p t i) 0)) =
      exsSc walkPieces Wc T s p t i := by rw [shiftsV_reidxL, hsc]
  have cfact₅ : neg LAct (substFact (^&(exsSc walkPieces Wc T s p t i)) (^&(exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i))
      (^&(exsIp walkPieces Wc T s p t i + exsSc walkPieces Wc T s p t i))) ∈ Γ₅ := by
    rw [hΓ₅, finalCtx_reidxL]
    have := cfact; rw [zero_add, vRef_of_ne _root_.one_ne_zero] at this; exact this
  have hLay₅ : Layout walkPieces Wc T Γ₅ s (i + vecCw walkPieces t + exsSl Wc T t + 1 + exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i) := by
    rw [hΓ₅]; have := hLay₄.transport cnd'; rwa [csh'] at this
  have hDpt₅ : DossF walkPieces Γ₅ 0 (substs1 LAct t p) (exsSc walkPieces Wc T s p t i) := by
    rw [hΓ₅]; have := dossF_transport' cnd' hDpt₄; rwa [csh', zero_add] at this
  have vD₅ : DossV walkPieces Γ₅ 0 1 (t ∷ 0) 1 (exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i) := by
    rw [hΓ₅]; have := dossV_transport (noDrop_reidxL (noDrop_of_hornOnly cho)) vD₄; rwa [csh'] at this
  have otl₅ : neg LAct (tlenFact (^&(0 + exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i))
      (^&(1 + exsSl Wc T t + 1 + exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i))) ∈ Γ₅ := by
    rw [hΓ₅]
    have := mem_finalCtx_of_mem' cnd' otl₄
    rwa [csh', shiftIterV_neg (isFormula_tlenFact (by simp) (by simp)), shiftIterV_tlenFact (by simp) (by simp),
      termShiftIterV_fvar, termShiftIterV_fvar] at this
  have ole₅ : neg LAct (leFact (^&(0 + exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i)) (bnum (termLen LAct t))) ∈ Γ₅ := by
    rw [hΓ₅]
    have := mem_finalCtx_of_mem' cnd' ole₄
    rwa [csh', shiftIterV_neg (isFormula_leFact (by simp) (isSemiterm_bnum0 _)), shiftIterV_leFact (by simp) (isSemiterm_bnum0 _),
      termShiftIterV_fvar, termShiftIterV_bnum'] at this
  have vD₅' : DossV walkPieces Γ₅ 0 1 (t ∷ 0) (0 + 1) (exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i) := by
    rw [zero_add]; exact vD₅
  obtain ⟨hadj₅, _, hDt₅, _⟩ := dossV_succ htbl hW rfl hw (j := 0) (by rw [zero_add]) vD₅'
  rw [h0] at hDt₅
  rw [vRef_zero] at hadj₅
  have htpi₅ := dossT_tPi htbl hW rfl ht hDt₅
  rw [cTV_zero] at htpi₅
  -- (6) `substsSubsts1`
  have hsc1 : exsSc walkPieces Wc T s p t i + 1 ≤ E := by
    calc exsSc walkPieces Wc T s p t i + 1 ≤ 2 * ((1 + D) * (1 + D + D)) * D + 1 := add_le_add hsc' le_rfl
      _ ≤ i + 2 * ((1 + D) * (1 + D + D)) * D + 40 * D + 20 := by
        calc 2 * ((1 + D) * (1 + D + D)) * D + 1 = 0 + 2 * ((1 + D) * (1 + D + D)) * D + 0 + 1 := by ring
          _ ≤ i + 2 * ((1 + D) * (1 + D + D)) * D + 40 * D + 20 :=
            add_le_add (add_le_add (add_le_add zero_le le_rfl) zero_le) (by norm_num)
      _ ≤ E := hiE
  have hiwsc : exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i + 1 + 1 ≤ E := by
    calc exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i + 1 + 1 ≤ (4 * D + 1) + 2 * ((1 + D) * (1 + D + D)) * D + 1 + 1 :=
          add_le_add (add_le_add (add_le_add hiw hsc') le_rfl) le_rfl
      _ = 0 + 2 * ((1 + D) * (1 + D + D)) * D + 4 * D + 3 := by ring
      _ ≤ i + 2 * ((1 + D) * (1 + D + D)) * D + 40 * D + 20 :=
          add_le_add (add_le_add (add_le_add zero_le le_rfl) (mul_le_mul_of_nonneg_right (by norm_num) zero_le)) (by norm_num)
      _ ≤ E := hiE
  have hiwsc' : exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i + 1 ≤ E := le_trans le_self_add hiwsc
  have hipsc : exsIp walkPieces Wc T s p t i + exsSc walkPieces Wc T s p t i + 1 ≤ E := by
    calc exsIp walkPieces Wc T s p t i + exsSc walkPieces Wc T s p t i + 1 ≤ (i + 12 * D + 3) + 2 * ((1 + D) * (1 + D + D)) * D + 1 :=
          add_le_add (add_le_add hmt hsc') le_rfl
      _ = i + 2 * ((1 + D) * (1 + D + D)) * D + 12 * D + 4 := by ring
      _ ≤ i + 2 * ((1 + D) * (1 + D + D)) * D + 40 * D + 20 :=
          add_le_add (add_le_add le_rfl (mul_le_mul_of_nonneg_right (by norm_num) zero_le)) (by norm_num)
      _ ≤ E := hiE
  obtain ⟨ok₆, tg₆, cx₆⟩ := gok_substsSubsts1 htbl hF2 rfl hΓ₅f (by simp) (termLen_fvar_le' hipsc) (by simp) (termLen_fvar_le' hiwsc)
    (by simp) (termLen_fvar_le' hiwsc') (by simp) (termLen_fvar_le' hsc1) hadj₅ cfact₅
  rw [← e149, ← hWp] at ok₆ tg₆ cx₆
  -- assembly
  have hsig : exsSig walkPieces Wc T s p t i = vecCw walkPieces t + exsSl Wc T t + 1 + exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i := rfl
  have tr₆ : ∀ x ∈ Γ₅, x ∈ finalCtx Γ₅ (?[mkStep W 149 ?[^&(exsIp walkPieces Wc T s p t i + exsSc walkPieces Wc T s p t i),
      ^&(exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i + 1), ^&(exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i),
      ^&(exsSc walkPieces Wc T s p t i)]] : V) := fun x hx ↦ by
    rw [finalCtx_single, cx₆]; exact memIns hx
  have e₁ : 0 + exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i = exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i := by ring
  have e₂ : 1 + exsSl Wc T t + 1 + exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i =
      exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i + 1 := by unfold exsIw; ring
  have e₃ : exsIp walkPieces Wc T s p t i + exsSc walkPieces Wc T s p t i =
      memTop walkPieces Wc T s (^∃ p) (i + exsSig walkPieces Wc T s p t i) + 1 := by
    unfold exsIp; rw [hsig]; unfold memTop mTop; ring
  rw [e₁, e₂] at otl₅
  rw [e₁] at ole₅
  unfold exsCert
  refine ⟨?_, ?_, ?_, hsc', ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩
  · refine listOK_appendV (vok.mono h89) ?_
    rw [← hΓ₁]
    refine listOK_appendV (listOK_reidxL hP lok |>.mono h89) ?_
    rw [← hΓ₂]
    refine listOK_appendV (ook.mono h89) ?_
    rw [← hΓ₃]
    refine listOK_appendV (pok'.mono h89) ?_
    rw [← hΓ₄]
    refine listOK_appendV (listOK_reidxL hP cok) ?_
    rw [← hΓ₅]
    exact listOK_single (ok₆.mono h89)
  · exact noDrop'_appendV vnd.noDrop' (noDrop'_appendV (noDrop'_reidxL lnd) (noDrop'_appendV ond
      (noDrop'_appendV pnd.noDrop' (noDrop'_appendV cnd' (noDrop'_single (Or.inl tg₆))))))
  · rw [shiftsV_appendV, shiftsV_appendV, shiftsV_appendV, shiftsV_appendV, shiftsV_appendV, vsh, shiftsV_reidxL, hsl, osh, psh,
      csh', shiftsV_single_tag0 tg₆, hsig, add_zero]
    ring
  · rw [hsig]
    calc vecCw walkPieces t + exsSl Wc T t + 1 + exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i
        ≤ 2 * D + 2 * D + 1 + 2 * D + 2 * ((1 + D) * (1 + D + D)) * D :=
          add_le_add (add_le_add (add_le_add (add_le_add hcv' hsl') le_rfl) hc₃') hsc'
      _ = 2 * ((1 + D) * (1 + D + D)) * D + 6 * D + 1 := by ring
  · rw [finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, ← hΓ₁, ← hΓ₂, ← hΓ₃, ← hΓ₄, ← hΓ₅,
      finalCtx_single, cx₆, hsig]
    have eL : i + (vecCw walkPieces t + exsSl Wc T t + 1 + exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i) =
        i + vecCw walkPieces t + exsSl Wc T t + 1 + exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i := by ring
    rw [eL]
    exact hLay₅.mono (fun x hx ↦ memIns hx)
  · rw [finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, ← hΓ₁, ← hΓ₂, ← hΓ₃, ← hΓ₄, ← hΓ₅,
      finalCtx_single, cx₆]
    exact hDpt₅.mono (fun x hx ↦ memIns hx)
  · rw [finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, ← hΓ₁, ← hΓ₂, ← hΓ₃, ← hΓ₄, ← hΓ₅,
      finalCtx_single, cx₆]
    exact memIns htpi₅
  · rw [finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, ← hΓ₁, ← hΓ₂, ← hΓ₃, ← hΓ₄, ← hΓ₅,
  ...
  · rw [finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, ← hΓ₁, ← hΓ₂, ← hΓ₃, ← hΓ₄, ← hΓ₅,
    ...
    exact memIns ole₅
  · rw [finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, ← hΓ₁, ← hΓ₂, ← hΓ₃, ← hΓ₄, ← hΓ₅,
      finalCtx_single, cx₆, ← e₃]
    exact memInsSelf _ _
  · intro B
    exact sizeOK_appendV (sizeOK_of_hornOnly vho)
      (sizeOK_appendV (sizeOK_reidxL ((lenTGraph_sizeOK hWc htblN 0 D t ht htD 1 _ (lenT_graph ht)).mono le_self_add le_self_add))
        (sizeOK_appendV (osz _ _)
          (sizeOK_appendV (sizeOK_of_hornOnly (hornOnly_describeF rfl hpt))
            (sizeOK_appendV (sizeOK_reidxL (sizeOK_of_hornOnly cho)) (sizeOK_single (stepSizeOK_of_tag (Or.inl tg₆)))))))

/-! ### 14.5 The whole `exs` prologue -/

/-- **`proExs s p t i`**: `exsCert` (the witness term, its length object, `substs1Fact PT T̂ P`), then
`proIns s (substs1 t p)` (the child `insert (substs1 t p) s` at `0`, `insFact cp PT S`, `eqFactB cp s''`). -/
noncomputable def proExs (Ww Wl Wc W T s p t i : V) : V :=
  appendV (exsCert Ww Wc W T s p t i)
    (proIns Ww Wl Wc W T s (substs1 LAct t p) (i + exsSig Ww Wc T s p t i) (exsSc Ww Wc T s p t i))

noncomputable def proExsDef : 𝚺₁.Semisentence 10 := .mkSigma
  “y Ww Wl Wc W T s p t i. ∃ C, !exsCertDef C Ww Wc W T s p t i ∧
    ∃ tv, !adjoinDef tv t 0 ∧ ∃ dv, !descTVecDef dv Ww 0 1 tv ∧ ∃ VW, !descVecAuxDef VW Ww 0 dv 1 ∧ ∃ cv, !pi₁Def cv VW ∧
    ∃ Lt, !lenTDef Lt Wc T 0 t 1 ∧ ∃ sl, !shiftsVDef sl Lt ∧ ∃ pt, !(substs1Graph LAct) pt t p ∧ ∃ c₃, !descCountFDef c₃ Ww 0 pt ∧
    ∃ iw, iw = sl + 1 + c₃ ∧
    ∃ r, !qqExsDef r p ∧ ∃ xs, !memberListDef xs s ∧ ∃ k, !lenDef k xs ∧ ∃ os, !offVecDef os xs Ww Wc T ∧
    ∃ a, !idxOfDef a xs r ∧ ∃ oa, !nthDef oa os a ∧ ∃ i'', i'' = i + cv + sl + 1 + c₃ ∧ ∃ ip, ip = i'' + (2 * k + 1 + oa) + 1 ∧
    ∃ Ls, !certSubstDef Ls Wc Ww 1 0 tv iw p ip 0 ∧ ∃ sc, !shiftsVDef sc Ls ∧
    ∃ σ, σ = cv + sl + 1 + c₃ + sc ∧ ∃ i', i' = i + σ ∧
    ∃ I, !proInsDef I Ww Wl Wc W T s pt i' sc ∧ !appendVDef y C I”

set_option maxHeartbeats 2000000 in
instance proExs_defined :
    𝚺₁.DefinedFunction (fun v : Fin 9 → V ↦ proExs (v 0) (v 1) (v 2) (v 3) (v 4) (v 5) (v 6) (v 7) (v 8)) proExsDef := .mk
  fun v ↦ by
    simp [proExsDef, proExs, exsSig, exsSc, exsIw, exsIp, exsSl, exsC3, vecCw, memTop, mTop, exsCert_defined.iff,
      lenT_defined.iff, shiftsV_defined.iff, substs1.defined.iff, descCountF_defined.iff, descTVec_defined.iff,
      descVecAux_defined.iff, memberList_defined.iff, offVec_defined.iff, idxOf_defined.iff, certSubst_defined.iff,
      proIns_defined.iff, appendV_defined.iff, numeral_eq_natCast]

set_option maxHeartbeats 4000000 in
/-- **The `exs` child's prologue is applicable** (cap `9`): afterwards the child `insert (substs1 t p) s` is laid out
at `0`, the parent at `iP = i + exsSig + 1 + σ₃` (`σ₃ = proSig` of the child), and — with `S = &(iP + (k + 1))`,
`ir = memTop … (∃p) iP`, `P = &(ir + 1)`, `T̂ = &it`, `l = &ilt`, `PT = &ipt`, `cp = &σ₃`, `s'' = &(k_c + 1)` —
`tPiFact 𝟎 T̂`, `tlenFact l T̂`, `leFact l (bnum |t|)`, `substs1Fact PT T̂ P`, `insFact cp PT S`, `eqFactB cp s''`:
every `nodeExs_ok` hypothesis except the child's own goal (`postIns`) is a fact here or follows from the two layouts. -/
theorem proExs_ok {tbl N N' B' Wl Wc W T s p t i D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWl : Wl = layoutPieces) (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 1 p) (hr : (^∃ p) ∈ s) (ht : IsSemiterm LAct 0 t)
    ...
    (hiE : i + 2 * ((1 + D) * (1 + D + D)) * D + 40 * D + 20 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout walkPieces Wc T Γ s i) :
    ListOK tbl E ((9 : ℕ) : V) Γ (proExs walkPieces Wl Wc W T s p t i) ∧ NoDrop' (proExs walkPieces Wl Wc W T s p t i) ∧
    shiftsV (proExs walkPieces Wl Wc W T s p t i) =
      exsSig walkPieces Wc T s p t i + (1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)) ∧
    Layout walkPieces Wc T (finalCtx Γ (proExs walkPieces Wl Wc W T s p t i)) (insert (substs1 LAct t p) s) 0 ∧
    Layout walkPieces Wc T (finalCtx Γ (proExs walkPieces Wl Wc W T s p t i)) s
      (i + exsSig walkPieces Wc T s p t i + 1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)) ∧
    neg LAct (tPiFact (𝟎 : V) (^&(exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i + 1 + 1 +
      proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)))) ∈ finalCtx Γ (proExs walkPieces Wl Wc W T s p t i) ∧
    neg LAct (tlenFact (^&(exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i + 1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)))
      (^&(exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i + 1 + 1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)))) ∈
      finalCtx Γ (proExs walkPieces Wl Wc W T s p t i) ∧
    neg LAct (leFact (^&(exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i + 1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)))
      (bnum (termLen LAct t))) ∈ finalCtx Γ (proExs walkPieces Wl Wc W T s p t i) ∧
    neg LAct (substs1Fact (^&(exsSc walkPieces Wc T s p t i + 1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)))
      (^&(exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i + 1 + 1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)))
      (^&(memTop walkPieces Wc T s (^∃ p) (i + exsSig walkPieces Wc T s p t i + 1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)) + 1))) ∈
      finalCtx Γ (proExs walkPieces Wl Wc W T s p t i) ∧
    neg LAct (insFact (^&(proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)))
      (^&(exsSc walkPieces Wc T s p t i + 1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)))
      (^&(i + exsSig walkPieces Wc T s p t i + 1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s) + (len (memberList s) + 1)))) ∈
      finalCtx Γ (proExs walkPieces Wl Wc W T s p t i) ∧
    neg LAct (eqFactB (^&(proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)))
      (^&(0 + (len (memberList (insert (substs1 LAct t p) s)) + 1)))) ∈ finalCtx Γ (proExs walkPieces Wl Wc W T s p t i) := by
  have h89 : ((8 : ℕ) : V) ≤ ((9 : ℕ) : V) := by exact_mod_cast (by decide : (8 : ℕ) ≤ 9)
  have hpt : IsSemiformula LAct 0 (substs1 LAct t p) := hp.substs1 ht
  ...
  have hk1 : 1 ≤ len (memberList s) := by
    ...
    have := lt_iff_succ_le.mp (lt_of_le_of_lt zero_le hlt); rwa [zero_add] at this
  -- (1) the certifications
  obtain ⟨cok, cnd, csh, hsc', hsig', cLay, cDpt, ctpi, ctl, cle, csub, _⟩ :=
    exsCert_ok htbl hP htblN hWc hWp hs hp hr ht hsD htD hptD hE hEQ hiE hΓ hLay
  obtain ⟨Γ₁, hΓ₁⟩ : ∃ Γ', Γ' = finalCtx Γ (exsCert walkPieces Wc W T s p t i) := ⟨_, rfl⟩
  have hΓ₁f : IsFormulaSet LAct Γ₁ := by rw [hΓ₁]; exact finalCtx_isFormulaSet 9 htbl hΓ cok
  have cLay₁ : Layout walkPieces Wc T Γ₁ s (i + exsSig walkPieces Wc T s p t i) := by rw [hΓ₁]; exact cLay
  have cDpt₁ : DossF walkPieces Γ₁ 0 (substs1 LAct t p) (exsSc walkPieces Wc T s p t i) := by rw [hΓ₁]; exact cDpt
  have ctpi₁ := ctpi; rw [← hΓ₁] at ctpi₁
  have ctl₁ := ctl; rw [← hΓ₁] at ctl₁
  have cle₁ := cle; rw [← hΓ₁] at cle₁
  have csub₁ := csub; rw [← hΓ₁] at csub₁
  -- (2) `proIns s (substs1 t p)`
  have hiE'' : i + exsSig walkPieces Wc T s p t i + 14 * D + 5 ≤ E := by
    calc i + exsSig walkPieces Wc T s p t i + 14 * D + 5 ≤ i + (2 * ((1 + D) * (1 + D + D)) * D + 6 * D + 1) + 14 * D + 5 :=
          add_le_add (add_le_add (add_le_add le_rfl hsig') le_rfl) le_rfl
      _ = i + 2 * ((1 + D) * (1 + D + D)) * D + 20 * D + 6 := by ring
      _ ≤ i + 2 * ((1 + D) * (1 + D + D)) * D + 40 * D + 20 :=
          add_le_add (add_le_add le_rfl (mul_le_mul_of_nonneg_right (by norm_num) zero_le)) (by norm_num)
      _ ≤ E := hiE
  have hipE : exsSc walkPieces Wc T s p t i + 8 * D + 4 ≤ E := by
    calc exsSc walkPieces Wc T s p t i + 8 * D + 4 ≤ 2 * ((1 + D) * (1 + D + D)) * D + 8 * D + 4 := add_le_add (add_le_add hsc' le_rfl) le_rfl
      _ = 0 + 2 * ((1 + D) * (1 + D + D)) * D + 8 * D + 4 := by ring
      _ ≤ i + 2 * ((1 + D) * (1 + D + D)) * D + 40 * D + 20 :=
          add_le_add (add_le_add (add_le_add zero_le le_rfl) (mul_le_mul_of_nonneg_right (by norm_num) zero_le)) (by norm_num)
      _ ≤ E := hiE
  obtain ⟨iok, ind, ish, iLay, iF, ifact⟩ := proIns_ok htbl hP htblN hWl hWc hWp hs hpt hk1 hcD hE hiE'' hipE hΓ₁f cLay₁ cDpt₁
  -- assembly
  have e₁ : exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i + 1 + (1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)) =
      exsIw walkPieces Wc T t p + exsSc walkPieces Wc T s p t i + 1 + 1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s) := by ring
  have e₂ : exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i + (1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)) =
      exsC3 walkPieces t p + exsSc walkPieces Wc T s p t i + 1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s) := by ring
  have e₃ : exsSc walkPieces Wc T s p t i + (1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)) =
      exsSc walkPieces Wc T s p t i + 1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s) := by ring
  have e₄ : memTop walkPieces Wc T s (^∃ p) (i + exsSig walkPieces Wc T s p t i) + 1 + (1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)) =
      memTop walkPieces Wc T s (^∃ p) (i + exsSig walkPieces Wc T s p t i + 1 + proSig walkPieces Wl Wc W T (insert (substs1 LAct t p) s)) + 1 := by
    unfold memTop mTop; ring
  unfold proExs
  refine ⟨listOK_appendV cok (by rw [← hΓ₁]; exact iok.mono h89), noDrop'_appendV cnd ind, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_, ?_⟩
  · rw [shiftsV_appendV, csh, ish]
  · rw [finalCtx_appendV, ← hΓ₁]; exact iLay
  · rw [finalCtx_appendV, ← hΓ₁]; exact iF.parent
  · rw [finalCtx_appendV, ← hΓ₁]
    have := mem_finalCtx_of_mem' ind ctpi₁
    rwa [ish, shiftIterV_neg (isFormula_tPiFact isSemiterm_zeroV (by simp)), shiftIterV_tPiFact isSemiterm_zeroV (by simp),
      termShiftIterV_zeroV, termShiftIterV_fvar, e₁] at this
  · rw [finalCtx_appendV, ← hΓ₁]
    have := mem_finalCtx_of_mem' ind ctl₁
    rwa [ish, shiftIterV_neg (isFormula_tlenFact (by simp) (by simp)), shiftIterV_tlenFact (by simp) (by simp),
      termShiftIterV_fvar, termShiftIterV_fvar, e₁, e₂] at this
  · rw [finalCtx_appendV, ← hΓ₁]
    have := mem_finalCtx_of_mem' ind cle₁
    rwa [ish, shiftIterV_neg (isFormula_leFact (by simp) (isSemiterm_bnum0 _)), shiftIterV_leFact (by simp) (isSemiterm_bnum0 _),
      termShiftIterV_fvar, termShiftIterV_bnum', e₂] at this
  · rw [finalCtx_appendV, ← hΓ₁]
    have := mem_finalCtx_of_mem' ind csub₁
    rwa [ish, shiftIterV_neg (isFormula_substs1Fact (by simp) (by simp) (by simp)), shiftIterV_substs1Fact (by simp) (by simp) (by simp),
      termShiftIterV_fvar, termShiftIterV_fvar, termShiftIterV_fvar, e₁, e₃, e₄] at this
  · rw [finalCtx_appendV, ← hΓ₁]; exact iF.cp
  · rw [finalCtx_appendV, ← hΓ₁]; exact ifact

end exsChild

/-! ## 15. Sizes and costs of the `all`/`exs` prologues (§11's shape at cap `9`)

Both prologues are size-disciplined at the layout class `(layQ B B' D, layD N' B' D)`: the certification blocks are
Horn-only (`allCert`) or Horn-only with one `eqTotal` and the term `lenT` at `(lenQ B' D, lenD N' B' D)` (`exsCert`), the
layouts/`proShift`/`proIns` pieces are those of §11, and every loop is Horn-only. The cost bounds are
`costSum_le_of_sizeOK 9` (the lists run at cap `9`). -/

section proSizes2

set_option maxHeartbeats 4000000 in
/-- **`proSS` is size-disciplined** at the layout class (the hypotheses are those of `proSS_ok`). -/
theorem sizeOK_proSS {tbl N N' B' B Wl Wc W T s i D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWl : Wl = layoutPieces) (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hPle : formulaLen LAct (Ple : V) ≤ B)
    (hs : IsFormulaSet LAct s) (hk1 : 1 ≤ len (memberList s)) (hsD : setLen LAct s ≤ D)
    (hvD : setLen LAct (setShift LAct s) ≤ D)
    (hE : 13 * D + 18 * ‖D‖ + 12 ≤ E) (hiE : i + 22 * D + 8 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout walkPieces Wc T Γ s i) :
    SizeOK (layQ B B' D) (layD N' B' D) (proSS walkPieces Wl Wc W T s i) := by
  have hV : IsFormulaSet LAct (setShift LAct s) := hs.setShift
  have hkV1 : 1 ≤ len (memberList (setShift LAct s)) := one_le_len_memberList_setShift rfl hk1
  have hkD : len (memberList s) ≤ D := le_trans (len_memberList_le_setLen hs) hsD
  have hkVD : len (memberList (setShift LAct s)) ≤ D := le_trans (len_memberList_le_setLen hV) hvD
  ...
  have hvt : 0 + 1 + (len (memberList s) + 1) + proSig walkPieces Wl Wc W T s + (len (memberList (setShift LAct s)) + 1) + 1 ≤ E := by
    calc 0 + 1 + (len (memberList s) + 1) + proSig walkPieces Wl Wc W T s + (len (memberList (setShift LAct s)) + 1) + 1
      ...
      _ ≤ 13 * D + 18 * ‖D‖ + 12 := eight_le_cap (by norm_num)
      _ ≤ E := hE
  obtain ⟨_, _, sho, _, _⟩ := ssIdent_ok htbl hP hWl hWc hWp hs hk1 hsD hE hi₁ hvt hΓ₂f pLc₂ hLay₂ pfact₂
  unfold proSS
  exact sizeOK_appendV (sizeOK_layoutSteps hWl hWc hWp htblN hPle hV hvD)
    (sizeOK_appendV (sizeOK_proShift htbl hP htblN hWl hWc hWp hPle hV hs rfl hk1 hvD hsD hE h16 hΓ₁f lLay₁)
      (sizeOK_of_hornOnly sho))

set_option maxHeartbeats 4000000 in
/-- **`proAll` is size-disciplined** at the layout class (the hypotheses are those of `proAll_ok`). -/
theorem sizeOK_proAll {tbl N N' B' B Wl Wc W T s p i D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWl : Wl = layoutPieces) (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hPle : formulaLen LAct (Ple : V) ≤ B)
    (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 1 p) (hr : (^∀ p) ∈ s)
    (hsD : setLen LAct s ≤ D) (hcD : setLen LAct (insert (free LAct p) (setShift LAct s)) ≤ D)
    ...
    (hiE : i + 2 * ((1 + D) * (1 + D + 1)) * D + 40 * D + 20 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout walkPieces Wc T Γ s i) :
    SizeOK (layQ B B' D) (layD N' B' D) (proAll walkPieces Wl Wc W T s p i) := by
  have hV : IsFormulaSet LAct (setShift LAct s) := hs.setShift
  have hfp : IsSemiformula LAct 0 (free LAct p) := hp.free
  have hk1 : 1 ≤ len (memberList s) := by
    obtain ⟨hlt, _⟩ := idxOf_spec hr
  ...
  have hsSS : proSig walkPieces Wl Wc W T (setShift LAct s) + (1 + (len (memberList s) + 1) + proSig walkPieces Wl Wc W T s) ≤ 13 * D + 4 := by
    calc proSig walkPieces Wl Wc W T (setShift LAct s) + (1 + (len (memberList s) + 1) + proSig walkPieces Wl Wc W T s)
        ≤ (6 * D + 1) + (1 + (D + 1) + (6 * D + 1)) := add_le_add hσV (add_le_add (add_le_add le_rfl (add_le_add hkD le_rfl)) hσS)
      _ = 13 * D + 4 := by ring
  -- the states (as in `proAll_ok`)
  obtain ⟨aok, and_, aho, ash, hcf, hcA, aLay, aDfp, _⟩ := allCert_ok htbl hP hWc hs hp hr hsD hspD hE hEQ hiE hΓ hLay
  obtain ⟨Γ₁, hΓ₁⟩ : ∃ Γ', Γ' = finalCtx Γ (allCert walkPieces Wc T s p i) := ⟨_, rfl⟩
  have hΓ₁f : IsFormulaSet LAct Γ₁ := by rw [hΓ₁]; exact finalCtx_isFormulaSet 9 htbl hΓ aok
  have aLay₁ : Layout walkPieces Wc T Γ₁ s (i + allCertSig walkPieces Wc p) := by rw [hΓ₁]; exact aLay
  have aDfp₁ : DossF walkPieces Γ₁ 0 (free LAct p) (allCf walkPieces Wc p) := by rw [hΓ₁]; exact aDfp
  have hiE' : i + allCertSig walkPieces Wc p + 22 * D + 8 ≤ E := by
    calc i + allCertSig walkPieces Wc p + 22 * D + 8 ≤ i + (4 * D + 2 + 2 * ((1 + D) * (1 + D + 1)) * D) + 22 * D + 8 :=
      ...
      _ ≤ i + 2 * ((1 + D) * (1 + D + 1)) * D + 40 * D + 20 :=
          add_le_add (add_le_add le_rfl (mul_le_mul_of_nonneg_right (by norm_num) zero_le)) (by norm_num)
      _ ≤ E := hiE
  obtain ⟨sok, snd, ssh, sLv, _, _⟩ := proSS_ok htbl hP htblN hWl hWc hWp hs hk1 hsD hvD hE hiE' hΓ₁f aLay₁
  obtain ⟨Γ₂, hΓ₂⟩ : ∃ Γ', Γ' = finalCtx Γ₁ (proSS walkPieces Wl Wc W T s (i + allCertSig walkPieces Wc p)) := ⟨_, rfl⟩
  have hΓ₂f : IsFormulaSet LAct Γ₂ := by rw [hΓ₂]; exact finalCtx_isFormulaSet 8 htbl hΓ₁f sok
  have sLv₂ : Layout walkPieces Wc T Γ₂ (setShift LAct s) (0 + 1 + (len (memberList s) + 1) + proSig walkPieces Wl Wc W T s) := by
    rw [hΓ₂]; exact sLv
  ...
  have hipE : allCf walkPieces Wc p + (proSig walkPieces Wl Wc W T (setShift LAct s) + (1 + (len (memberList s) + 1) + proSig walkPieces Wl Wc W T s)) +
    ...
    calc allCf walkPieces Wc p + (proSig walkPieces Wl Wc W T (setShift LAct s) + (1 + (len (memberList s) + 1) + proSig walkPieces Wl Wc W T s)) + 8 * D + 4
      ...
      _ ≤ E := hiE
  unfold proAll
  exact sizeOK_appendV (sizeOK_of_hornOnly aho)
    (sizeOK_appendV (sizeOK_proSS htbl hP htblN hWl hWc hWp hPle hs hk1 hsD hvD hE hiE' hΓ₁f aLay₁)
      (sizeOK_proIns htbl hP htblN hWl hWc hWp hPle hV hfp hkV1 hcD hE hiE'' hipE hΓ₂f sLv₂ aDfp₂))

/-- **The cost of `proAll`** in the `costSum_le_of_sizeOK 9` shape. -/
theorem costSum_proAll_le {tbl N N' B' B Wl Wc W T s p i D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    ...
    (hiE : i + 2 * ((1 + D) * (1 + D + 1)) * D + 40 * D + 20 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout walkPieces Wc T Γ s i) :
    costSum N E Γ (proAll walkPieces Wl Wc W T s p i) ≤ len (proAll walkPieces Wl Wc W T s p i) *
      (costK N E B 9 (layQ B B' D) (layD N' B' D) + (3 * ((9 : ℕ) : V) + 11) *
        (ctxBoundG (growK B E (layQ B B' D)) Γ (len (proAll walkPieces Wl Wc W T s p i)) +
          (fvOccS LAct Γ + len (proAll walkPieces Wl Wc W T s p i) * growK B E (layQ B B' D)))) := by
  obtain ⟨ok, _⟩ := proAll_ok htbl hP htblN hWl hWc hWp hs hp hr hsD hcD hspD hE hEQ hiE hΓ hLay
  have hE1 : (1 : V) ≤ E := le_trans (by norm_num) (le_trans le_add_self hE)
  exact costSum_le_of_sizeOK 9 hE1 htbl hBt ok (sizeOK_proAll htbl hP htblN hWl hWc hWp hPle hs hp hr hsD hcD hspD hE hEQ hiE hΓ hLay)

set_option maxHeartbeats 4000000 in
/-- **`proExs` is size-disciplined** at the layout class (the hypotheses are those of `proExs_ok`). -/
theorem sizeOK_proExs {tbl N N' B' B Wl Wc W T s p t i D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWl : Wl = layoutPieces) (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hPle : formulaLen LAct (Ple : V) ≤ B)
    (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 1 p) (hr : (^∃ p) ∈ s) (ht : IsSemiterm LAct 0 t)
    (hsD : setLen LAct s ≤ D) (hcD : setLen LAct (insert (substs1 LAct t p) s) ≤ D) (htD : termLen LAct t ≤ D)
    ...
    (hiE : i + 2 * ((1 + D) * (1 + D + D)) * D + 40 * D + 20 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout walkPieces Wc T Γ s i) :
    SizeOK (layQ B B' D) (layD N' B' D) (proExs walkPieces Wl Wc W T s p t i) := by
  have hpt : IsSemiformula LAct 0 (substs1 LAct t p) := hp.substs1 ht
  have hptD : formulaLen LAct (substs1 LAct t p) ≤ D := le_trans (formulaLen_le_setLen_of_mem (by simp)) hcD
  have hk1 : 1 ≤ len (memberList s) := by
    obtain ⟨hlt, _⟩ := idxOf_spec hr
    have := lt_iff_succ_le.mp (lt_of_le_of_lt zero_le hlt); rwa [zero_add] at this
  obtain ⟨cok, _, _, hsc', hsig', cLay, cDpt, _, _, _, _, csz⟩ :=
    exsCert_ok htbl hP htblN hWc hWp hs hp hr ht hsD htD hptD hE hEQ hiE hΓ hLay
  obtain ⟨Γ₁, hΓ₁⟩ : ∃ Γ', Γ' = finalCtx Γ (exsCert walkPieces Wc W T s p t i) := ⟨_, rfl⟩
  have hΓ₁f : IsFormulaSet LAct Γ₁ := by rw [hΓ₁]; exact finalCtx_isFormulaSet 9 htbl hΓ cok
  have cLay₁ : Layout walkPieces Wc T Γ₁ s (i + exsSig walkPieces Wc T s p t i) := by rw [hΓ₁]; exact cLay
  ...
  have hipE : exsSc walkPieces Wc T s p t i + 8 * D + 4 ≤ E := by
    calc exsSc walkPieces Wc T s p t i + 8 * D + 4 ≤ 2 * ((1 + D) * (1 + D + D)) * D + 8 * D + 4 := add_le_add (add_le_add hsc' le_rfl) le_rfl
      ...
      _ ≤ E := hiE
  unfold proExs
  exact sizeOK_appendV (csz B) (sizeOK_proIns htbl hP htblN hWl hWc hWp hPle hs hpt hk1 hcD hE hiE'' hipE hΓ₁f cLay₁ cDpt₁)

/-- **The cost of `proExs`** in the `costSum_le_of_sizeOK 9` shape. -/
theorem costSum_proExs_le {tbl N N' B' B Wl Wc W T s p t i D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    ...
    (hiE : i + 2 * ((1 + D) * (1 + D + D)) * D + 40 * D + 20 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout walkPieces Wc T Γ s i) :
    costSum N E Γ (proExs walkPieces Wl Wc W T s p t i) ≤ len (proExs walkPieces Wl Wc W T s p t i) *
      (costK N E B 9 (layQ B B' D) (layD N' B' D) + (3 * ((9 : ℕ) : V) + 11) *
        (ctxBoundG (growK B E (layQ B B' D)) Γ (len (proExs walkPieces Wl Wc W T s p t i)) +
          (fvOccS LAct Γ + len (proExs walkPieces Wl Wc W T s p t i) * growK B E (layQ B B' D)))) := by
  obtain ⟨ok, _⟩ := proExs_ok htbl hP htblN hWl hWc hWp hs hp hr ht hsD hcD htD hE hEQ hiE hΓ hLay
  have hE1 : (1 : V) ≤ E := le_trans (by norm_num) (le_trans le_add_self hE)
  exact costSum_le_of_sizeOK 9 hE1 htbl hBt ok
    (sizeOK_proExs htbl hP htblN hWl hWc hWp hPle hs hp hr ht hsD hcD htD hE hEQ hiE hΓ hLay)

end proSizes2

/-! ## 16. The `insert` child of the EMPTY parent: `proIns0` (§12's leftover — the `cut` at `∅`)

`proIns` needs `1 ≤ k` (the parent's `subChain` and Loop B run over the parent's members); at `s = ∅` the identification
`eqFactB cp s''` is instead: Loop A (every child member — the single `p` — is in `cp`), the child's `subChain`
(`s'' ⊆ cp`), `emptySubsetC [s'']` and the table row `congSubsetL [𝟎, S, s'']` (`S ⊆ s''` from the `Layout0` fact
`S = 𝟎`), the `p` block, `insertSubset [S, s'', P, cp]`, `subsetAntisymm`. -/

section insertChild0

/-- **`identIns0 q`**: Loop A, `s'' ⊆ cp`, `emptySubsetC [s'']`, `congSubsetL [𝟎, S, s'']`, the `p` block,
`insertSubset [S, s'', P, cp]`, `subsetAntisymm [cp, s'']`. -/
noncomputable def identIns0 (q : V) : V :=
  appendV (loopA q (qK' q)) (appendV (subChain (qW q) 0 (qK' q) (qOs' q) (^&(qSig q)))
    (appendV ?[mkStep (qW q) 81 ?[^&(0 + (qK' q + 1))],
        mkStep (qW q) 157 ?[(𝟎 : V), ^&(qI q + (qK q + 1)), ^&(0 + (qK' q + 1))]]
      (appendV (blockP q)
        ?[mkStep (qW q) 112 ?[^&(qI q + (qK q + 1)), ^&(0 + (qK' q + 1)), ^&(qIp q), ^&(qSig q)],
          mkStep (qW q) 75 ?[^&(qSig q), ^&(0 + (qK' q + 1))]])))

noncomputable def identIns0Def : 𝚺₁.Semisentence 2 := .mkSigma
  “y q. ∃ Wl, !pi₁Def Wl q ∧ ∃ r1, !pi₂Def r1 q ∧ ∃ W, !pi₁Def W r1 ∧ ∃ r2, !pi₂Def r2 r1 ∧ ∃ i, !pi₁Def i r2 ∧ ∃ r3, !pi₂Def r3 r2 ∧ ∃ k, !pi₁Def k r3 ∧ ∃ r4, !pi₂Def r4 r3 ∧ ∃ os, !pi₁Def os r4 ∧ ∃ r5, !pi₂Def r5 r4 ∧ ∃ xs, !pi₁Def xs r5 ∧ ∃ r6, !pi₂Def r6 r5 ∧ ∃ ip, !pi₁Def ip r6 ∧ ∃ r7, !pi₂Def r7 r6 ∧ ∃ kk, !pi₁Def kk r7 ∧ ∃ r8, !pi₂Def r8 r7 ∧ ∃ oss, !pi₁Def oss r8 ∧ ∃ r9, !pi₂Def r9 r8 ∧ ∃ ys, !pi₁Def ys r9 ∧ ∃ r10, !pi₂Def r10 r9 ∧ ∃ p, !pi₁Def p r10 ∧ ∃ sg, !pi₂Def sg r10 ∧
    ∃ A, !loopADef A q kk ∧ ∃ zc, !qqFvarDef zc sg ∧ ∃ C₁, !subChainDef C₁ W 0 kk oss zc ∧
    ∃ is, is = 0 + (kk + 1) ∧ ∃ zs, !qqFvarDef zs is ∧ ∃ iS, iS = i + (k + 1) ∧ ∃ zS, !qqFvarDef zS iS ∧ ∃ z, !cTVGraph z 0 ∧
    ∃ e₁, !adjoinDef e₁ zs 0 ∧ ∃ t₁, !mkStepDef t₁ W 81 e₁ ∧
    ∃ f₂, !adjoinDef f₂ zS e₁ ∧ ∃ f₃, !adjoinDef f₃ z f₂ ∧ ∃ t₂, !mkStepDef t₂ W 157 f₃ ∧
    ∃ m₂, !adjoinDef m₂ t₂ 0 ∧ ∃ m₁, !adjoinDef m₁ t₁ m₂ ∧
    ∃ P, !blockPDef P q ∧ ∃ zip, !qqFvarDef zip ip ∧
    ∃ a₁, !adjoinDef a₁ zc 0 ∧ ∃ a₂, !adjoinDef a₂ zip a₁ ∧ ∃ a₃, !adjoinDef a₃ zs a₂ ∧ ∃ a₄, !adjoinDef a₄ zS a₃ ∧
    ∃ s₁, !mkStepDef s₁ W 112 a₄ ∧
    ∃ b₁, !adjoinDef b₁ zs 0 ∧ ∃ b₂, !adjoinDef b₂ zc b₁ ∧ ∃ s₂, !mkStepDef s₂ W 75 b₂ ∧
    ∃ l₂, !adjoinDef l₂ s₂ 0 ∧ ∃ l₁, !adjoinDef l₁ s₁ l₂ ∧
    ∃ r₄, !appendVDef r₄ P l₁ ∧ ∃ r₃, !appendVDef r₃ m₁ r₄ ∧ ∃ r₂, !appendVDef r₂ C₁ r₃ ∧ !appendVDef y A r₂”

set_option maxHeartbeats 1000000 in
instance identIns0_defined : 𝚺₁-Function₁ (identIns0 : V → V) via identIns0Def := .mk fun v ↦ by
  simp [identIns0Def, identIns0, qWl, qW, qI, qK, qOs, qXs, qIp, qK', qOs', qYs, qP, qSig, loopA_defined.iff,
    blockP_defined.iff, subChain_defined.iff, mkStep_defined.iff, appendV_defined.iff, cTV.defined.iff, cTV_zero,
    numeral_eq_natCast]
instance identIns0_definable : 𝚺₁-Function₁ (identIns0 : V → V) := identIns0_defined.to_definable

set_option maxHeartbeats 4000000 in
/-- **The identification at the empty parent is applicable** and leaves `eqFactB cp s''`. -/
theorem identIns0_ok {tbl N Wl W Wc T p i₁ ip₁ σ D E Γ q : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (hWl : Wl = layoutPieces) (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hq : q = qPack Wl W i₁ (len (memberList (0 : V))) (offVec (memberList (0 : V)) walkPieces Wc T) (memberList (0 : V)) ip₁
      (len (memberList (insert p (0 : V)))) (offVec (memberList (insert p (0 : V))) walkPieces Wc T) (memberList (insert p (0 : V))) p σ)
    (hp : IsSemiformula LAct 0 p) (hsD : setLen LAct (insert p (0 : V)) ≤ D)
    (hcap : IdCap D E i₁ ip₁ σ) (hΓ : IsFormulaSet LAct Γ) (hF : IdFrame walkPieces Wc T Γ (0 : V) p i₁ ip₁ σ)
    (h0 : neg LAct (eqFactB (^&(i₁ + (len (memberList (0 : V)) + 1))) (𝟎 : V)) ∈ Γ) :
    ListOK tbl E ((8 : ℕ) : V) Γ (identIns0 q) ∧ NoDrop (identIns0 q) ∧ HornOnly (identIns0 q) ∧
    shiftsV (identIns0 q) = 0 ∧
    neg LAct (eqFactB (^&σ) (^&(0 + (len (memberList (insert p (0 : V))) + 1)))) ∈ finalCtx Γ (identIns0 q) := by
  have hW := hP.walkTable
  have hL := hP.layoutTable
  have hF1 := hP.frag1Table
  have hs : IsFormulaSet LAct (0 : V) := fun x hx ↦ by simp at hx
  have hcs : IsFormulaSet LAct (insert p (0 : V)) := IsFormulaSet.insert_iff.mpr ⟨hp, hs⟩
  have hsD' : setLen LAct (0 : V) ≤ D := le_trans (setLen_le_insert p 0) hsD
  have hkD : len (memberList (0 : V)) ≤ D := le_trans (len_memberList_le_setLen hs) hsD'
  have hk'D : len (memberList (insert p (0 : V))) ≤ D := le_trans (len_memberList_le_setLen hcs) hsD
  have hk1' : 1 ≤ len (memberList (insert p (0 : V))) := one_le_len_memberList_insert p 0
  have e112 : ∀ ev : V, mkStep proPieces (112 : V) ev = mkStep frag1Pieces (112 : V) ev := fun ev ↦ by
    have := mkStep_pro_frag1 112 (by decide) ev; simpa using this
  have e75 : ∀ ev : V, mkStep proPieces (75 : V) ev = mkStep layoutPieces (75 : V) ev := fun ev ↦ by
    have := mkStep_pro_layout 75 (by decide) ev; simpa using this
  have e81 : ∀ ev : V, mkStep proPieces (81 : V) ev = mkStep layoutPieces (81 : V) ev := fun ev ↦ by
    have := mkStep_pro_layout 81 (by decide) ev; simpa using this
  have hE1 : (1 : V) ≤ E := le_trans (by norm_num) (le_trans le_add_self hcap.hE)
  have hs''E : 0 + (len (memberList (insert p (0 : V))) + 1) + 1 ≤ E := by
    calc 0 + (len (memberList (insert p (0 : V))) + 1) + 1 = len (memberList (insert p (0 : V))) + 2 := by ring
      _ ≤ 6 * D + 2 := add_le_add (le_trans hk'D (le_mul_of_one_le_left zero_le (by norm_num))) le_rfl
      _ ≤ 13 * D + 18 * ‖D‖ + 12 := six_le_cap (by norm_num)
      _ ≤ E := hcap.hE
  have hSE : i₁ + (len (memberList (0 : V)) + 1) + 1 ≤ E := by
    calc i₁ + (len (memberList (0 : V)) + 1) + 1 = i₁ + (len (memberList (0 : V)) + 2) := by ring
      _ ≤ i₁ + (8 * D + 3) := add_le_add le_rfl (add_le_add (le_trans hkD (le_mul_of_one_le_left zero_le (by norm_num))) (by norm_num))
      _ = i₁ + 8 * D + 3 := by ring
      _ ≤ E := hcap.hi
  have hipE : ip₁ + 1 ≤ E := le_trans (add_le_add le_rfl (by norm_num)) (le_trans (add_le_add le_self_add le_rfl) hcap.hip)
  have hkE' : 0 + (2 * len (memberList (insert p (0 : V))) + 2) ≤ E := by
    calc 0 + (2 * len (memberList (insert p (0 : V))) + 2) ≤ 6 * D + 2 := by
          rw [zero_add]
          exact add_le_add (le_trans (mul_le_mul_of_nonneg_left hk'D zero_le) (mul_le_mul_of_nonneg_right (by norm_num) zero_le)) le_rfl
      _ ≤ 13 * D + 18 * ‖D‖ + 12 := six_le_cap (by norm_num)
      _ ≤ E := hcap.hE
  have hosE' : ∀ j < len (memberList (insert p (0 : V))),
      mTop 0 (len (memberList (insert p (0 : V)))) (offVec (memberList (insert p (0 : V))) walkPieces Wc T).[j] + 1 ≤ E := fun j hj ↦ by
    calc _ ≤ 0 + 6 * D + 1 + 1 := add_le_add (mTop_le htbl hW hWc T hcs hsD hj) le_rfl
      _ = 6 * D + 2 := by ring
      _ ≤ 13 * D + 18 * ‖D‖ + 12 := six_le_cap (by norm_num)
      _ ≤ E := hcap.hE
  -- Loop A
  obtain ⟨aok, and_, aho, ash, afacts⟩ := loopA_ok htbl hP hWl hWc hWp hq hs hp hsD hcap hΓ hF _ le_rfl
  obtain ⟨Γ₁, hΓ₁⟩ : ∃ Γ', Γ' = finalCtx Γ (loopA q (len (memberList (insert p (0 : V))))) := ⟨_, rfl⟩
  have hΓ₁f : IsFormulaSet LAct Γ₁ := by rw [hΓ₁]; exact finalCtx_isFormulaSet 8 htbl hΓ aok
  have tr₁ : ∀ x ∈ Γ, x ∈ Γ₁ := fun x hx ↦ by rw [hΓ₁]; exact tr_of_zero and_ ash hx
  have hF₁ : IdFrame walkPieces Wc T Γ₁ (0 : V) p i₁ ip₁ σ := by rw [hΓ₁]; exact hF.zero_transport and_ ash
  -- the child chain is in `cp`
  have hin₁ : SubIn Γ₁ 0 (len (memberList (insert p (0 : V)))) (offVec (memberList (insert p (0 : V))) walkPieces Wc T) (^&σ) :=
    fun j hj ↦ ⟨(hF₁.child.1 j hj).2.2.2.1, by
      have hK' : qK' q = len (memberList (insert p (0 : V))) := by rw [hq, qK'_pack]
      have hOs' : qOs' q = offVec (memberList (insert p (0 : V))) walkPieces Wc T := by rw [hq, qOs'_pack]
      have hSig : qSig q = σ := by rw [hq, qSig_pack]
      have := afacts j hj; rw [hK', hOs', hSig] at this; rw [hΓ₁]; exact this⟩
  obtain ⟨c1ok, c1nd, c1ho, c1sh, _, c1fact⟩ := subChain_ok htbl hP hWp hΓ₁f (by simp) (termLen_fvar_le' hcap.hσ)
    hk1' hkE' hosE' (len_offVec _ _ _ _) hin₁
  obtain ⟨Γ₂, hΓ₂⟩ : ∃ Γ', Γ' = finalCtx Γ₁ (subChain W 0 (len (memberList (insert p (0 : V)))) (offVec (memberList (insert p (0 : V))) walkPieces Wc T) (^&σ)) := ⟨_, rfl⟩
  have hΓ₂f : IsFormulaSet LAct Γ₂ := by rw [hΓ₂]; exact finalCtx_isFormulaSet 8 htbl hΓ₁f c1ok
  have tr₂ : ∀ x ∈ Γ₁, x ∈ Γ₂ := fun x hx ↦ by rw [hΓ₂]; exact tr_of_zero c1nd c1sh hx
  have hF₂ : IdFrame walkPieces Wc T Γ₂ (0 : V) p i₁ ip₁ σ := by rw [hΓ₂]; exact hF₁.zero_transport c1nd c1sh
  have c1fact₂ : neg LAct (subsetFact (^&(0 + (len (memberList (insert p (0 : V))) + 1))) (^&σ)) ∈ Γ₂ := by rw [hΓ₂]; exact c1fact
  -- `S ⊆ s''`: `emptySubsetC [s'']`, `congSubsetL [𝟎, S, s'']`
  obtain ⟨ok₃, tg₃, cx₃⟩ := lok_emptySubsetC htbl hL rfl hΓ₂f (by simp) (termLen_fvar_le' hs''E)
  rw [← e81, ← hWp] at ok₃ tg₃ cx₃
  have hΓ₃f : IsFormulaSet LAct (insert (neg LAct (subsetFact (𝟎 : V) (^&(0 + (len (memberList (insert p (0 : V))) + 1))))) Γ₂) := by
    rw [← cx₃]; exact isFormulaSet_ctxAfter 8 htbl ok₃
  have hrow := hP.congSubsetL
  obtain ⟨ok₄, tg₄, cx₄⟩ := pok_congSubsetL htbl hWp hrow.1 ⟨hrow.2.1, hrow.2.2⟩ hΓ₃f isSemiterm_zeroV (termLen_zeroV_le hE1)
    (by simp) (termLen_fvar_le' hSE) (by simp) (termLen_fvar_le' hs''E) (memIns (tr₂ _ (tr₁ _ h0))) (memInsSelf _ _)
  have h34nd : NoDrop (?[mkStep W 81 ?[^&(0 + (len (memberList (insert p (0 : V))) + 1))],
      mkStep W 157 ?[(𝟎 : V), ^&(i₁ + (len (memberList (0 : V)) + 1)), ^&(0 + (len (memberList (insert p (0 : V))) + 1))]] : V) :=
    noDrop_cons (Or.inl tg₃) (noDrop_single (Or.inl tg₄))
  have h34sh : shiftsV (?[mkStep W 81 ?[^&(0 + (len (memberList (insert p (0 : V))) + 1))],
      mkStep W 157 ?[(𝟎 : V), ^&(i₁ + (len (memberList (0 : V)) + 1)), ^&(0 + (len (memberList (insert p (0 : V))) + 1))]] : V) = 0 := by
    rw [shiftsV_cons_tag0 tg₃, shiftsV_single_tag0 tg₄]
  obtain ⟨Γ₃, hΓ₃⟩ : ∃ Γ', Γ' = finalCtx Γ₂ (?[mkStep W 81 ?[^&(0 + (len (memberList (insert p (0 : V))) + 1))],
      mkStep W 157 ?[(𝟎 : V), ^&(i₁ + (len (memberList (0 : V)) + 1)), ^&(0 + (len (memberList (insert p (0 : V))) + 1))]] : V) := ⟨_, rfl⟩
  have hΓ₃e : Γ₃ = insert (neg LAct (subsetFact (^&(i₁ + (len (memberList (0 : V)) + 1))) (^&(0 + (len (memberList (insert p (0 : V))) + 1)))))
      (insert (neg LAct (subsetFact (𝟎 : V) (^&(0 + (len (memberList (insert p (0 : V))) + 1))))) Γ₂) := by
    rw [hΓ₃, finalCtx_cons, cx₃, finalCtx_single, cx₄]
  have hΓ₃f' : IsFormulaSet LAct Γ₃ := by rw [hΓ₃e, ← cx₄]; exact isFormulaSet_ctxAfter 8 htbl ok₄
  have tr₃ : ∀ x ∈ Γ₂, x ∈ Γ₃ := fun x hx ↦ by rw [hΓ₃e]; exact memIns (memIns hx)
  have hF₃ : IdFrame walkPieces Wc T Γ₃ (0 : V) p i₁ ip₁ σ := by rw [hΓ₃]; exact hF₂.zero_transport h34nd h34sh
  have hSsub₃ : neg LAct (subsetFact (^&(i₁ + (len (memberList (0 : V)) + 1))) (^&(0 + (len (memberList (insert p (0 : V))) + 1)))) ∈ Γ₃ := by
    rw [hΓ₃e]; exact memInsSelf _ _
  -- the `p` block
  obtain ⟨pok, pnd, pho, psh, pfact⟩ := blockP_ok htbl hP hWl hWc hWp hq hs hp hsD hcap hΓ₃f' hF₃
  obtain ⟨Γ₄, hΓ₄⟩ : ∃ Γ', Γ' = finalCtx Γ₃ (blockP q) := ⟨_, rfl⟩
  have hΓ₄f : IsFormulaSet LAct Γ₄ := by rw [hΓ₄]; exact finalCtx_isFormulaSet 8 htbl hΓ₃f' pok
  have tr₄ : ∀ x ∈ Γ₃, x ∈ Γ₄ := fun x hx ↦ by rw [hΓ₄]; exact tr_of_zero pnd psh hx
  have hF₄ : IdFrame walkPieces Wc T Γ₄ (0 : V) p i₁ ip₁ σ := by rw [hΓ₄]; exact hF₃.zero_transport pnd psh
  have pfact₄ : neg LAct (memFact (^&ip₁) (^&(0 + (len (memberList (insert p (0 : V))) + 1)))) ∈ Γ₄ := by rw [hΓ₄]; exact pfact
  -- the two closing steps
  obtain ⟨ok₁, tg₁, cx₁⟩ := fok_insertSubset htbl hF1 rfl hΓ₄f (by simp) (termLen_fvar_le' hSE)
    (by simp) (termLen_fvar_le' hs''E) (by simp) (termLen_fvar_le' hipE) (by simp) (termLen_fvar_le' hcap.hσ)
    (tr₄ _ hSsub₃) pfact₄ hF₄.cp
  rw [← e112, ← hWp] at ok₁ tg₁ cx₁
  have hΓ₅f : IsFormulaSet LAct (insert (neg LAct (subsetFact (^&σ) (^&(0 + (len (memberList (insert p (0 : V))) + 1))))) Γ₄) := by
    rw [← cx₁]; exact isFormulaSet_ctxAfter 8 htbl ok₁
  have hc1' : neg LAct (subsetFact (^&(0 + (len (memberList (insert p (0 : V))) + 1))) (^&σ)) ∈ Γ₄ := tr₄ _ (tr₃ _ c1fact₂)
  obtain ⟨ok₂, tg₂, cx₂⟩ := lok_subsetAntisymm htbl hL rfl hΓ₅f (by simp) (termLen_fvar_le' hcap.hσ)
    (by simp) (termLen_fvar_le' hs''E) (memInsSelf _ _) (memIns hc1')
  rw [← e75, ← hWp] at ok₂ tg₂ cx₂
  -- assembly
  have hlist : identIns0 q = appendV (loopA q (len (memberList (insert p (0 : V)))))
      (appendV (subChain W 0 (len (memberList (insert p (0 : V)))) (offVec (memberList (insert p (0 : V))) walkPieces Wc T) (^&σ))
        (appendV ?[mkStep W 81 ?[^&(0 + (len (memberList (insert p (0 : V))) + 1))],
            mkStep W 157 ?[(𝟎 : V), ^&(i₁ + (len (memberList (0 : V)) + 1)), ^&(0 + (len (memberList (insert p (0 : V))) + 1))]]
          (appendV (blockP q)
            ?[mkStep W 112 ?[^&(i₁ + (len (memberList (0 : V)) + 1)), ^&(0 + (len (memberList (insert p (0 : V))) + 1)), ^&ip₁, ^&σ],
              mkStep W 75 ?[^&σ, ^&(0 + (len (memberList (insert p (0 : V))) + 1))]]))) := by
    unfold identIns0; rw [hq]; simp only [qWl_pack, qW_pack, qI_pack, qK_pack, qOs_pack, qXs_pack, qIp_pack, qK'_pack,
      qOs'_pack, qYs_pack, qP_pack, qSig_pack]
  rw [hlist]
  refine ⟨?_, noDrop_appendV and_ (noDrop_appendV c1nd (noDrop_appendV h34nd (noDrop_appendV pnd
      (noDrop_cons (Or.inl tg₁) (noDrop_single (Or.inl tg₂)))))),
    hornOnly_appendV aho (hornOnly_appendV c1ho (hornOnly_appendV (hornOnly_cons (Or.inl tg₃) (hornOnly_single (Or.inl tg₄)))
      (hornOnly_appendV pho (hornOnly_cons (Or.inl tg₁) (hornOnly_single (Or.inl tg₂)))))), ?_, ?_⟩
  · refine listOK_appendV aok ?_
    rw [← hΓ₁]
    refine listOK_appendV c1ok ?_
    rw [← hΓ₂]
    refine listOK_appendV (listOK_cons ok₃ (by rw [cx₃]; exact listOK_single ok₄)) ?_
    rw [← hΓ₃]
    refine listOK_appendV pok ?_
    rw [← hΓ₄]
    refine listOK_cons ok₁ ?_
    rw [cx₁]
    exact listOK_single ok₂
  · rw [shiftsV_appendV, shiftsV_appendV, shiftsV_appendV, shiftsV_appendV, ash, c1sh, h34sh, psh,
      shiftsV_cons_tag0 tg₁, shiftsV_single_tag0 tg₂]
    simp
  · rw [finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, finalCtx_appendV, ← hΓ₁, ← hΓ₂, ← hΓ₃, ← hΓ₄,
      finalCtx_cons, cx₁, finalCtx_single, cx₂]
...
/-- **`proIns0 p i ip`**: `insertTotalC [P, S]`, the child's layout, `identIns0` — `proIns` at the empty parent
(`i` = the parent's `Layout0` offset: `S = &(i + (len (memberList 0) + 1)) = &(i + 1)`). -/
noncomputable def proIns0 (Ww Wl Wc W T p i ip : V) : V :=
  appendV ?[mkStep W 76 ?[^&ip, ^&(i + (len (memberList (0 : V)) + 1))]]
    (appendV (layoutSteps Ww Wl Wc W T (insert p (0 : V)))
      (identIns0 (qPack Wl W (i + 1 + proSig Ww Wl Wc W T (insert p (0 : V))) (len (memberList (0 : V)))
        (offVec (memberList (0 : V)) Ww Wc T) (memberList (0 : V)) (ip + 1 + proSig Ww Wl Wc W T (insert p (0 : V)))
        (len (memberList (insert p (0 : V)))) (offVec (memberList (insert p (0 : V))) Ww Wc T) (memberList (insert p (0 : V))) p
        (proSig Ww Wl Wc W T (insert p (0 : V))))))

noncomputable def proIns0Def : 𝚺₁.Semisentence 9 := .mkSigma
  “y Ww Wl Wc W T p i ip. ∃ zip, !qqFvarDef zip ip ∧ ∃ xs, !memberListDef xs 0 ∧ ∃ k, !lenDef k xs ∧
    ∃ is, is = i + (k + 1) ∧ ∃ zs, !qqFvarDef zs is ∧ ∃ e₁, !adjoinDef e₁ zs 0 ∧ ∃ e₂, !adjoinDef e₂ zip e₁ ∧
    ∃ s₁, !mkStepDef s₁ W 76 e₂ ∧ ∃ l₁, !adjoinDef l₁ s₁ 0 ∧
    ∃ c, !insertDef c p 0 ∧ ∃ L, !layoutStepsDef L Ww Wl Wc W T c ∧ ∃ σ, !shiftsVDef σ L ∧
    ∃ os, !offVecDef os xs Ww Wc T ∧ ∃ ys, !memberListDef ys c ∧ ∃ kk, !lenDef kk ys ∧ ∃ oss, !offVecDef oss ys Ww Wc T ∧
    ∃ i₁, i₁ = i + 1 + σ ∧ ∃ ip₁, ip₁ = ip + 1 + σ ∧
    ∃ q₁₁, !pairDef q₁₁ p σ ∧ ∃ q₁₀, !pairDef q₁₀ ys q₁₁ ∧ ∃ q₉, !pairDef q₉ oss q₁₀ ∧ ∃ q₈, !pairDef q₈ kk q₉ ∧
    ∃ q₇, !pairDef q₇ ip₁ q₈ ∧ ∃ q₆, !pairDef q₆ xs q₇ ∧ ∃ q₅, !pairDef q₅ os q₆ ∧ ∃ q₄, !pairDef q₄ k q₅ ∧
    ∃ q₃, !pairDef q₃ i₁ q₄ ∧ ∃ q₂, !pairDef q₂ W q₃ ∧ ∃ q, !pairDef q Wl q₂ ∧
    ∃ I, !identIns0Def I q ∧ ∃ r, !appendVDef r L I ∧ !appendVDef y l₁ r”

set_option maxHeartbeats 1000000 in
instance proIns0_defined :
    𝚺₁.DefinedFunction (fun v : Fin 8 → V ↦ proIns0 (v 0) (v 1) (v 2) (v 3) (v 4) (v 5) (v 6) (v 7)) proIns0Def := .mk
  fun v ↦ by
    simp [proIns0Def, proIns0, proSig, qPack, memberList_defined.iff, layoutSteps_defined.iff, shiftsV_defined.iff,
      offVec_defined.iff, identIns0_defined.iff, mkStep_defined.iff, appendV_defined.iff, numeral_eq_natCast]

set_option maxHeartbeats 4000000 in
/-- **The `insert` child of the EMPTY parent is applicable**: afterwards the child `insert p ∅` is laid out at `0`, the
parent's `Layout0` is at `i + 1 + σ`, and `insFact cp P S`, `eqFactB cp s''` hold (`cp = &σ`, `s'' = &(k' + 1)`). -/
theorem proIns0_ok {tbl N N' B' Wl Wc W T p i ip D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWl : Wl = layoutPieces) (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hp : IsSemiformula LAct 0 p) (hsD : setLen LAct (insert p (0 : V)) ≤ D)
    (hE : 13 * D + 18 * ‖D‖ + 12 ≤ E) (hiE : i + 14 * D + 5 ≤ E) (hipE : ip + 8 * D + 4 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout0 walkPieces Wc T Γ i) (hDp : DossF walkPieces Γ 0 p ip) :
    ListOK tbl E ((8 : ℕ) : V) Γ (proIns0 walkPieces Wl Wc W T p i ip) ∧ NoDrop' (proIns0 walkPieces Wl Wc W T p i ip) ∧
    shiftsV (proIns0 walkPieces Wl Wc W T p i ip) = 1 + proSig walkPieces Wl Wc W T (insert p (0 : V)) ∧
    Layout walkPieces Wc T (finalCtx Γ (proIns0 walkPieces Wl Wc W T p i ip)) (insert p (0 : V)) 0 ∧
    Layout0 walkPieces Wc T (finalCtx Γ (proIns0 walkPieces Wl Wc W T p i ip)) (i + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V))) ∧
    neg LAct (insFact (^&(proSig walkPieces Wl Wc W T (insert p (0 : V)))) (^&(ip + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V))))
      (^&(i + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V)) + (len (memberList (0 : V)) + 1)))) ∈
      finalCtx Γ (proIns0 walkPieces Wl Wc W T p i ip) ∧
    neg LAct (eqFactB (^&(proSig walkPieces Wl Wc W T (insert p (0 : V)))) (^&(0 + (len (memberList (insert p (0 : V))) + 1)))) ∈
      finalCtx Γ (proIns0 walkPieces Wl Wc W T p i ip) := by
  have hL := hP.layoutTable
  have hs : IsFormulaSet LAct (0 : V) := fun x hx ↦ by simp at hx
  have hcs : IsFormulaSet LAct (insert p (0 : V)) := IsFormulaSet.insert_iff.mpr ⟨hp, hs⟩
  have hsD' : setLen LAct (0 : V) ≤ D := le_trans (setLen_le_insert p 0) hsD
  ...
  have e76 : ∀ ev : V, mkStep proPieces (76 : V) ev = mkStep layoutPieces (76 : V) ev := fun ev ↦ by
    have := mkStep_pro_layout 76 (by decide) ev; simpa using this
  have hσle : proSig walkPieces Wl Wc W T (insert p (0 : V)) ≤ 6 * D + 1 := proSig_le htbl hP hWc htblN hWl hWp hcs hk1' hsD hE8 hΓ
  have hipE1 : ip + 1 ≤ E := le_trans (add_le_add le_rfl (by norm_num)) (le_trans (add_le_add le_self_add le_rfl) hipE)
  have hSE : i + (len (memberList (0 : V)) + 1) + 1 ≤ E := by
    calc i + (len (memberList (0 : V)) + 1) + 1 = i + (len (memberList (0 : V)) + 2) := by ring
      _ ≤ i + (14 * D + 5) := add_le_add le_rfl (add_le_add (le_trans hkD (le_mul_of_one_le_left zero_le (by norm_num))) (by norm_num))
  ...
  rw [← e76, ← hWp] at ok₁ tg₁ cx₁
  rw [Nat.cast_zero, termShift_fvar, termShift_fvar] at cx₁
  have h1sh : shiftsV (?[mkStep W 76 ?[^&ip, ^&(i + (len (memberList (0 : V)) + 1))]] : V) = 1 := shiftsV_single_tag2 tg₁
  have h1nd : NoDrop' (?[mkStep W 76 ?[^&ip, ^&(i + (len (memberList (0 : V)) + 1))]] : V) := noDrop'_single (Or.inr (Or.inr (Or.inl tg₁)))
  obtain ⟨Γ₁, hΓ₁⟩ : ∃ Γ', Γ' = finalCtx Γ ?[mkStep W 76 ?[^&ip, ^&(i + (len (memberList (0 : V)) + 1))]] := ⟨_, rfl⟩
  have hΓ₁e : Γ₁ = insert (neg LAct (insFact (^&0) (^&(ip + 1)) (^&(i + (len (memberList (0 : V)) + 1) + 1)))) (setShift LAct Γ) := by
    rw [hΓ₁, finalCtx_single, cx₁]
  have hΓ₁f : IsFormulaSet LAct Γ₁ := by rw [hΓ₁]; exact finalCtx_isFormulaSet 8 htbl hΓ (listOK_single ok₁)
  have hLay₁ : Layout0 walkPieces Wc T Γ₁ (i + 1) := by rw [hΓ₁]; have := hLay.transport h1nd; rwa [h1sh] at this
  have hDp₁ : DossF walkPieces Γ₁ 0 p (ip + 1) := by rw [hΓ₁]; have := dossF_transport' h1nd hDp; rwa [h1sh] at this
  have hcp₁ : neg LAct (insFact (^&0) (^&(ip + 1)) (^&(i + (len (memberList (0 : V)) + 1) + 1))) ∈ Γ₁ := by
    rw [hΓ₁e]; exact memInsSelf _ _
  -- step 2: the child's layout
  obtain ⟨lok, lnd, _, _, lLay⟩ := layoutSteps_ok htbl hP htblN hWl hWc hWp hcs hk1' hsD hE8 hΓ₁f
  have lsh : shiftsV (layoutSteps walkPieces Wl Wc W T (insert p (0 : V))) = proSig walkPieces Wl Wc W T (insert p (0 : V)) := rfl
  obtain ⟨Γ₂, hΓ₂⟩ : ∃ Γ', Γ' = finalCtx Γ₁ (layoutSteps walkPieces Wl Wc W T (insert p (0 : V))) := ⟨_, rfl⟩
  have hΓ₂f : IsFormulaSet LAct Γ₂ := by rw [hΓ₂]; exact finalCtx_isFormulaSet 8 htbl hΓ₁f lok
  have lLay₂ : Layout walkPieces Wc T Γ₂ (insert p (0 : V)) 0 := by rw [hΓ₂]; exact lLay
  have hLay₂ : Layout0 walkPieces Wc T Γ₂ (i + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V))) := by
    rw [hΓ₂]; have := hLay₁.transport lnd; rwa [lsh] at this
  have hDp₂ : DossF walkPieces Γ₂ 0 p (ip + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V))) := by
    rw [hΓ₂]; have := dossF_transport' lnd hDp₁; rwa [lsh] at this
  have hcp₂ : neg LAct (insFact (^&(proSig walkPieces Wl Wc W T (insert p (0 : V)))) (^&(ip + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V))))
      (^&(i + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V)) + (len (memberList (0 : V)) + 1)))) ∈ Γ₂ := by
    rw [hΓ₂]
    have := mem_finalCtx_of_mem' lnd hcp₁
    rw [lsh, shiftIterV_neg (isFormula_insFact (by simp) (by simp) (by simp)), shiftIterV_insFact (by simp) (by simp) (by simp),
      termShiftIterV_fvar, termShiftIterV_fvar, termShiftIterV_fvar, zero_add] at this
    have e : i + (len (memberList (0 : V)) + 1) + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V)) =
        i + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V)) + (len (memberList (0 : V)) + 1) := by ring
    rwa [e] at this
  have hF₂ : IdFrame walkPieces Wc T Γ₂ (0 : V) p (i + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V)))
      (ip + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V))) (proSig walkPieces Wl Wc W T (insert p (0 : V))) :=
    ⟨lLay₂, hLay₂.1, hDp₂, hcp₂⟩
  have h0₂ : neg LAct (eqFactB (^&(i + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V)) + (len (memberList (0 : V)) + 1))) (𝟎 : V)) ∈ Γ₂ := by
    rw [len_memberList_zero, zero_add]; exact hLay₂.2
  -- step 3: the identification
  have hcap : IdCap D E (i + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V))) (ip + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V)))
      (proSig walkPieces Wl Wc W T (insert p (0 : V))) := ⟨hE, by
      calc i + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V)) + 8 * D + 3 ≤ i + 1 + (6 * D + 1) + 8 * D + 3 :=
            add_le_add (add_le_add (add_le_add le_rfl hσle) le_rfl) le_rfl
        _ = i + 14 * D + 5 := by ring
        _ ≤ E := hiE, by
      calc ip + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V)) + 2 * D + 2 ≤ ip + 1 + (6 * D + 1) + 2 * D + 2 :=
            add_le_add (add_le_add (add_le_add le_rfl hσle) le_rfl) le_rfl
        _ = ip + 8 * D + 4 := by ring
        _ ≤ E := hipE, by
      calc proSig walkPieces Wl Wc W T (insert p (0 : V)) + 1 ≤ 6 * D + 1 + 1 := add_le_add hσle le_rfl
        _ = 6 * D + 2 := by ring
        _ ≤ 13 * D + 18 * ‖D‖ + 12 := six_le_cap (by norm_num)
        _ ≤ E := hE⟩
  obtain ⟨iok, ind, iho, ish, ifact⟩ := identIns0_ok htbl hP hWl hWc hWp rfl hp hsD hcap hΓ₂f hF₂ h0₂
  -- assembly
  unfold proIns0
  refine ⟨listOK_appendV (listOK_single ok₁) (by rw [← hΓ₁]; exact listOK_appendV lok (by rw [← hΓ₂]; exact iok)),
    noDrop'_appendV h1nd (noDrop'_appendV lnd ind.noDrop'), ?_, ?_, ?_, ?_, ?_⟩
  · rw [shiftsV_appendV, shiftsV_appendV, h1sh, lsh, ish, add_zero]
  · rw [finalCtx_appendV, finalCtx_appendV, ← hΓ₁, ← hΓ₂]
    have := lLay₂.transport ind.noDrop'; rwa [ish, add_zero] at this
  · rw [finalCtx_appendV, finalCtx_appendV, ← hΓ₁, ← hΓ₂]
    have := hLay₂.transport ind.noDrop'; rwa [ish, add_zero] at this
  · rw [finalCtx_appendV, finalCtx_appendV, ← hΓ₁, ← hΓ₂]
    exact (hF₂.zero_transport ind ish).cp
  · rw [finalCtx_appendV, finalCtx_appendV, ← hΓ₁, ← hΓ₂]
    exact ifact

end insertChild0

/-! ## 17. Sizes and costs of the `0`-producers (§12) and of `proIns0` -/

section proSizes0

/-- `layoutSteps0` is size-disciplined at every class (tags `2, 2, 0, 0, 0`). -/
lemma sizeOK_layoutSteps0 {W : V} (hWp : W = proPieces) (Q Dd : V) : SizeOK Q Dd (layoutSteps0 W) := by
  subst hWp
  have e40 : ∀ ev : V, mkStep proPieces (40 : V) ev = mkStep layoutPieces (40 : V) ev := fun ev ↦ by
    have := mkStep_pro_layout 40 (by decide) ev; simpa using this
  have e85 : ∀ ev : V, mkStep proPieces (85 : V) ev = mkStep layoutPieces (85 : V) ev := fun ev ↦ by
  ...
  refine sizeOK_cons (stepSizeOK_of_tag ?_) (sizeOK_cons (stepSizeOK_of_tag ?_) (sizeOK_cons (stepSizeOK_of_tag ?_)
    (sizeOK_cons (stepSizeOK_of_tag ?_) (sizeOK_single (stepSizeOK_of_tag ?_)))))
  · rw [e40, ltag_eqTotal rfl]; simp
  · rw [e85, ltag_setLenTotalC rfl]; simp
  · rw [e42, ltag_eqSymm rfl]; simp
  · rw [e124, ftag_congSetLenR rfl]; simp
  · rw [ptag_setLenEmptyLe rfl]; simp

/-- `proWk0` is size-disciplined at every class. -/
lemma sizeOK_proWk0 {W : V} (hWp : W = proPieces) (s i Q Dd : V) : SizeOK Q Dd (proWk0 W s i) := by
  have e81 : ∀ ev : V, mkStep proPieces (81 : V) ev = mkStep layoutPieces (81 : V) ev := fun ev ↦ by
    have := mkStep_pro_layout 81 (by decide) ev; simpa using this
  unfold proWk0
  refine sizeOK_appendV (sizeOK_layoutSteps0 hWp Q Dd) (sizeOK_cons (stepSizeOK_of_tag ?_) (sizeOK_single (stepSizeOK_of_tag ?_)))
  · rw [hWp, e81, ltag_emptySubsetC rfl]; simp
  · rw [hWp, ptag_congSubsetL rfl]; simp

/-- `proShift0` is size-disciplined at every class. -/
lemma sizeOK_proShift0 {W : V} (hWp : W = proPieces) (i Q Dd : V) : SizeOK Q Dd (proShift0 W i) := by
  have e138 : ∀ ev : V, mkStep proPieces (138 : V) ev = mkStep frag2Pieces (138 : V) ev := fun ev ↦ by
    have := mkStep_pro_frag2 138 (by decide) ev; simpa using this
  have e145 : ∀ ev : V, mkStep proPieces (145 : V) ev = mkStep frag2Pieces (145 : V) ev := fun ev ↦ by
    have := mkStep_pro_frag2 145 (by decide) ev; simpa using this
  ...
  have e148 : ∀ ev : V, mkStep proPieces (148 : V) ev = mkStep frag2Pieces (148 : V) ev := fun ev ↦ by
    have := mkStep_pro_frag2 148 (by decide) ev; simpa using this
  unfold proShift0
  refine sizeOK_appendV (sizeOK_layoutSteps0 hWp Q Dd) (sizeOK_cons (stepSizeOK_of_tag ?_) (sizeOK_cons (stepSizeOK_of_tag ?_)
    (sizeOK_cons (stepSizeOK_of_tag ?_) (sizeOK_cons (stepSizeOK_of_tag ?_) (sizeOK_cons (stepSizeOK_of_tag ?_)
      (sizeOK_single (stepSizeOK_of_tag ?_)))))))
  · rw [hWp, e138, gtag_setShiftTotal rfl]; simp
  · rw [hWp, e145, gtag_setShiftEmpty rfl]; simp
  · rw [hWp, e42, ltag_eqSymm rfl]; simp
  · rw [hWp, e43, ltag_eqTrans rfl]; simp
  · rw [hWp, e148, gtag_congSetShiftL rfl]; simp
  · rw [hWp, ptag_congSetShiftR rfl]; simp

theorem costSum_layoutSteps0_le {tbl N B W Wc T E Γ Q Dd : V} (htbl : TableOK tbl N) (hP : ProTable tbl) (hWp : W = proPieces)
    (hBt : ∀ j < len tbl, formulaLen LAct (rowB tbl.[j]) ≤ B) (hΓ : IsFormulaSet LAct Γ) (hE : (2 : V) ≤ E) :
    costSum N E Γ (layoutSteps0 W) ≤ 5 *
      (costK N E B 8 Q Dd + (3 * ((8 : ℕ) : V) + 11) * (ctxBoundG (growK B E Q) Γ 5 + (fvOccS LAct Γ + 5 * growK B E Q))) := by
  obtain ⟨ok, _, _, hlen, _⟩ := layoutSteps0_ok (Wc := Wc) (T := T) htbl hP hWp hΓ hE
  have hE1 : (1 : V) ≤ E := le_trans (by norm_num) hE
  have := costSum_le_of_sizeOK 8 hE1 htbl hBt ok (sizeOK_layoutSteps0 hWp Q Dd)
  rwa [hlen] at this

theorem costSum_proWk0_le {tbl N B W Wc T s i E Γ Q Dd : V} (htbl : TableOK tbl N) (hP : ProTable tbl) (hWp : W = proPieces)
    (hBt : ∀ j < len tbl, formulaLen LAct (rowB tbl.[j]) ≤ B) (hΓ : IsFormulaSet LAct Γ)
    (hiE : i + 2 + (len (memberList s) + 1) + 1 ≤ E) (hLay : Layout walkPieces Wc T Γ s i) :
    costSum N E Γ (proWk0 W s i) ≤ len (proWk0 W s i) *
      (costK N E B 8 Q Dd + (3 * ((8 : ℕ) : V) + 11) * (ctxBoundG (growK B E Q) Γ (len (proWk0 W s i)) +
        (fvOccS LAct Γ + len (proWk0 W s i) * growK B E Q))) := by
  obtain ⟨ok, _⟩ := proWk0_ok htbl hP hWp hΓ hiE hLay
  have hE1 : (1 : V) ≤ E := le_trans (by norm_num) (le_trans le_add_self hiE)
  exact costSum_le_of_sizeOK 8 hE1 htbl hBt ok (sizeOK_proWk0 hWp s i Q Dd)

theorem costSum_proShift0_le {tbl N B W Wc T i E Γ Q Dd : V} (htbl : TableOK tbl N) (hP : ProTable tbl) (hWp : W = proPieces)
    (hBt : ∀ j < len tbl, formulaLen LAct (rowB tbl.[j]) ≤ B) (hΓ : IsFormulaSet LAct Γ)
    (hiE : i + 3 + 1 + 1 ≤ E) (hLay : Layout0 walkPieces Wc T Γ i) :
    costSum N E Γ (proShift0 W i) ≤ len (proShift0 W i) *
      (costK N E B 8 Q Dd + (3 * ((8 : ℕ) : V) + 11) * (ctxBoundG (growK B E Q) Γ (len (proShift0 W i)) +
        (fvOccS LAct Γ + len (proShift0 W i) * growK B E Q))) := by
  obtain ⟨ok, _⟩ := proShift0_ok htbl hP hWp hΓ hiE hLay
  have hE1 : (1 : V) ≤ E := le_trans (by norm_num) (le_trans le_add_self hiE)
  exact costSum_le_of_sizeOK 8 hE1 htbl hBt ok (sizeOK_proShift0 hWp i Q Dd)

set_option maxHeartbeats 4000000 in
/-- **`proIns0` is size-disciplined** at the layout class (its identification is Horn-only by `identIns0_ok`). -/
theorem sizeOK_proIns0 {tbl N N' B' B Wl Wc W T p i ip D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWl : Wl = layoutPieces) (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hPle : formulaLen LAct (Ple : V) ≤ B)
    (hp : IsSemiformula LAct 0 p) (hsD : setLen LAct (insert p (0 : V)) ≤ D)
    (hE : 13 * D + 18 * ‖D‖ + 12 ≤ E) (hiE : i + 14 * D + 5 ≤ E) (hipE : ip + 8 * D + 4 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout0 walkPieces Wc T Γ i) (hDp : DossF walkPieces Γ 0 p ip) :
    SizeOK (layQ B B' D) (layD N' B' D) (proIns0 walkPieces Wl Wc W T p i ip) := by
  have hL := hP.layoutTable
  have hs : IsFormulaSet LAct (0 : V) := fun x hx ↦ by simp at hx
  have hcs : IsFormulaSet LAct (insert p (0 : V)) := IsFormulaSet.insert_iff.mpr ⟨hp, hs⟩
  have hsD' : setLen LAct (0 : V) ≤ D := le_trans (setLen_le_insert p 0) hsD
  ...
  have hcap : IdCap D E (i + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V))) (ip + 1 + proSig walkPieces Wl Wc W T (insert p (0 : V)))
      ...
      calc proSig walkPieces Wl Wc W T (insert p (0 : V)) + 1 ≤ 6 * D + 1 + 1 := add_le_add hσle le_rfl
        ...
        _ ≤ 13 * D + 18 * ‖D‖ + 12 := six_le_cap (by norm_num)
        _ ≤ E := hE⟩
  obtain ⟨_, _, iho, _, _⟩ := identIns0_ok htbl hP hWl hWc hWp rfl hp hsD hcap hΓ₂f hF₂ h0₂
  unfold proIns0
  exact sizeOK_appendV (sizeOK_single (stepSizeOK_of_tag (Or.inr (Or.inr tg₁))))
    (sizeOK_appendV (sizeOK_layoutSteps hWl hWc hWp htblN hPle hcs hsD) (sizeOK_of_hornOnly iho))

theorem costSum_proIns0_le {tbl N N' B' B Wl Wc W T p i ip D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWl : Wl = layoutPieces) (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hBt : ∀ j < len tbl, formulaLen LAct (rowB tbl.[j]) ≤ B) (hPle : formulaLen LAct (Ple : V) ≤ B)
    (hp : IsSemiformula LAct 0 p) (hsD : setLen LAct (insert p (0 : V)) ≤ D)
    (hE : 13 * D + 18 * ‖D‖ + 12 ≤ E) (hiE : i + 14 * D + 5 ≤ E) (hipE : ip + 8 * D + 4 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout0 walkPieces Wc T Γ i) (hDp : DossF walkPieces Γ 0 p ip) :
    costSum N E Γ (proIns0 walkPieces Wl Wc W T p i ip) ≤ len (proIns0 walkPieces Wl Wc W T p i ip) *
      (costK N E B 8 (layQ B B' D) (layD N' B' D) + (3 * ((8 : ℕ) : V) + 11) *
        (ctxBoundG (growK B E (layQ B B' D)) Γ (len (proIns0 walkPieces Wl Wc W T p i ip)) +
          (fvOccS LAct Γ + len (proIns0 walkPieces Wl Wc W T p i ip) * growK B E (layQ B B' D)))) := by
  obtain ⟨ok, _⟩ := proIns0_ok htbl hP htblN hWl hWc hWp hp hsD hE hiE hipE hΓ hLay hDp
  have hE1 : (1 : V) ≤ E := le_trans (by norm_num) (le_trans le_add_self hE)
  exact costSum_le_of_sizeOK 8 hE1 htbl hBt ok (sizeOK_proIns0 htbl hP htblN hWl hWc hWp hPle hp hsD hE hiE hipE hΓ hLay hDp)

end proSizes0

/-! ## 18. The lengths and shifts of the prologues (`Top.lean` §9: `VerifyKit'.cost` needs `len ≤ Ck·(dlen ρ + 1)`)

Every `costSum_pro<Tag>_le` of §11/§15/§17 has `len (pro…)` as its multiplier; this section bounds those lengths by the
sizes the node charges in its own `dlen` (`NodeSize.lean`): the parent's and the child's `setLen`, the principal
formula, the `exs` witness. All bounds are STRUCTURAL (no context, no cap) except where a loop contains an `eqSteps`,
whose only known length bound is the one `eqSteps_ok` gives — extracted once, by a double walk of the formula from the
empty context (`len_eqSteps_le`, needs only the tables). The `sizeOK`/`costSum`/`_ok` statements above are untouched;
the corollaries `costSum_pro<Tag>_le'` restate them with the concrete length substituted (`costShape_mono`).

THE FINDING (2026-09-15): the prologues of `and`/`or`/`cut`/`wk`/`shift` (and the empty-parent producers) are LINEAR in
the node sizes, but `all` and `exs` are NOT: `Cert`'s substitution certificate walks the quantifier vectors `qVecIterV`,
whose term-length sums are QUADRATIC in the depth (`Σ_{i<e}(i+1)` — unary bvar codes, `Cert` §6.7), so `certSubst` is
`sfK L Q · |r|` with `Q, L` quadratic in the sizes: DEGREE 5 in `D` here (`len_certSubst_single_le : ≤ 192·(D+1)⁵`), and
degree 3 with `Pin`'s `len_certSubst_le_lin` (`sfL L Q = L + 24Q + 22`; `Pin` imports this file, so the linear-in-`Q`
bound cannot be used here — the parametric lemmas `len_allCert_le_of`/`len_proAll_le_of`/`len_exsCert_le_of`/
`len_proExs_le_of` take the substitution pass's length as a hypothesis for exactly that reason). Consequence for
`VerifyKit'.cost` as stated: `len ≤ Ck·(dlen ρ + 1)` is NOT what these producers give at `all`/`exs` nodes; the honest
shape is `Ck·(dlen ρ + 1)³` (with the `Pin` bound) — one degree more per factor of `len` in `boundedInnerNec_four_of_kit`
unless the substitution certificate is restructured (a `qVec` walk shared across the depths).

Contents: 18.1 prefix sums `preSum`; 18.2 `len_relocS`, `len_eqSteps_le`; 18.3 the identification blocks/loops
(A, B, W, E, `p`); 18.4 the exact Horn-only loops (`subChain`, `uSub`, `loopI`, `loopM`, `loopS`); 18.5 the layout
builder (`memberBlock(s)`, `chainSteps`, `lenFold`, `layoutSteps ≤ 26·setLen s + 13k + 6 ≤ 39·setLen s + 6`,
`layoutSteps0 = 5`, `shBase = 4`, `postIns = 6`, `proWk0 = 7`, `proShift0 = 11`, `lenObj = 4`, `proAxL + 4 ≤ 12|p|`,
`proCutPre ≤ 64|p|`); 18.6 `identIns ≤ 16·S' + 5`, `proIns ≤ 55·S' + 12`, `proOr ≤ 110·S'' + 25`, `proWk ≤ 44·Sc + 7`;
18.7 `shTail`, `proShift ≤ 61·Sc + Ss + 20`, `ssIdent ≤ 8·Ss + 4`, `proSS ≤ 40·Sv + 69·Ss + 30`; 18.8 the structural
`certSubst` bound; 18.9 `all` (`allCert ≤ 244·(D+1)⁵`, `proAll ≤ 450·(D+1)⁵`); 18.10 `exs` (`exsCert ≤ 237·(D+1)⁵`,
`proExs ≤ 310·(D+1)⁵`); 18.11 `identIns0 ≤ 10·S' + 6`, `proIns0 ≤ 49·S' + 13`; 18.12 the cost corollaries;
18.13 the shift counts (`shiftsV ≤ len` uniformly, the sharp `6D + 2`/`6D + 1`/`12D + 4`/`7D + 3`/`8D` from the `_ok`s). -/

section proLen

/-! ### 18.1 Prefix sums of formula lengths (`Σ_{j<m} |ys.[j]|`) -/

namespace PreSum

noncomputable def blueprint : PR.Blueprint 1 where
  zero := .mkSigma “y ys. y = 0”
  succ := .mkSigma “y ih m ys. ∃ x, !nthDef x ys m ∧ ∃ l, !(formulaLenGraph LAct) l x ∧ y = ih + l”

noncomputable def construction : PR.Construction V blueprint where
  zero := fun _ ↦ 0
  succ := fun v m ih ↦ ih + formulaLen LAct (v 0).[m]
  zero_defined := .mk fun v ↦ by simp [blueprint]
  succ_defined := .mk fun v ↦ by simp [blueprint, formulaLen.defined.iff]

end PreSum

/-- `preSum ys m = Σ_{j < m} formulaLen ys.[j]`. -/
noncomputable def preSum (ys m : V) : V := PreSum.construction.result ![ys] m

@[simp] lemma preSum_zero (ys : V) : preSum ys 0 = 0 := by simp [preSum, PreSum.construction]
lemma preSum_succ (ys m : V) : preSum ys (m + 1) = preSum ys m + formulaLen LAct ys.[m] := by
  simp [preSum, PreSum.construction]

noncomputable def preSumDef : 𝚺₁.Semisentence 3 := PreSum.blueprint.resultDef |>.rew (Rew.subst ![#0, #2, #1])

instance preSum_defined : 𝚺₁-Function₂ (preSum : V → V → V) via preSumDef := .mk
  fun v ↦ by simp [PreSum.construction.result_defined_iff, preSumDef]; rfl
instance preSum_definable : 𝚺₁-Function₂ (preSum : V → V → V) := preSum_defined.to_definable

lemma preSum_adjoin_succ (x v : V) : ∀ m : V, preSum (x ∷ v) (m + 1) = formulaLen LAct x + preSum v m := by
  intro m
  induction m using ISigma1.pi1_succ_induction with
  | hP => definability
  | zero => rw [preSum_succ, preSum_zero, preSum_zero, nth_adjoin_zero, zero_add, add_zero]
  | succ m ih => rw [preSum_succ, ih, nth_adjoin_succ, preSum_succ, add_assoc]

/-- The whole prefix sum is the list sum of the lengths. -/
lemma preSum_len_eq : ∀ ys : V, preSum ys (len ys) = listSum (flenVec ys) := by
  intro ys
  induction ys using adjoin_ISigma1.pi1_succ_induction with
  | hP => definability
  | nil => simp
  | adjoin x v ih => rw [len_adjoin, preSum_adjoin_succ, ih, flenVec_adjoin, listSum_adjoin]

lemma preSum_mono (ys m : V) : ∀ c : V, preSum ys m ≤ preSum ys (m + c) := by
  intro c
  induction c using ISigma1.pi1_succ_induction with
  | hP => definability
  | zero => rw [add_zero]
  | succ c ih => rw [← add_assoc, preSum_succ]; exact le_trans ih le_self_add

lemma preSum_le_listSum {ys m : V} (hm : m ≤ len ys) : preSum ys m ≤ listSum (flenVec ys) := by
  obtain ⟨c, hc⟩ := exists_add_of_le hm
  rw [← preSum_len_eq, hc]; exact preSum_mono ys m c

lemma preSum_memberList_le (s m : V) (hm : m ≤ len (memberList s)) : preSum (memberList s) m ≤ setLen LAct s := by
  rw [setLen_eq_listSum_memberList]; exact preSum_le_listSum hm

/-! ### 18.2 `eqSteps` has at most `2·eqCount + 1` steps (a structural fact, extracted by a double walk) -/

lemma len_relocS (W i j : V) : ∀ L : V, len (relocS L W i j) = len L := by
  intro L
  induction L using adjoin_ISigma1.pi1_succ_induction with
  | hP => definability
  | nil => simp
  | adjoin x L ih => rw [relocS_adjoin, len_adjoin, len_adjoin, ih]

/-- The identification walk of a closed formula has at most `2·eqCount r + 1` steps, at any offsets. -/
theorem len_eqSteps_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl : V} (hWl : Wl = layoutPieces)
    {r : V} (hr : IsSemiformula LAct 0 r) (i j : V) : len (eqSteps Wl i j r) ≤ 2 * eqCount r + 1 := by
  have hW := hP.walkTable
  have hL := hP.layoutTable
  -- two walks of `r` from the empty context: dossiers at `descCountF` and at `0`
  obtain ⟨ok₁, _, sh₁, hc₁, _⟩ := describeF_ok htbl hW hr (E := 2 * 0 + 2 * formulaLen LAct r + 8) le_rfl (Γ := 0) IsFormulaSet.empty
  have hΓ₁ : IsFormulaSet LAct (finalCtx 0 (describeF walkPieces 0 r)) := finalCtx_isFormulaSet 8 htbl IsFormulaSet.empty ok₁
  obtain ⟨ok₂, _, _, _, _⟩ := describeF_ok htbl hW hr (E := 2 * 0 + 2 * formulaLen LAct r + 8) le_rfl hΓ₁
  have hΓ₂ : IsFormulaSet LAct (finalCtx (finalCtx 0 (describeF walkPieces 0 r)) (describeF walkPieces 0 r)) :=
    finalCtx_isFormulaSet 8 htbl hΓ₁ ok₂
  have hD₂ : DossierAt factPreds (finalCtx (finalCtx 0 (describeF walkPieces 0 r)) (describeF walkPieces 0 r)) 0 r :=
    dossierAt_of_walk htbl hW rfl hr
  have hD₁ : DossierAt factPreds (finalCtx (finalCtx 0 (describeF walkPieces 0 r)) (describeF walkPieces 0 r))
      (descCountF walkPieces 0 r) r := by
    have := dossierAt_of_walk_transport htbl hW rfl hr (Γ := 0) (S := describeF walkPieces 0 r)
      (describeF_noDrop_shifts htbl hW rfl hr).1
    rwa [sh₁] at this
  obtain ⟨_, _, _, _, hlen, _⟩ := eqSteps_ok htbl hL hWl rfl hr
    (E := 2 * (0 + formulaLen LAct r) + 12 + (descCountF walkPieces 0 r + eqCount r + 1)) le_self_add
    le_add_self (by rw [zero_add]; exact le_trans (add_le_add le_add_self le_rfl) le_add_self) hΓ₂ hD₁ hD₂
  unfold eqSteps at hlen ⊢
  rwa [len_relocS] at hlen ⊢

/-! ### 18.3 The blocks and loops of the identifications (Loops A, B, W, E, the `p` block) -/

lemma len_blockA_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {q : V} (hWl : qWl q = layoutPieces)
    {j : V} (hy : IsSemiformula LAct 0 (qYs q).[j]) : len (blockA q j) ≤ 4 * formulaLen LAct (qYs q).[j] + 1 := by
  have hc := eqCount_succ_le htbl hP.walkTable hy
  unfold blockA
  split_ifs with h
  · rw [len_appendV, len_vec2, h]
    have := len_eqSteps_le htbl hP hWl (h ▸ hy) (mTop 0 (qK' q) (qOs' q).[j]) (qIp q)
    rw [← h] at this ⊢
    calc len (eqSteps (qWl q) (mTop 0 (qK' q) (qOs' q).[j]) (qIp q) (qYs q).[j]) + 2 ≤ 2 * eqCount (qYs q).[j] + 1 + 2 := add_le_add this le_rfl
      _ = 2 * (eqCount (qYs q).[j] + 1) + 1 := by ring
      _ ≤ 2 * (2 * formulaLen LAct (qYs q).[j]) + 1 := add_le_add (mul_le_mul_of_nonneg_left hc zero_le) le_rfl
      _ = 4 * formulaLen LAct (qYs q).[j] + 1 := by ring
  · rw [len_appendV, len_vec2]
    have := len_eqSteps_le htbl hP hWl hy (mTop 0 (qK' q) (qOs' q).[j]) (mTop (qI q) (qK q) (qOs q).[idxOf (qXs q) (qYs q).[j]])
    calc _ ≤ 2 * eqCount (qYs q).[j] + 1 + 2 := add_le_add this le_rfl
      _ = 2 * (eqCount (qYs q).[j] + 1) + 1 := by ring
      _ ≤ 2 * (2 * formulaLen LAct (qYs q).[j]) + 1 := add_le_add (mul_le_mul_of_nonneg_left hc zero_le) le_rfl
      _ = 4 * formulaLen LAct (qYs q).[j] + 1 := by ring

lemma len_loopA_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {q : V} (hWl : qWl q = layoutPieces) :
    ∀ m : V, (∀ j < m, IsSemiformula LAct 0 (qYs q).[j]) → len (loopA q m) ≤ 4 * preSum (qYs q) m + m := by
  intro m
  induction m using ISigma1.pi1_succ_induction with
  | hP => definability
  | zero => intro _; simp
  | succ m ih =>
    intro hys
    rw [loopA_succ, len_appendV, preSum_succ]
    have h1 := ih (fun j hj ↦ hys j (lt_trans hj (lt_add_one m)))
    have h2 := len_blockA_le htbl hP hWl (hys m (lt_add_one m))
    calc len (loopA q m) + len (blockA q m) ≤ (4 * preSum (qYs q) m + m) + (4 * formulaLen LAct (qYs q).[m] + 1) := add_le_add h1 h2
      _ = 4 * (preSum (qYs q) m + formulaLen LAct (qYs q).[m]) + (m + 1) := by ring

lemma len_blockB_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {q : V} (hWl : qWl q = layoutPieces)
    {j : V} (hx : IsSemiformula LAct 0 (qXs q).[j]) : len (blockB q j) ≤ 4 * formulaLen LAct (qXs q).[j] + 1 := by
  have hc := eqCount_succ_le htbl hP.walkTable hx
  unfold blockB
  rw [len_appendV, len_vec2]
  have := len_eqSteps_le htbl hP hWl hx (mTop 0 (qK' q) (qOs' q).[idxOf (qYs q) (qXs q).[j]]) (mTop (qI q) (qK q) (qOs q).[j])
  calc _ ≤ 2 * eqCount (qXs q).[j] + 1 + 2 := add_le_add this le_rfl
    _ = 2 * (eqCount (qXs q).[j] + 1) + 1 := by ring
    _ ≤ 2 * (2 * formulaLen LAct (qXs q).[j]) + 1 := add_le_add (mul_le_mul_of_nonneg_left hc zero_le) le_rfl
    _ = 4 * formulaLen LAct (qXs q).[j] + 1 := by ring

lemma len_loopB_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {q : V} (hWl : qWl q = layoutPieces) :
    ∀ m : V, (∀ j < m, IsSemiformula LAct 0 (qXs q).[j]) → len (loopB q m) ≤ 4 * preSum (qXs q) m + m := by
  intro m
  induction m using ISigma1.pi1_succ_induction with
  | hP => definability
  | zero => intro _; simp
  | succ m ih =>
    intro hxs
    rw [loopB_succ, len_appendV, preSum_succ]
    have h1 := ih (fun j hj ↦ hxs j (lt_trans hj (lt_add_one m)))
    have h2 := len_blockB_le htbl hP hWl (hxs m (lt_add_one m))
    calc len (loopB q m) + len (blockB q m) ≤ (4 * preSum (qXs q) m + m) + (4 * formulaLen LAct (qXs q).[m] + 1) := add_le_add h1 h2
      _ = 4 * (preSum (qXs q) m + formulaLen LAct (qXs q).[m]) + (m + 1) := by ring

lemma len_blockW_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {q : V} (hWl : qWl q = layoutPieces)
    {j : V} (hy : IsSemiformula LAct 0 (qYs q).[j]) : len (blockW q j) ≤ 4 * formulaLen LAct (qYs q).[j] := by
  have hc := eqCount_succ_le htbl hP.walkTable hy
  unfold blockW
  rw [len_appendV, len_vec1]
  have := len_eqSteps_le htbl hP hWl hy (mTop 0 (qK' q) (qOs' q).[j]) (mTop (qI q) (qK q) (qOs q).[idxOf (qXs q) (qYs q).[j]])
  calc _ ≤ 2 * eqCount (qYs q).[j] + 1 + 1 := add_le_add this le_rfl
    _ = 2 * (eqCount (qYs q).[j] + 1) := by ring
    _ ≤ 2 * (2 * formulaLen LAct (qYs q).[j]) := mul_le_mul_of_nonneg_left hc zero_le
    _ = 4 * formulaLen LAct (qYs q).[j] := by ring

lemma len_loopW_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {q : V} (hWl : qWl q = layoutPieces) :
    ∀ m : V, (∀ j < m, IsSemiformula LAct 0 (qYs q).[j]) → len (loopW q m) ≤ 4 * preSum (qYs q) m := by
  intro m
  induction m using ISigma1.pi1_succ_induction with
  | hP => definability
  | zero => intro _; simp
  | succ m ih =>
    intro hys
    rw [loopW_succ, len_appendV, preSum_succ]
    have h1 := ih (fun j hj ↦ hys j (lt_trans hj (lt_add_one m)))
    have h2 := len_blockW_le htbl hP hWl (hys m (lt_add_one m))
    calc len (loopW q m) + len (blockW q m) ≤ 4 * preSum (qYs q) m + 4 * formulaLen LAct (qYs q).[m] := add_le_add h1 h2
      _ = 4 * (preSum (qYs q) m + formulaLen LAct (qYs q).[m]) := by ring

lemma len_blockE_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {q : V} (hWl : qWl q = layoutPieces)
    {j : V} (hx : IsSemiformula LAct 0 (qXs q).[j]) : len (blockE q j) ≤ 4 * formulaLen LAct (qXs q).[j] + 2 := by
  have hc := eqCount_succ_le htbl hP.walkTable hx
  unfold blockE
  rw [len_appendV, len_vec3]
  have := len_eqSteps_le htbl hP hWl hx (mTop 0 (qK q) (qOs q).[j]) (mTop (qI q) (qK q) (qOs q).[j])
  calc _ ≤ 2 * eqCount (qXs q).[j] + 1 + 3 := add_le_add this le_rfl
    _ = 2 * (eqCount (qXs q).[j] + 1) + 2 := by ring
    _ ≤ 2 * (2 * formulaLen LAct (qXs q).[j]) + 2 := add_le_add (mul_le_mul_of_nonneg_left hc zero_le) le_rfl
    _ = 4 * formulaLen LAct (qXs q).[j] + 2 := by ring

lemma len_loopE_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {q : V} (hWl : qWl q = layoutPieces) :
    ∀ m : V, (∀ j < m, IsSemiformula LAct 0 (qXs q).[j]) → len (loopE q m) ≤ 4 * preSum (qXs q) m + 2 * m := by
  intro m
  induction m using ISigma1.pi1_succ_induction with
  | hP => definability
  | zero => intro _; simp
  | succ m ih =>
    intro hxs
    rw [loopE_succ, len_appendV, preSum_succ]
    have h1 := ih (fun j hj ↦ hxs j (lt_trans hj (lt_add_one m)))
    have h2 := len_blockE_le htbl hP hWl (hxs m (lt_add_one m))
    calc len (loopE q m) + len (blockE q m) ≤ (4 * preSum (qXs q) m + 2 * m) + (4 * formulaLen LAct (qXs q).[m] + 2) := add_le_add h1 h2
      _ = 4 * (preSum (qXs q) m + formulaLen LAct (qXs q).[m]) + 2 * (m + 1) := by ring

lemma len_blockP_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {q : V} (hWl : qWl q = layoutPieces)
    (hp : IsSemiformula LAct 0 (qP q)) : len (blockP q) ≤ 4 * formulaLen LAct (qP q) + 1 := by
  have hc := eqCount_succ_le htbl hP.walkTable hp
  unfold blockP
  rw [len_appendV, len_vec2]
  have := len_eqSteps_le htbl hP hWl hp (mTop 0 (qK' q) (qOs' q).[idxOf (qYs q) (qP q)]) (qIp q)
  calc _ ≤ 2 * eqCount (qP q) + 1 + 2 := add_le_add this le_rfl
    _ = 2 * (eqCount (qP q) + 1) + 1 := by ring
    _ ≤ 2 * (2 * formulaLen LAct (qP q)) + 1 := add_le_add (mul_le_mul_of_nonneg_left hc zero_le) le_rfl
    _ = 4 * formulaLen LAct (qP q) + 1 := by ring

/-! ### 18.4 The Horn-only chains and loops with exact lengths -/

lemma len_subChainAux (W i' k' os A : V) : ∀ c : V, len (subChainAux ⟪W, i', k', os, A⟫ c) = c + 1 := by
  intro c
  induction c using ISigma1.pi1_succ_induction with
  | hP => definability
  | zero => rw [subChainAux_zero, len_vec1, zero_add]
  | succ c ih => rw [subChainAux_succ, len_appendV, ih, len_vec1]

lemma len_subChain (W i' k' os A : V) : len (subChain W i' k' os A) = k' + 1 := len_subChainAux W i' k' os A k'

lemma len_uSubAux (q : V) : ∀ c : V, len (uSubAux q c) = c + 1 := by
  intro c
  induction c using ISigma1.pi1_succ_induction with
  | hP => definability
  | zero => rw [uSubAux_zero, len_vec1, zero_add]
  | succ c ih => rw [uSubAux_succ, len_appendV, ih, len_vec1]

lemma len_uSub (q : V) : len (uSub q) = qK' q + 1 := len_uSubAux q (qK' q)

lemma len_loopI (q : V) : ∀ c : V, len (loopI q c) = c := by
  intro c
  induction c using ISigma1.pi1_succ_induction with
  | hP => definability
  | zero => simp
  | succ c ih => rw [loopI_succ, len_appendV, ih]; unfold blockI; rw [len_vec1]

lemma len_loopM (q : V) : ∀ m : V, len (loopM q m) = m := by
  intro m
  induction m using ISigma1.pi1_succ_induction with
  | hP => definability
  | zero => simp
  | succ m ih => rw [loopM_succ, len_appendV, ih]; unfold blockM; rw [len_vec1]

/-- Loop S: one re-indexed `certShift` per child member, `≤ 12|y_j|` each. -/
lemma len_loopS_le (q : V) : ∀ m : V, (∀ j < m, IsSemiformula LAct 0 (qYs q).[j]) → len (loopS q m) ≤ 12 * preSum (qYs q) m := by
  intro m
  induction m using ISigma1.pi1_succ_induction with
  | hP => definability
  | zero => intro _; simp
  | succ m ih =>
    intro hys
    rw [loopS_succ, len_appendV, preSum_succ]
    have h1 := ih (fun j hj ↦ hys j (lt_trans hj (lt_add_one m)))
    have h2 : len (blockS q m) ≤ 12 * formulaLen LAct (qYs q).[m] := by
      unfold blockS; rw [len_reidxL]; exact le_trans le_self_add (len_certShift_le (hys m (lt_add_one m)))
    calc len (loopS q m) + len (blockS q m) ≤ 12 * preSum (qYs q) m + 12 * formulaLen LAct (qYs q).[m] := add_le_add h1 h2
      _ = 12 * (preSum (qYs q) m + formulaLen LAct (qYs q).[m]) := by ring

/-! ### 18.5 The layout builder: member blocks, the chain, the fold -/

lemma len_memberBlock_le {Wc : V} (hWc : Wc = certPieces) (Ww T : V) {x : V} (hx : IsSemiformula LAct 0 x) :
    len (memberBlock Ww Wc T x) ≤ 26 * formulaLen LAct x := by
  unfold memberBlock
  rw [len_appendV, len_reidxL]
  have h1 := len_describeF_le Ww hx
  have h2 := (lenSteps_struct hWc T hx 0).2.2
  calc len (describeF Ww 0 x) + len (lenSteps Wc T 0 x 0) ≤ 12 * formulaLen LAct x + 14 * formulaLen LAct x :=
        add_le_add (le_trans le_self_add h1) h2
    _ = 26 * formulaLen LAct x := by ring

lemma len_memberBlocks_le {Wc : V} (hWc : Wc = certPieces) (Ww T : V) :
    ∀ v : V, (∀ j < len v, IsSemiformula LAct 0 v.[j]) → len (memberBlocks v Ww Wc T) ≤ 26 * listSum (flenVec v) := by
  intro v
  induction v using adjoin_ISigma1.pi1_succ_induction with
  | hP => definability
  | nil => intro _; simp
  | adjoin x v ih =>
    ...
    have hv' : ∀ j < len v, IsSemiformula LAct 0 v.[j] := fun j hj ↦ by
      have := hv (j + 1) (by rw [len_adjoin]; exact (add_lt_add_iff_right 1).mpr hj)
      rwa [nth_adjoin_succ] at this
    rw [memberBlocks_adjoin, len_appendV, flenVec_adjoin, listSum_adjoin]
    calc len (memberBlock Ww Wc T x) + len (memberBlocks v Ww Wc T)
        ≤ 26 * formulaLen LAct x + 26 * listSum (flenVec v) := add_le_add (len_memberBlock_le hWc Ww T hx) (ih hv')
      _ = 26 * (formulaLen LAct x + listSum (flenVec v)) := by ring

lemma len_chainA (W xs : V) : ∀ c : V, len (chainA W xs c) = 3 * c := by
  intro c
  induction c using ISigma1.pi1_succ_induction with
  | hP => definability
  | zero => simp
  | succ c ih => rw [chainA_succ, len_appendV, ih]; unfold chainNodeA; rw [len_vec3]; ring

lemma len_chainB_le (W xs k : V) : ∀ c : V, len (chainB W xs k c) ≤ 4 * c := by
  intro c
  induction c using ISigma1.pi1_succ_induction with
  | hP => definability
  | zero => simp
  | succ c ih =>
    rw [chainB_succ, len_appendV]
    have h2 : len (chainNodeB W xs k c) ≤ 4 := by
      unfold chainNodeB
      split_ifs
      · rw [len_adjoin, len_adjoin, len_adjoin, len_adjoin, len_nil]; norm_num
      · rw [len_nil]; exact zero_le
    calc len (chainB W xs k c) + len (chainNodeB W xs k c) ≤ 4 * c + 4 := add_le_add ih h2
      _ = 4 * (c + 1) := by ring

lemma len_chainSteps_le (W xs : V) : len (chainSteps W xs) ≤ 7 * len xs + 6 := by
  unfold chainSteps
  rw [len_appendV, len_appendV, len_appendV, len_appendV, len_chainA, len_vec1]
  have h1 : len (chainPre W) = 3 := by unfold chainPre; rw [len_vec3]
  have h2 : len (chainBpre W xs (len xs)) = 2 := by unfold chainBpre; rw [len_vec2]
  rw [h1, h2]
  have h3 := len_chainB_le W xs (len xs) (len xs)
  calc 3 + (3 * len xs + (2 + (len (chainB W xs (len xs) (len xs)) + 1))) ≤ 3 + (3 * len xs + (2 + (4 * len xs + 1))) :=
        add_le_add le_rfl (add_le_add le_rfl (add_le_add le_rfl (add_le_add h3 le_rfl)))
    _ = 7 * len xs + 6 := by ring

lemma len_foldBlock_le (W T xs os k c : V) : len (foldBlock W T xs os k c) ≤ 6 := by
  unfold foldBlock
  split_ifs
  · unfold foldBlock0; rw [len_vec2]; norm_num
  · unfold foldBlockS; rw [len_adjoin, len_adjoin, len_adjoin, len_adjoin, len_adjoin, len_adjoin, len_nil]; norm_num

lemma len_lenFoldAux_le (W T xs os k : V) : ∀ c : V, len (lenFoldAux W T xs os k c) ≤ 6 * c := by
  have key : ∀ c : V, len (lenFoldAuxP ⟪W, T, xs, os, k⟫ c) ≤ 6 * c := by
    intro c
    induction c using ISigma1.pi1_succ_induction with
    | hP => definability
    | zero =>
      have e := lenFoldAux_zero W T xs os k
      unfold lenFoldAux at e
      rw [e]; simp
    | succ c ih =>
      have e := lenFoldAux_succ W T xs os k c
      unfold lenFoldAux at e
      rw [e, len_appendV]
      calc len (lenFoldAuxP ⟪W, T, xs, os, k⟫ c) + len (foldBlock W T xs os k c) ≤ 6 * c + 6 :=
            add_le_add ih (len_foldBlock_le _ _ _ _ _ _)
        _ = 6 * (c + 1) := by ring
  intro c
  exact key c

/-- **The layout builder has `≤ 26·setLen s + 13·k + 6` steps** (structurally, any walk/layout pieces). -/
theorem len_layoutSteps_le {Wc : V} (hWc : Wc = certPieces) (Ww Wl W T : V) {s : V} (hs : IsFormulaSet LAct s) :
    len (layoutSteps Ww Wl Wc W T s) ≤ 26 * setLen LAct s + 13 * len (memberList s) + 6 := by
  unfold layoutSteps
  rw [len_appendV, len_appendV]
  have h1 : len (memberBlocks (memberList s) Ww Wc T) ≤ 26 * setLen LAct s := by
    rw [setLen_eq_listSum_memberList]
    exact len_memberBlocks_le hWc Ww T _ (fun j hj ↦ hs _ (nth_memberList_mem hj))
  have h2 : len (chainSteps Wl (fvarVec (offVec (memberList s) Ww Wc T))) ≤ 7 * len (memberList s) + 6 := by
    have := len_chainSteps_le Wl (fvarVec (offVec (memberList s) Ww Wc T))
    rwa [len_fvarVec, len_offVec] at this
  have h3 : len (lenFold W T (memberList s) (offVec (memberList s) Ww Wc T)) ≤ 6 * len (memberList s) :=
    len_lenFoldAux_le _ _ _ _ _ _
  calc _ ≤ 26 * setLen LAct s + ((7 * len (memberList s) + 6) + 6 * len (memberList s)) := add_le_add h1 (add_le_add h2 h3)
    _ = 26 * setLen LAct s + 13 * len (memberList s) + 6 := by ring

/-- The same with the member count absorbed: `≤ 39·setLen s + 6`. -/
theorem len_layoutSteps_le' {Wc : V} (hWc : Wc = certPieces) (Ww Wl W T : V) {s : V} (hs : IsFormulaSet LAct s) :
    len (layoutSteps Ww Wl Wc W T s) ≤ 39 * setLen LAct s + 6 := by
  refine le_trans (len_layoutSteps_le hWc Ww Wl W T hs) ?_
  have hk := len_memberList_le_setLen hs
  calc 26 * setLen LAct s + 13 * len (memberList s) + 6 ≤ 26 * setLen LAct s + 13 * setLen LAct s + 6 :=
        add_le_add (add_le_add le_rfl (mul_le_mul_of_nonneg_left hk zero_le)) le_rfl
    _ = 39 * setLen LAct s + 6 := by ring

lemma len_layoutSteps0 (W : V) : len (layoutSteps0 W) = 5 := by
  unfold layoutSteps0; rw [len_adjoin, len_adjoin, len_adjoin, len_adjoin, len_adjoin, len_nil]; norm_num

lemma len_shBase (W : V) : len (shBase W) = 4 := by
  unfold shBase; rw [len_adjoin, len_adjoin, len_adjoin, len_adjoin, len_nil]; norm_num

lemma len_postIns (W s'' cp n : V) : len (postIns W s'' cp n) = 6 := by
  unfold postIns; rw [len_appendV, len_goalElim, len_vec1]; norm_num

lemma len_proWk0 (W s i : V) : len (proWk0 W s i) = 7 := by
  unfold proWk0; rw [len_appendV, len_layoutSteps0, len_vec2]; norm_num

lemma len_proShift0 (W i : V) : len (proShift0 W i) = 11 := by
  unfold proShift0
  rw [len_appendV, len_layoutSteps0, len_adjoin, len_adjoin, len_adjoin, len_adjoin, len_adjoin, len_adjoin, len_nil]; norm_num

lemma len_lenObj (Wc W lt it₀ : V) : len (lenObj Wc W lt it₀) = 4 := by
  unfold lenObj; rw [len_appendV, len_appendV, len_appendV, len_reidxL, len_vec1, len_vec1, len_vec1, len_vec1]; norm_num

/-- **`proAxL` has `≤ 12|p| − 4` steps** (the re-indexed `certNeg`). -/
theorem len_proAxL_le {Wc : V} (hWc : Wc = certPieces) (Ww T s i : V) {p : V} (hp : IsSemiformula LAct 0 p) :
    len (proAxL Ww Wc T s p i) + 4 ≤ 12 * formulaLen LAct p := by
  unfold proAxL; rw [len_reidxL]; exact len_certNeg_le hp

/-- **`proCutPre` has `≤ 64|p|` steps** (two member blocks and the re-indexed `certNeg`; `|neg p| = |p|`). -/
theorem len_proCutPre_le {Wc : V} (hWc : Wc = certPieces) (Ww T : V) {p : V} (hp : IsSemiformula LAct 0 p) :
    len (proCutPre Ww Wc T p) ≤ 64 * formulaLen LAct p := by
  unfold proCutPre
  rw [len_appendV, len_appendV, len_reidxL]
  have h1 := len_memberBlock_le hWc Ww T hp
  have h2 := len_memberBlock_le hWc Ww T hp.neg
  have h3 := len_certNeg_le (W := Wc) (i := mLen Wc T p + mShift Ww Wc T (neg LAct p)) (j := mLen Wc T (neg LAct p)) hp
  rw [formulaLen_neg hp.isUFormula] at h2
  calc _ ≤ 26 * formulaLen LAct p + (26 * formulaLen LAct p + 12 * formulaLen LAct p) :=
        add_le_add h1 (add_le_add h2 (le_trans le_self_add h3))
    _ = 64 * formulaLen LAct p := by ring


/-! ### 18.6 The per-tag prologues: `and`/`cut` (`proIns`), `or`, `wk` -/

/-- **The identification has `≤ 16·setLen (insert p s) + 5` steps** (Loops A and B, the two `subChain`s, the `p` block). -/
theorem len_identIns_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl : V} (hWl : Wl = layoutPieces)
    (W i₁ os ip₁ os' σ : V) {s p : V} (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 0 p) :
    len (identIns (qPack Wl W i₁ (len (memberList s)) os (memberList s) ip₁ (len (memberList (insert p s))) os'
      (memberList (insert p s)) p σ)) ≤ 16 * setLen LAct (insert p s) + 5 := by
  have hcs : IsFormulaSet LAct (insert p s) := IsFormulaSet.insert_iff.mpr ⟨hp, hs⟩
  have hS : setLen LAct s ≤ setLen LAct (insert p s) := setLen_le_insert p s
  have hk : len (memberList s) ≤ setLen LAct (insert p s) := le_trans (len_memberList_le_setLen hs) hS
  have hk' : len (memberList (insert p s)) ≤ setLen LAct (insert p s) := len_memberList_le_setLen hcs
  have hpS : formulaLen LAct p ≤ setLen LAct (insert p s) := formulaLen_le_setLen_of_mem (by simp)
  have hA := len_loopA_le htbl hP (q := qPack Wl W i₁ (len (memberList s)) os (memberList s) ip₁ (len (memberList (insert p s))) os'
    (memberList (insert p s)) p σ) (by rw [qWl_pack]; exact hWl) (len (memberList (insert p s)))
    (fun j hj ↦ by rw [qYs_pack]; exact hcs _ (nth_memberList_mem hj))
  rw [qYs_pack] at hA
  have hB := len_loopB_le htbl hP (q := qPack Wl W i₁ (len (memberList s)) os (memberList s) ip₁ (len (memberList (insert p s))) os'
    (memberList (insert p s)) p σ) (by rw [qWl_pack]; exact hWl) (len (memberList s))
    (fun j hj ↦ by rw [qXs_pack]; exact hs _ (nth_memberList_mem hj))
  rw [qXs_pack] at hB
  have hPb := len_blockP_le htbl hP (q := qPack Wl W i₁ (len (memberList s)) os (memberList s) ip₁ (len (memberList (insert p s))) os'
    (memberList (insert p s)) p σ) (by rw [qWl_pack]; exact hWl) (by rw [qP_pack]; exact hp)
  rw [qP_pack] at hPb
  have hpre := preSum_memberList_le (insert p s) _ le_rfl
  have hpre' := preSum_memberList_le s _ le_rfl
  unfold identIns
  simp only [qW_pack, qK'_pack, qOs'_pack, qSig_pack, qI_pack, qK_pack, qOs_pack, qIp_pack]
  rw [len_appendV, len_appendV, len_appendV, len_appendV, len_appendV, len_vec2, len_subChain, len_subChain]
  calc _ ≤ (4 * setLen LAct (insert p s) + setLen LAct (insert p s)) + ((setLen LAct (insert p s) + 1) +
      ((4 * setLen LAct (insert p s) + setLen LAct (insert p s)) + ((4 * setLen LAct (insert p s) + 1) +
        ((setLen LAct (insert p s) + 1) + 2)))) := by
        refine add_le_add (le_trans hA (add_le_add (mul_le_mul_of_nonneg_left hpre zero_le) hk'))
          (add_le_add (add_le_add hk' le_rfl) (add_le_add
            (le_trans hB (add_le_add (mul_le_mul_of_nonneg_left (le_trans hpre' hS) zero_le) hk))
            (add_le_add (le_trans hPb (add_le_add (mul_le_mul_of_nonneg_left hpS zero_le) le_rfl))
              (add_le_add (add_le_add hk le_rfl) le_rfl))))
    _ = 16 * setLen LAct (insert p s) + 5 := by ring

/-- **`proIns` has `≤ 55·setLen (insert p s) + 12` steps.** -/
theorem len_proIns_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T i ip : V) {s p : V} (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 0 p) :
    len (proIns Ww Wl Wc W T s p i ip) ≤ 55 * setLen LAct (insert p s) + 12 := by
  have hcs : IsFormulaSet LAct (insert p s) := IsFormulaSet.insert_iff.mpr ⟨hp, hs⟩
  unfold proIns
  rw [len_appendV, len_appendV, len_vec1]
  have h1 := len_layoutSteps_le' hWc Ww Wl W T hcs
  have h2 := len_identIns_le htbl hP hWl W (i + 1 + proSig Ww Wl Wc W T (insert p s)) (offVec (memberList s) Ww Wc T)
    (ip + 1 + proSig Ww Wl Wc W T (insert p s)) (offVec (memberList (insert p s)) Ww Wc T) (proSig Ww Wl Wc W T (insert p s)) hs hp
  calc _ ≤ 1 + ((39 * setLen LAct (insert p s) + 6) + (16 * setLen LAct (insert p s) + 5)) := add_le_add le_rfl (add_le_add h1 h2)
    _ = 55 * setLen LAct (insert p s) + 12 := by ring

/-- **`proOr` has `≤ 110·setLen (insert p (insert q s)) + 25` steps.** -/
theorem len_proOr_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T i ip iq : V) {s p q : V} (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 0 p)
    (hq : IsSemiformula LAct 0 q) :
    len (proOr Ww Wl Wc W T s p q i ip iq) ≤ 110 * setLen LAct (insert p (insert q s)) + 25 := by
  have hqs : IsFormulaSet LAct (insert q s) := IsFormulaSet.insert_iff.mpr ⟨hq, hs⟩
  unfold proOr
  rw [len_appendV, len_appendV, len_vec1]
  have h1 := len_proIns_le htbl hP hWl hWc Ww W T i iq hs hq
  have h2 := len_proIns_le htbl hP hWl hWc Ww W T 0 (ip + 1 + proSig Ww Wl Wc W T (insert q s)) hqs hp
  have hS : setLen LAct (insert q s) ≤ setLen LAct (insert p (insert q s)) := setLen_le_insert p _
  calc _ ≤ (55 * setLen LAct (insert p (insert q s)) + 12) + ((55 * setLen LAct (insert p (insert q s)) + 12) + 1) :=
        add_le_add (le_trans h1 (add_le_add (mul_le_mul_of_nonneg_left hS zero_le) le_rfl)) (add_le_add h2 le_rfl)
    _ = 110 * setLen LAct (insert p (insert q s)) + 25 := by ring

/-- **`proWk` has `≤ 44·setLen c + 7` steps** (the child's layout, Loop W, the child's `subChain`). -/
theorem len_proWk_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T s i : V) {c : V} (hc : IsFormulaSet LAct c) :
    len (proWk Ww Wl Wc W T s c i) ≤ 44 * setLen LAct c + 7 := by
  unfold proWk
  rw [len_appendV, len_appendV, len_subChain]
  have h1 := len_layoutSteps_le' hWc Ww Wl W T hc
  have h2 := len_loopW_le htbl hP (q := qPack Wl W (i + proSig Ww Wl Wc W T c) (len (memberList s)) (offVec (memberList s) Ww Wc T)
    (memberList s) 0 (len (memberList c)) (offVec (memberList c) Ww Wc T) (memberList c) 0 0) (by rw [qWl_pack]; exact hWl)
    (len (memberList c)) (fun j hj ↦ by rw [qYs_pack]; exact hc _ (nth_memberList_mem hj))
  rw [qYs_pack] at h2
  have hpre := preSum_memberList_le c _ le_rfl
  have hk' := len_memberList_le_setLen hc
  calc _ ≤ (39 * setLen LAct c + 6) + (4 * setLen LAct c + (setLen LAct c + 1)) :=
        add_le_add h1 (add_le_add (le_trans h2 (mul_le_mul_of_nonneg_left hpre zero_le)) (add_le_add hk' le_rfl))
    _ = 44 * setLen LAct c + 7 := by ring

/-! ### 18.7 `shift`: the tail and the whole prologue; `proSS` -/

/-- The Horn-only tail of `proShift`: Loop S (`≤ 12·setLen c`), Loops I/M and Fold U (`k'` each, `+1`), the parent's
`subChain` (`k + 1`), two steps. -/
theorem len_shTail_le {Wc : V} (hWc : Wc = certPieces) (Ww W T i₂ σ : V) {s c : V} (hs : IsFormulaSet LAct s)
    (hc : IsFormulaSet LAct c) :
    len (shTail Ww Wc W T s c i₂ σ) ≤ 15 * setLen LAct c + setLen LAct s + 4 := by
  have hk := len_memberList_le_setLen hs
  have hk' := len_memberList_le_setLen hc
  have hS := len_loopS_le (shPack Wc W i₂ (len (memberList s)) (offVec (memberList s) Ww Wc T) (memberList s) (len (memberList c))
    (offVec (memberList c) Ww Wc T) (memberList c) σ) (len (memberList c))
    (fun j hj ↦ by rw [qYs_shPack]; exact hc _ (nth_memberList_mem hj))
  rw [qYs_shPack] at hS
  have hpre := preSum_memberList_le c _ le_rfl
  unfold shTail
  rw [len_appendV, len_appendV, len_appendV, len_appendV, len_appendV, len_vec2, len_loopI, len_uSub, len_loopM, len_subChain]
  simp only [shPack, qK'_pack]
  calc _ ≤ 12 * setLen LAct c + (setLen LAct c + ((setLen LAct c + 1) + (setLen LAct c + ((setLen LAct s + 1) + 2)))) :=
        add_le_add (le_trans hS (mul_le_mul_of_nonneg_left hpre zero_le))
          (add_le_add hk' (add_le_add (add_le_add hk' le_rfl) (add_le_add hk' (add_le_add (add_le_add hk le_rfl) le_rfl))))
    _ = 15 * setLen LAct c + setLen LAct s + 4 := by ring

/-- **`proShift` has `≤ 61·setLen c + setLen s + 20` steps** (`shBase`, the parent's chain, the child's layout, the tail). -/
theorem len_proShift_le {Wc : V} (hWc : Wc = certPieces) (Ww Wl W T i : V) {s c : V} (hs : IsFormulaSet LAct s)
    (hc : IsFormulaSet LAct c) :
    len (proShift Ww Wl Wc W T s c i) ≤ 61 * setLen LAct c + setLen LAct s + 20 := by
  have hk' := len_memberList_le_setLen hc
  unfold proShift
  rw [len_appendV, len_appendV, len_appendV, len_shBase]
  have h1 : len (chainSteps Wl (fvarVec (shPos (memberList c) (memberList s) (offVec (memberList s) Ww Wc T) (len (memberList s)) (i + 1)))) ≤
      7 * setLen LAct c + 6 := by
    have := len_chainSteps_le Wl (fvarVec (shPos (memberList c) (memberList s) (offVec (memberList s) Ww Wc T) (len (memberList s)) (i + 1)))
    rw [len_fvarVec, len_shPos] at this
    exact le_trans this (add_le_add (mul_le_mul_of_nonneg_left hk' zero_le) le_rfl)
  have h2 := len_layoutSteps_le' hWc Ww Wl W T hc
  have h3 := len_shTail_le hWc Ww W T (i + 1 + (len (memberList c) + 1) + proSig Ww Wl Wc W T c) (proSig Ww Wl Wc W T c) hs hc
  calc _ ≤ 4 + ((7 * setLen LAct c + 6) + ((39 * setLen LAct c + 6) + (15 * setLen LAct c + setLen LAct s + 4))) :=
        add_le_add le_rfl (add_le_add h1 (add_le_add h2 h3))
    _ = 61 * setLen LAct c + setLen LAct s + 20 := by ring

/-- The identification of the fresh chain top with the parent's: `≤ 8·setLen s + 4`. -/
theorem len_ssIdent_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl : V} (hWl : Wl = layoutPieces)
    (W i₁ os vt : V) {s : V} (hs : IsFormulaSet LAct s) :
    len (ssIdent Wl W i₁ (len (memberList s)) os (memberList s) vt) ≤ 8 * setLen LAct s + 4 := by
  have hk := len_memberList_le_setLen hs
  have hE := len_loopE_le htbl hP (q := qPack Wl W i₁ (len (memberList s)) os (memberList s) 0 (len (memberList s)) os (memberList s) 0 0)
    (by rw [qWl_pack]; exact hWl) (len (memberList s)) (fun j hj ↦ by rw [qXs_pack]; exact hs _ (nth_memberList_mem hj))
  rw [qXs_pack] at hE
  have hpre := preSum_memberList_le s _ le_rfl
  unfold ssIdent
  rw [len_appendV, len_appendV, len_appendV, len_vec2, len_subChain, len_subChain]
  calc _ ≤ (4 * setLen LAct s + 2 * setLen LAct s) + ((setLen LAct s + 1) + ((setLen LAct s + 1) + 2)) :=
        add_le_add (le_trans hE (add_le_add (mul_le_mul_of_nonneg_left hpre zero_le) (mul_le_mul_of_nonneg_left hk zero_le)))
          (add_le_add (add_le_add hk le_rfl) (add_le_add (add_le_add hk le_rfl) le_rfl))
    _ = 8 * setLen LAct s + 4 := by ring

/-- **`proSS` has `≤ 40·setLen (setShift s) + 69·setLen s + 30` steps.** -/
theorem len_proSS_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T i : V) {s : V} (hs : IsFormulaSet LAct s) :
    len (proSS Ww Wl Wc W T s i) ≤ 40 * setLen LAct (setShift LAct s) + 69 * setLen LAct s + 30 := by
  have hv : IsFormulaSet LAct (setShift LAct s) := hs.setShift
  unfold proSS
  rw [len_appendV, len_appendV]
  have h1 := len_layoutSteps_le' hWc Ww Wl W T hv
  have h2 := len_proShift_le hWc Ww Wl W T 0 hv hs
  have h3 := len_ssIdent_le htbl hP hWl W (i + proSig Ww Wl Wc W T (setShift LAct s) + (1 + (len (memberList s) + 1) + proSig Ww Wl Wc W T s))
    (offVec (memberList s) Ww Wc T) (0 + 1 + (len (memberList s) + 1) + proSig Ww Wl Wc W T s + (len (memberList (setShift LAct s)) + 1)) hs
  calc _ ≤ (39 * setLen LAct (setShift LAct s) + 6) + ((61 * setLen LAct s + setLen LAct (setShift LAct s) + 20) + (8 * setLen LAct s + 4)) :=
        add_le_add h1 (add_le_add h2 h3)
    _ = 40 * setLen LAct (setShift LAct s) + 69 * setLen LAct s + 30 := by ring

/-! ### 18.8 The certified substitution, structurally: `len (certSubst …) ≤ sfK L Q · |r|` -/

/-- The length bound of `Cert`'s substitution pass with the caps of `Cert` §6.7 substituted (`w` `SubstInv` at `B`):
`Q = (n + |r|)(n + |r| + B)`, `L = 12·(n + |r| + 1)(n + |r| + 1 + B) + 2`. No table, no context. -/
theorem len_certSubst_le_sfK {Wd W n m w iw r i j B : V} (hw : IsSemitermVec LAct n m w) (hinv : SubstInv LAct B w)
    (hr : IsSemiformula LAct n r) :
    len (certSubst W Wd n m w iw r i j) ≤
      sfK (12 * ((n + formulaLen LAct r + 1) * (n + formulaLen LAct r + 1 + B)) + 2)
        ((n + formulaLen LAct r) * (n + formulaLen LAct r + B)) * formulaLen LAct r :=
  len_subFGraph_le W Wd hw (formulaLen LAct r) _ _ (listSum_termLenVec_qVecIterV_cap hw hinv) (len_qWalkP_cap (Wd := Wd) hw hinv)
    hr 0 (by rw [zero_add]) (by rw [add_zero]) i j iw _ (by rw [add_zero, qVecIterV_zero]; exact certSubst_graph hw hr)

lemma sfK_mono {L L' Q Q' : V} (hL : L ≤ L') (hQ : Q ≤ Q') : sfK L Q ≤ sfK L' Q' := by
  unfold sfK
  exact add_le_add (add_le_add (add_le_add hL (mul_le_mul (mul_le_mul_of_nonneg_left hQ zero_le) (add_le_add hQ le_rfl) zero_le zero_le))
    (mul_le_mul_of_nonneg_left (add_le_add hQ le_rfl) zero_le)) le_rfl

lemma one_le_sq (D : V) : (1 : V) ≤ (D + 1) * (D + 1) :=
  le_trans (by norm_num : (1 : V) ≤ 1 * 1) (mul_le_mul le_add_self le_add_self zero_le zero_le)

/-- `sfK (74X) (2X) ≤ 192·X²` for `X ≥ 1`. -/
lemma sfK_crude (X : V) (hX : 1 ≤ X) : sfK (74 * X) (2 * X) ≤ 192 * (X * X) := by
  unfold sfK
  have hXX : X ≤ X * X := le_mul_of_one_le_left zero_le hX
  calc 74 * X + 12 * (2 * X) * (2 * X + 1) + 12 * (2 * X + 1) + 10 = 48 * (X * X) + 122 * X + 22 * 1 := by ring
    _ ≤ 48 * (X * X) + 122 * (X * X) + 22 * (X * X) :=
        add_le_add (add_le_add le_rfl (mul_le_mul_of_nonneg_left hXX zero_le)) (mul_le_mul_of_nonneg_left (le_trans hX hXX) zero_le)
    _ = 192 * (X * X) := by ring

/-- The singleton-vector instance at a size bound `D` (`|r| ≤ D`, `B ≤ D`, `n = 1`): `len ≤ 192·(D+1)⁵`. -/
theorem len_certSubst_single_le {Wd W m w iw r i j B D : V} (hw : IsSemitermVec LAct 1 m w) (hinv : SubstInv LAct B w)
    (hr : IsSemiformula LAct 1 r) (hrD : formulaLen LAct r ≤ D) (hB : B ≤ D) :
    len (certSubst W Wd 1 m w iw r i j) ≤ 192 * (D + 1) ^ 5 := by
  have h := len_certSubst_le_sfK (Wd := Wd) (W := W) (iw := iw) (i := i) (j := j) hw hinv hr
  have hX := one_le_sq D
  have hr1 : formulaLen LAct r + 1 ≤ D + 1 := add_le_add hrD le_rfl
  have hQ : (1 + formulaLen LAct r) * (1 + formulaLen LAct r + B) ≤ 2 * ((D + 1) * (D + 1)) := by
    calc (1 + formulaLen LAct r) * (1 + formulaLen LAct r + B) ≤ (D + 1) * ((D + 1) + (D + 1)) :=
          mul_le_mul (by rw [add_comm]; exact hr1) (add_le_add (by rw [add_comm]; exact hr1) (le_trans hB le_self_add)) zero_le zero_le
      _ = 2 * ((D + 1) * (D + 1)) := by ring
  have hL : 12 * ((1 + formulaLen LAct r + 1) * (1 + formulaLen LAct r + 1 + B)) + 2 ≤ 74 * ((D + 1) * (D + 1)) := by
    have h2 : 1 + formulaLen LAct r + 1 ≤ 2 * (D + 1) := by
      calc 1 + formulaLen LAct r + 1 ≤ (D + 1) + (D + 1) := add_le_add (by rw [add_comm]; exact hr1) le_add_self
        _ = 2 * (D + 1) := by ring
    have h3 : 1 + formulaLen LAct r + 1 + B ≤ 3 * (D + 1) := by
      calc 1 + formulaLen LAct r + 1 + B ≤ 2 * (D + 1) + (D + 1) := add_le_add h2 (le_trans hB le_self_add)
        _ = 3 * (D + 1) := by ring
    calc 12 * ((1 + formulaLen LAct r + 1) * (1 + formulaLen LAct r + 1 + B)) + 2 ≤ 12 * (2 * (D + 1) * (3 * (D + 1))) + 2 * ((D + 1) * (D + 1)) :=
          add_le_add (mul_le_mul_of_nonneg_left (mul_le_mul h2 h3 zero_le zero_le) zero_le) (le_mul_of_one_le_right zero_le hX)
      _ = 74 * ((D + 1) * (D + 1)) := by ring
  calc len (certSubst W Wd 1 m w iw r i j) ≤ sfK (74 * ((D + 1) * (D + 1))) (2 * ((D + 1) * (D + 1))) * (D + 1) :=
        le_trans h (mul_le_mul (sfK_mono hL hQ) (le_trans hrD le_self_add) zero_le zero_le)
    _ ≤ 192 * (((D + 1) * (D + 1)) * ((D + 1) * (D + 1))) * (D + 1) :=
        mul_le_mul_of_nonneg_right (sfK_crude _ hX) zero_le
    _ = 192 * (D + 1) ^ 5 := by ring

/-! ### 18.9 `all`: `certFree`, `allCert`, `proAll` -/

lemma len_freeWalk_le (Wd : V) : len (π₂ (freeWalk Wd)) ≤ 14 := by
  have hw : IsSemitermVec LAct 1 0 (fvec : V) := isSemitermVec_fvec
  obtain ⟨_, hl⟩ := len_descVecAux_le hw (fun i hi ↦ len_describeT_le Wd 0 _ (hw.nth hi)) 1 le_rfl
  have htl : takeLast (fvec : V) 1 = fvec := by have := takeLast_len_self (fvec : V); rwa [hw.lh] at this
  rw [htl, listSum_termLenVec_fvec] at hl
  have h2 : len (π₂ (freeWalk Wd)) + 2 ≤ 14 + 2 := by
    unfold freeWalk; exact le_trans hl (le_of_eq (by ring))
  exact le_of_add_le_add_right h2

/-- `certFree` parametrically in the substitution pass's length: `≤ 14 + Lc + 12|p| + 2`. -/
theorem len_certFree_le_of {Wc Wd p ip isp ifp Lc : V} (hp : IsSemiformula LAct 1 p)
    (hcs : ∀ a b : V, len (certSubst Wc Wd 1 0 fvec 0 (shift LAct p) a b) ≤ Lc) :
    len (certFree Wc Wd p ip isp ifp) ≤ Lc + 12 * formulaLen LAct p + 16 := by
  unfold certFree
  rw [len_appendV, len_appendV, len_appendV, len_vec2]
  have h1 := len_freeWalk_le Wd
  have h2 := hcs (isp + freeCw Wd) (ifp + freeCw Wd)
  have h3 := len_certShift_le (W := Wc) (i := ip + freeCw Wd + freeS Wc Wd p isp ifp) (j := isp + freeCw Wd + freeS Wc Wd p isp ifp) hp
  calc _ ≤ 14 + (Lc + (12 * formulaLen LAct p + 2)) := add_le_add h1 (add_le_add h2 (add_le_add (le_trans le_self_add h3) le_rfl))
    _ = Lc + 12 * formulaLen LAct p + 16 := by ring

/-- `allCert` parametrically: the two walks, then `certFree`. -/
theorem len_allCert_le_of {Wc : V} (Ww T s i : V) {p Lc : V} (hp : IsSemiformula LAct 1 p)
    (hcs : ∀ a b : V, len (certSubst Wc Ww 1 0 fvec 0 (shift LAct p) a b) ≤ Lc) :
    len (allCert Ww Wc T s p i) ≤ 12 * formulaLen LAct (shift LAct p) + 12 * formulaLen LAct (free LAct p) +
      12 * formulaLen LAct p + Lc + 16 := by
  unfold allCert
  rw [len_appendV, len_appendV, len_reidxL]
  have h1 := len_describeF_le Ww hp.shift
  have h2 := len_describeF_le Ww hp.free
  have h3 := len_certFree_le_of hp hcs (ip := memTop Ww Wc T s (^∀ p) (i + descCountF Ww 1 (shift LAct p) + descCountF Ww 0 (free LAct p)) + 1)
    (isp := descCountF Ww 0 (free LAct p)) (ifp := 0)
  calc _ ≤ 12 * formulaLen LAct (shift LAct p) + (12 * formulaLen LAct (free LAct p) + (Lc + 12 * formulaLen LAct p + 16)) :=
        add_le_add (le_trans le_self_add h1) (add_le_add (le_trans le_self_add h2) h3)
    _ = _ := by ring

/-- **`allCert` at a size bound `D`** (`|shift p|, |free p|, |p| ≤ D`): `≤ 244·(D+1)⁵` — the substitution certificate's
quantifier vectors are QUADRATIC in `|shift p|` (`Cert` §6.7), and the pass is linear in their walks. -/
theorem len_allCert_le {Wc : V} (hWc : Wc = certPieces) (Ww T s i : V) {p D : V} (hp : IsSemiformula LAct 1 p)
    (hspD : formulaLen LAct (shift LAct p) ≤ D) (hfpD : formulaLen LAct (free LAct p) ≤ D) (hpD : formulaLen LAct p ≤ D) :
    len (allCert Ww Wc T s p i) ≤ 244 * (D + 1) ^ 5 := by
  have hX := one_le_sq D
  have hD1 : (1 : V) ≤ D + 1 := le_add_self
  have hpow : D + 1 ≤ (D + 1) ^ 5 := by
    calc D + 1 = (D + 1) * 1 := by ring
      _ ≤ (D + 1) * (((D + 1) * (D + 1)) * ((D + 1) * (D + 1))) :=
          mul_le_mul_of_nonneg_left (le_trans (by norm_num : (1 : V) ≤ 1 * 1) (mul_le_mul hX hX zero_le zero_le)) zero_le
      _ = (D + 1) ^ 5 := by ring
  have hcs : ∀ a b : V, len (certSubst Wc Ww 1 0 fvec 0 (shift LAct p) a b) ≤ 192 * (D + 1) ^ 5 := fun a b ↦
    len_certSubst_single_le (isSemitermVec_fvec : IsSemitermVec LAct 1 0 (fvec : V)) (substInv_fvec : SubstInv LAct 1 (fvec : V)) hp.shift hspD (le_trans (one_le_formulaLen_V hp.shift) hspD)
  have h := len_allCert_le_of Ww T s i hp hcs
  calc _ ≤ 12 * D + 12 * D + 12 * D + 192 * (D + 1) ^ 5 + 16 :=
    ...
    _ = 36 * D + 16 + 192 * (D + 1) ^ 5 := by ring
    _ ≤ 36 * (D + 1) ^ 5 + 16 * (D + 1) ^ 5 + 192 * (D + 1) ^ 5 :=
...
/-- **`proAll` parametrically**: `≤ 200·D + 58 + Lc` with `Lc` bounding the substitution pass. -/
theorem len_proAll_le_of {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T i : V) {s p D Lc : V} (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 1 p)
    (hsD : setLen LAct s ≤ D) (hcD : setLen LAct (insert (free LAct p) (setShift LAct s)) ≤ D)
    (hspD : formulaLen LAct (shift LAct p) ≤ D) (hpD : formulaLen LAct p ≤ D)
    (hcs : ∀ a b : V, len (certSubst Wc Ww 1 0 fvec 0 (shift LAct p) a b) ≤ Lc) :
    len (proAll Ww Wl Wc W T s p i) ≤ 200 * D + 58 + Lc := by
  have hv : IsFormulaSet LAct (setShift LAct s) := hs.setShift
  have hvD : setLen LAct (setShift LAct s) ≤ D := le_trans (setLen_le_insert _ _) hcD
  have hfpD : formulaLen LAct (free LAct p) ≤ D := le_trans (formulaLen_le_setLen_of_mem (by simp)) hcD
  unfold proAll
  rw [len_appendV, len_appendV]
  have h1 := len_allCert_le_of (Wc := Wc) Ww T s i hp hcs
  have h2 := len_proSS_le htbl hP hWl hWc Ww W T (i + allCertSig Ww Wc p) hs
  have h3 := len_proIns_le htbl hP hWl hWc Ww W T (0 + 1 + (len (memberList s) + 1) + proSig Ww Wl Wc W T s)
    (allCf Ww Wc p + (proSig Ww Wl Wc W T (setShift LAct s) + (1 + (len (memberList s) + 1) + proSig Ww Wl Wc W T s))) hv hp.free
  calc _ ≤ (12 * D + 12 * D + 12 * D + Lc + 16) + ((40 * D + 69 * D + 30) + (55 * D + 12)) := by
        refine add_le_add (le_trans h1 ?_) (add_le_add (le_trans h2 ?_) (le_trans h3 ?_))
        · exact add_le_add (add_le_add (add_le_add (add_le_add (mul_le_mul_of_nonneg_left hspD zero_le)
            (mul_le_mul_of_nonneg_left hfpD zero_le)) (mul_le_mul_of_nonneg_left hpD zero_le)) le_rfl) le_rfl
        · exact add_le_add (add_le_add (mul_le_mul_of_nonneg_left hvD zero_le) (mul_le_mul_of_nonneg_left hsD zero_le)) le_rfl
        · exact add_le_add (mul_le_mul_of_nonneg_left hcD zero_le) le_rfl
    _ = 200 * D + 58 + Lc := by ring

/-- **`proAll` has `≤ 450·(D+1)⁵` steps** at the size bound `D` of `proAll_ok` (`|p| ≤ D` from `∀p ∈ s`). -/
theorem len_proAll_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T i : V) {s p D : V} (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 1 p) (hr : (^∀ p) ∈ s)
    (hsD : setLen LAct s ≤ D) (hcD : setLen LAct (insert (free LAct p) (setShift LAct s)) ≤ D)
    (hspD : formulaLen LAct (shift LAct p) ≤ D) :
    len (proAll Ww Wl Wc W T s p i) ≤ 450 * (D + 1) ^ 5 := by
  have hX := one_le_sq D
  have hD1 : (1 : V) ≤ D + 1 := le_add_self
  have hpow : D + 1 ≤ (D + 1) ^ 5 := by
    calc D + 1 = (D + 1) * 1 := by ring
  ...
  have hcs : ∀ a b : V, len (certSubst Wc Ww 1 0 fvec 0 (shift LAct p) a b) ≤ 192 * (D + 1) ^ 5 := fun a b ↦
    len_certSubst_single_le (isSemitermVec_fvec : IsSemitermVec LAct 1 0 (fvec : V)) (substInv_fvec : SubstInv LAct 1 (fvec : V)) hp.shift hspD
      (le_trans (one_le_formulaLen_V hp.shift) hspD)
  have h := len_proAll_le_of htbl hP hWl hWc Ww W T i hs hp hsD hcD hspD hpD hcs
  calc _ ≤ 200 * D + 58 + 192 * (D + 1) ^ 5 := h
    _ ≤ 200 * (D + 1) ^ 5 + 58 * (D + 1) ^ 5 + 192 * (D + 1) ^ 5 :=
        add_le_add (add_le_add (mul_le_mul_of_nonneg_left (le_trans le_self_add hpow) zero_le)
          (le_mul_of_one_le_right zero_le (le_trans hD1 hpow))) le_rfl
    _ = 450 * (D + 1) ^ 5 := by ring

/-! ### 18.10 `exs`: `exsCert`, `proExs` -/

lemma len_vecWalk_le (Ww : V) {t : V} (ht : IsSemiterm LAct 0 t) : len (vecWalk Ww t) ≤ 12 * termLen LAct t + 2 := by
  have hw : IsSemitermVec LAct 1 0 (t ∷ (0 : V)) := by simp [ht]
  obtain ⟨_, hl⟩ := len_descVecAux_le hw (fun i hi ↦ len_describeT_le Ww 0 _ (hw.nth hi)) 1 le_rfl
  have htl : takeLast (t ∷ (0 : V)) 1 = t ∷ 0 := by have := takeLast_len_self (t ∷ (0 : V)); rwa [hw.lh] at this
  rw [htl, listSum_termLenVec_single ht] at hl
  unfold vecWalk
  exact le_of_add_le_add_right (le_trans hl (le_of_eq (by ring)))

/-- `exsCert` parametrically in the substitution pass: `≤ 26|t| + 12|substs1 t p| + Lc + 7`. -/
theorem len_exsCert_le_of {Wc : V} (Ww W T s i : V) {p t Lc : V} (hp : IsSemiformula LAct 1 p) (ht : IsSemiterm LAct 0 t)
    (hcs : ∀ a b : V, len (certSubst Wc Ww 1 0 (t ∷ 0) a p b 0) ≤ Lc) :
    len (exsCert Ww Wc W T s p t i) ≤ 26 * termLen LAct t + 12 * formulaLen LAct (substs1 LAct t p) + Lc + 7 := by
  unfold exsCert
  rw [len_appendV, len_appendV, len_appendV, len_appendV, len_appendV, len_reidxL, len_reidxL, len_lenObj, len_vec1]
  have h1 := len_vecWalk_le Ww ht
  have h2 : len (lenT Wc T 0 t 1) ≤ 14 * termLen LAct t :=
    le_trans le_self_add (lenTGraph_len Wc T 0 t ht 1 _ (lenT_graph ht))
  have h3 := len_describeF_le Ww (hp.substs1 ht)
  have h4 := hcs (exsIw Ww Wc T t p) (exsIp Ww Wc T s p t i)
  calc _ ≤ (12 * termLen LAct t + 2) + (14 * termLen LAct t + (4 + (12 * formulaLen LAct (substs1 LAct t p) + (Lc + 1)))) :=
        add_le_add h1 (add_le_add h2 (add_le_add le_rfl (add_le_add (le_trans le_self_add h3) (add_le_add h4 le_rfl))))
    _ = 26 * termLen LAct t + 12 * formulaLen LAct (substs1 LAct t p) + Lc + 7 := by ring

/-- **`exsCert` at a size bound `D`** (`|p|, |t|, |substs1 t p| ≤ D`): `≤ 237·(D+1)⁵`. -/
theorem len_exsCert_le {Wc : V} (Ww W T s i : V) {p t D : V} (hp : IsSemiformula LAct 1 p) (ht : IsSemiterm LAct 0 t)
    (hpD : formulaLen LAct p ≤ D) (htD : termLen LAct t ≤ D) (hptD : formulaLen LAct (substs1 LAct t p) ≤ D) :
    len (exsCert Ww Wc W T s p t i) ≤ 237 * (D + 1) ^ 5 := by
  have hX := one_le_sq D
  have hD1 : (1 : V) ≤ D + 1 := le_add_self
  have hpow : D + 1 ≤ (D + 1) ^ 5 := by
    calc D + 1 = (D + 1) * 1 := by ring
      ...
      _ = (D + 1) ^ 5 := by ring
  have hw : IsSemitermVec LAct 1 0 (t ∷ (0 : V)) := by simp [ht]
  have hcs : ∀ a b : V, len (certSubst Wc Ww 1 0 (t ∷ 0) a p b 0) ≤ 192 * (D + 1) ^ 5 := fun a b ↦
    len_certSubst_single_le hw (substInv_single' ht) hp hpD htD
  have h := len_exsCert_le_of (Wc := Wc) Ww W T s i hp ht hcs
  calc _ ≤ 26 * D + 12 * D + 192 * (D + 1) ^ 5 + 7 :=
    ...
    _ = 38 * D + 192 * (D + 1) ^ 5 + 7 := by ring
    _ ≤ 38 * (D + 1) ^ 5 + 192 * (D + 1) ^ 5 + 7 * (D + 1) ^ 5 :=
        add_le_add (add_le_add (mul_le_mul_of_nonneg_left (le_trans le_self_add hpow) zero_le) le_rfl)
          (le_mul_of_one_le_right zero_le (le_trans hD1 hpow))
    _ = 237 * (D + 1) ^ 5 := by ring

/-- **`proExs` parametrically**: `≤ 93·D + 19 + Lc`. -/
theorem len_proExs_le_of {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T i : V) {s p t D Lc : V} (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 1 p)
    (ht : IsSemiterm LAct 0 t) (hcD : setLen LAct (insert (substs1 LAct t p) s) ≤ D) (htD : termLen LAct t ≤ D)
    (hcs : ∀ a b : V, len (certSubst Wc Ww 1 0 (t ∷ 0) a p b 0) ≤ Lc) :
    len (proExs Ww Wl Wc W T s p t i) ≤ 93 * D + 19 + Lc := by
  have hptD : formulaLen LAct (substs1 LAct t p) ≤ D := le_trans (formulaLen_le_setLen_of_mem (by simp)) hcD
  unfold proExs
  rw [len_appendV]
  have h1 := len_exsCert_le_of (Wc := Wc) Ww W T s i hp ht hcs
  have h2 := len_proIns_le htbl hP hWl hWc Ww W T (i + exsSig Ww Wc T s p t i) (exsSc Ww Wc T s p t i) hs (hp.substs1 ht)
  calc _ ≤ (26 * D + 12 * D + Lc + 7) + (55 * D + 12) := by
        refine add_le_add (le_trans h1 ?_) (le_trans h2 (add_le_add (mul_le_mul_of_nonneg_left hcD zero_le) le_rfl))
        exact add_le_add (add_le_add (add_le_add (mul_le_mul_of_nonneg_left htD zero_le) (mul_le_mul_of_nonneg_left hptD zero_le)) le_rfl) le_rfl
    _ = 93 * D + 19 + Lc := by ring

/-- **`proExs` has `≤ 310·(D+1)⁵` steps** at the size bound `D` of `proExs_ok` (`|p| ≤ D` from `∃p ∈ s`). -/
theorem len_proExs_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    ...
    len (proExs Ww Wl Wc W T s p t i) ≤ 310 * (D + 1) ^ 5 := by
  have hX := one_le_sq D
  have hD1 : (1 : V) ≤ D + 1 := le_add_self
  have hpow : D + 1 ≤ (D + 1) ^ 5 := by
    calc D + 1 = (D + 1) * 1 := by ring
  ...
  calc _ ≤ 93 * D + 19 + 192 * (D + 1) ^ 5 := h
    _ ≤ 93 * (D + 1) ^ 5 + 19 * (D + 1) ^ 5 + 192 * (D + 1) ^ 5 :=
        add_le_add (add_le_add (mul_le_mul_of_nonneg_left (le_trans le_self_add hpow) zero_le)
          (le_mul_of_one_le_right zero_le (le_trans hD1 hpow))) le_rfl
    _ = 304 * (D + 1) ^ 5 := by ring
    _ ≤ 310 * (D + 1) ^ 5 := mul_le_mul_of_nonneg_right (by norm_num) zero_le

/-! ### 18.11 The empty parent: `identIns0`, `proIns0` -/

theorem len_identIns0_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl : V} (hWl : Wl = layoutPieces)
    (W i₁ os ip₁ os' σ : V) {p : V} (hp : IsSemiformula LAct 0 p) :
    len (identIns0 (qPack Wl W i₁ (len (memberList (0 : V))) os (memberList (0 : V)) ip₁ (len (memberList (insert p (0 : V)))) os'
      (memberList (insert p (0 : V))) p σ)) ≤ 10 * setLen LAct (insert p (0 : V)) + 6 := by
  have hcs : IsFormulaSet LAct (insert p (0 : V)) := IsFormulaSet.insert_iff.mpr ⟨hp, fun x hx ↦ by simp at hx⟩
  have hk' : len (memberList (insert p (0 : V))) ≤ setLen LAct (insert p (0 : V)) := len_memberList_le_setLen hcs
  have hpS : formulaLen LAct p ≤ setLen LAct (insert p (0 : V)) := formulaLen_le_setLen_of_mem (by simp)
  have hA := len_loopA_le htbl hP (q := qPack Wl W i₁ (len (memberList (0 : V))) os (memberList (0 : V)) ip₁
    (len (memberList (insert p (0 : V)))) os' (memberList (insert p (0 : V))) p σ) (by rw [qWl_pack]; exact hWl)
    (len (memberList (insert p (0 : V)))) (fun j hj ↦ by rw [qYs_pack]; exact hcs _ (nth_memberList_mem hj))
  rw [qYs_pack] at hA
  have hPb := len_blockP_le htbl hP (q := qPack Wl W i₁ (len (memberList (0 : V))) os (memberList (0 : V)) ip₁
    (len (memberList (insert p (0 : V)))) os' (memberList (insert p (0 : V))) p σ) (by rw [qWl_pack]; exact hWl) (by rw [qP_pack]; exact hp)
  rw [qP_pack] at hPb
  have hpre := preSum_memberList_le (insert p (0 : V)) _ le_rfl
  unfold identIns0
  simp only [qW_pack, qK'_pack, qOs'_pack, qSig_pack, qI_pack, qK_pack, qIp_pack]
  rw [len_appendV, len_appendV, len_appendV, len_appendV, len_vec2, len_vec2, len_subChain]
  calc _ ≤ (4 * setLen LAct (insert p (0 : V)) + setLen LAct (insert p (0 : V))) + ((setLen LAct (insert p (0 : V)) + 1) +
      (2 + ((4 * setLen LAct (insert p (0 : V)) + 1) + 2))) := by
        refine add_le_add (le_trans hA (add_le_add (mul_le_mul_of_nonneg_left hpre zero_le) hk'))
          (add_le_add (add_le_add hk' le_rfl) (add_le_add le_rfl (add_le_add
            (le_trans hPb (add_le_add (mul_le_mul_of_nonneg_left hpS zero_le) le_rfl)) le_rfl)))
    _ = 10 * setLen LAct (insert p (0 : V)) + 6 := by ring

/-- **`proIns0` has `≤ 49·setLen (insert p ∅) + 13` steps.** -/
theorem len_proIns0_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T i ip : V) {p : V} (hp : IsSemiformula LAct 0 p) :
    len (proIns0 Ww Wl Wc W T p i ip) ≤ 49 * setLen LAct (insert p (0 : V)) + 13 := by
  have hcs : IsFormulaSet LAct (insert p (0 : V)) := IsFormulaSet.insert_iff.mpr ⟨hp, fun x hx ↦ by simp at hx⟩
  unfold proIns0
  rw [len_appendV, len_appendV, len_vec1]
  have h1 := len_layoutSteps_le' hWc Ww Wl W T hcs
  have h2 := len_identIns0_le htbl hP hWl W (i + 1 + proSig Ww Wl Wc W T (insert p (0 : V))) (offVec (memberList (0 : V)) Ww Wc T)
    (ip + 1 + proSig Ww Wl Wc W T (insert p (0 : V))) (offVec (memberList (insert p (0 : V))) Ww Wc T)
    (proSig Ww Wl Wc W T (insert p (0 : V))) hp
  calc _ ≤ 1 + ((39 * setLen LAct (insert p (0 : V)) + 6) + (10 * setLen LAct (insert p (0 : V)) + 6)) :=
        add_le_add le_rfl (add_le_add h1 h2)
    _ = 49 * setLen LAct (insert p (0 : V)) + 13 := by ring


/-! ### 18.12 The coarse cost shape is monotone in the length; the cost corollaries with concrete lengths -/

/-- `n · (K + c · (ctxBoundG G Γ n + (F + n · g)))` is monotone in `n`. -/
lemma costShape_mono {a b K c G Γ F g : V} (h : a ≤ b) :
    a * (K + c * (ctxBoundG G Γ a + (F + a * g))) ≤ b * (K + c * (ctxBoundG G Γ b + (F + b * g))) :=
  mul_le_mul h (add_le_add le_rfl (mul_le_mul_of_nonneg_left (add_le_add (ctxBoundG_mono_len h)
    (add_le_add le_rfl (mul_le_mul_of_nonneg_right h zero_le))) zero_le)) zero_le zero_le

lemma lin_le {a b c k : V} (h : a ≤ b) : k * a + c ≤ k * b + c := add_le_add (mul_le_mul_of_nonneg_left h zero_le) le_rfl
...
theorem costSum_layoutSteps_le' {tbl N N' B' B Wl Wc W T s D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    ...
    (hs : IsFormulaSet LAct s) (hk1 : 1 ≤ len (memberList s)) (hsD : setLen LAct s ≤ D)
    (hE : 13 * D + 18 * ‖D‖ + 8 ≤ E) (hΓ : IsFormulaSet LAct Γ) :
    costSum N E Γ (layoutSteps walkPieces Wl Wc W T s) ≤ (39 * D + 6) *
      (costK N E B 8 (layQ B B' D) (layD N' B' D) + (3 * ((8 : ℕ) : V) + 11) *
        (ctxBoundG (growK B E (layQ B B' D)) Γ (39 * D + 6) + (fvOccS LAct Γ + (39 * D + 6) * growK B E (layQ B B' D)))) :=
  le_trans (costSum_layoutSteps_le htbl hP htblN hWl hWc hWp hBt hPle hs hk1 hsD hE hΓ).1
    (costShape_mono (le_trans (len_layoutSteps_le' hWc walkPieces Wl W T hs) (lin_le hsD)))
...
theorem costSum_proAxL_le' {tbl N N' B' B Wc T s p D E Γ i Q Dd : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    ...
    (hE : 13 * D + 18 * ‖D‖ + 8 ≤ E) (hiE : i + 8 * D + 3 ≤ E) (hΓ : IsFormulaSet LAct Γ)
    (hLay : Layout walkPieces Wc T Γ s i) :
    costSum N E Γ (proAxL walkPieces Wc T s p i) ≤ (12 * D) *
      (costK N E B 8 Q Dd + (3 * ((8 : ℕ) : V) + 11) * (ctxBoundG (growK B E Q) Γ (12 * D) +
        (fvOccS LAct Γ + (12 * D) * growK B E Q))) := by
  have hpD : formulaLen LAct p ≤ D := le_trans (formulaLen_le_setLen_of_mem (L := LAct) hp) hsD
  have hl : len (proAxL walkPieces Wc T s p i) ≤ 12 * D :=
    le_trans le_self_add (le_trans (len_proAxL_le hWc walkPieces T s i (hs p hp)) (mul_le_mul_of_nonneg_left hpD zero_le))
  exact le_trans (costSum_proAxL_le htbl hP htblN hWc hBt hs hp hnp hsD hE hiE hΓ hLay).1 (costShape_mono hl)
...
theorem costSum_proIns_le' {tbl N N' B' B Wl Wc W T s p i ip D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    ...
    (hipE : ip + 8 * D + 4 ≤ E) (hΓ : IsFormulaSet LAct Γ)
    (hLay : Layout walkPieces Wc T Γ s i) (hDp : DossF walkPieces Γ 0 p ip) :
    costSum N E Γ (proIns walkPieces Wl Wc W T s p i ip) ≤ (55 * D + 12) *
      (costK N E B 8 (layQ B B' D) (layD N' B' D) + (3 * ((8 : ℕ) : V) + 11) *
        (ctxBoundG (growK B E (layQ B B' D)) Γ (55 * D + 12) + (fvOccS LAct Γ + (55 * D + 12) * growK B E (layQ B B' D)))) :=
  le_trans (costSum_proIns_le htbl hP htblN hWl hWc hWp hBt hPle hs hp hk1 hsD hE hiE hipE hΓ hLay hDp)
    (costShape_mono (le_trans (len_proIns_le htbl hP hWl hWc walkPieces W T i ip hs hp) (lin_le hsD)))

theorem costSum_proCutPre_le' {tbl N N' B' B Wc T p D E Γ Q' D' : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWc : Wc = certPieces) (hBt : ∀ j < len tbl, formulaLen LAct (rowB tbl.[j]) ≤ B)
    (hp : IsSemiformula LAct 0 p) (hpD : formulaLen LAct p ≤ D) (hnpD : formulaLen LAct (neg LAct p) ≤ D)
    (hE : 13 * D + 8 ≤ E) (hΓ : IsFormulaSet LAct Γ) :
    costSum N E Γ (proCutPre walkPieces Wc T p) ≤ (64 * D) *
      (costK N E B 8 (lenQ B' D + Q') (lenD N' B' D + D') + (3 * ((8 : ℕ) : V) + 11) *
        (ctxBoundG (growK B E (lenQ B' D + Q')) Γ (64 * D) + (fvOccS LAct Γ + (64 * D) * growK B E (lenQ B' D + Q')))) :=
  le_trans (costSum_proCutPre_le htbl hP htblN hWc hBt hp hpD hnpD hE hΓ).1
    (costShape_mono (le_trans (len_proCutPre_le hWc walkPieces T hp) (mul_le_mul_of_nonneg_left hpD zero_le)))
...
theorem costSum_proWk_le' {tbl N N' B' B Wl Wc W T s c i D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    ...
    (hsD : setLen LAct s ≤ D) (hcD : setLen LAct c ≤ D) (hE : 13 * D + 18 * ‖D‖ + 12 ≤ E) (hiE : i + 14 * D + 5 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout walkPieces Wc T Γ s i) :
    costSum N E Γ (proWk walkPieces Wl Wc W T s c i) ≤ (44 * D + 7) *
      (costK N E B 8 (layQ B B' D) (layD N' B' D) + (3 * ((8 : ℕ) : V) + 11) *
        (ctxBoundG (growK B E (layQ B B' D)) Γ (44 * D + 7) + (fvOccS LAct Γ + (44 * D + 7) * growK B E (layQ B B' D)))) :=
  le_trans (costSum_proWk_le htbl hP htblN hWl hWc hWp hBt hPle hs hc hk1 hsD hcD hE hiE hΓ hLay)
    (costShape_mono (le_trans (len_proWk_le htbl hP hWl hWc walkPieces W T s i (fun x hx ↦ hs x (hc hx))) (lin_le hcD)))
...
theorem costSum_proOr_le' {tbl N N' B' B Wl Wc W T s p q i ip iq D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    ...
    (hipE : ip + 14 * D + 6 ≤ E) (hiqE : iq + 8 * D + 4 ≤ E) (hΓ : IsFormulaSet LAct Γ)
    (hLay : Layout walkPieces Wc T Γ s i) (hDp : DossF walkPieces Γ 0 p ip) (hDq : DossF walkPieces Γ 0 q iq) :
    costSum N E Γ (proOr walkPieces Wl Wc W T s p q i ip iq) ≤ (110 * D + 25) *
      (costK N E B 8 (layQ B B' D) (layD N' B' D) + (3 * ((8 : ℕ) : V) + 11) *
        (ctxBoundG (growK B E (layQ B B' D)) Γ (110 * D + 25) + (fvOccS LAct Γ + (110 * D + 25) * growK B E (layQ B B' D)))) :=
  le_trans (costSum_proOr_le htbl hP htblN hWl hWc hWp hBt hPle hs hp hq hk1 hsD hE hiE hipE hiqE hΓ hLay hDp hDq)
    (costShape_mono (le_trans (len_proOr_le htbl hP hWl hWc walkPieces W T i ip iq hs hp hq) (lin_le hsD)))
...
theorem costSum_proShift_le' {tbl N N' B' B Wl Wc W T s c i D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    ...
    (hsD : setLen LAct s ≤ D) (hcD : setLen LAct c ≤ D) (hE : 13 * D + 18 * ‖D‖ + 12 ≤ E) (hiE : i + 16 * D + 8 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout walkPieces Wc T Γ s i) :
    costSum N E Γ (proShift walkPieces Wl Wc W T s c i) ≤ (62 * D + 20) *
      (costK N E B 8 (layQ B B' D) (layD N' B' D) + (3 * ((8 : ℕ) : V) + 11) *
        (ctxBoundG (growK B E (layQ B B' D)) Γ (62 * D + 20) + (fvOccS LAct Γ + (62 * D + 20) * growK B E (layQ B B' D)))) := by
  have hl : len (proShift walkPieces Wl Wc W T s c i) ≤ 62 * D + 20 := by
    refine le_trans (len_proShift_le hWc walkPieces Wl W T i hs hc) ?_
    calc 61 * setLen LAct c + setLen LAct s + 20 ≤ 61 * D + D + 20 := add_le_add (add_le_add (mul_le_mul_of_nonneg_left hcD zero_le) hsD) le_rfl
      _ = 62 * D + 20 := by ring
  exact le_trans (costSum_proShift_le htbl hP htblN hWl hWc hWp hBt hPle hs hc hsc hk1' hsD hcD hE hiE hΓ hLay) (costShape_mono hl)
...
theorem costSum_proAll_le' {tbl N N' B' B Wl Wc W T s p i D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    ...
    (hiE : i + 2 * ((1 + D) * (1 + D + 1)) * D + 40 * D + 20 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout walkPieces Wc T Γ s i) :
    costSum N E Γ (proAll walkPieces Wl Wc W T s p i) ≤ (450 * (D + 1) ^ 5) *
      (costK N E B 9 (layQ B B' D) (layD N' B' D) + (3 * ((9 : ℕ) : V) + 11) *
        (ctxBoundG (growK B E (layQ B B' D)) Γ (450 * (D + 1) ^ 5) +
          (fvOccS LAct Γ + (450 * (D + 1) ^ 5) * growK B E (layQ B B' D)))) :=
  le_trans (costSum_proAll_le htbl hP htblN hWl hWc hWp hBt hPle hs hp hr hsD hcD hspD hE hEQ hiE hΓ hLay)
    (costShape_mono (len_proAll_le htbl hP hWl hWc walkPieces W T i hs hp hr hsD hcD hspD))
...
theorem costSum_proExs_le' {tbl N N' B' B Wl Wc W T s p t i D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    ...
    (hiE : i + 2 * ((1 + D) * (1 + D + D)) * D + 40 * D + 20 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout walkPieces Wc T Γ s i) :
    costSum N E Γ (proExs walkPieces Wl Wc W T s p t i) ≤ (310 * (D + 1) ^ 5) *
      (costK N E B 9 (layQ B B' D) (layD N' B' D) + (3 * ((9 : ℕ) : V) + 11) *
        (ctxBoundG (growK B E (layQ B B' D)) Γ (310 * (D + 1) ^ 5) +
          (fvOccS LAct Γ + (310 * (D + 1) ^ 5) * growK B E (layQ B B' D)))) :=
  le_trans (costSum_proExs_le htbl hP htblN hWl hWc hWp hBt hPle hs hp hr ht hsD hcD htD hE hEQ hiE hΓ hLay)
    (costShape_mono (len_proExs_le htbl hP hWl hWc walkPieces W T i hs hp hr ht hsD hcD htD))

theorem costSum_proWk0_le' {tbl N B W Wc T s i E Γ Q Dd : V} (htbl : TableOK tbl N) (hP : ProTable tbl) (hWp : W = proPieces)
    (hBt : ∀ j < len tbl, formulaLen LAct (rowB tbl.[j]) ≤ B) (hΓ : IsFormulaSet LAct Γ)
    (hiE : i + 2 + (len (memberList s) + 1) + 1 ≤ E) (hLay : Layout walkPieces Wc T Γ s i) :
    costSum N E Γ (proWk0 W s i) ≤ 7 *
      (costK N E B 8 Q Dd + (3 * ((8 : ℕ) : V) + 11) * (ctxBoundG (growK B E Q) Γ 7 + (fvOccS LAct Γ + 7 * growK B E Q))) := by
  have := costSum_proWk0_le htbl hP hWp hBt hΓ hiE hLay (Q := Q) (Dd := Dd)
  rwa [len_proWk0] at this

theorem costSum_proShift0_le' {tbl N B W Wc T i E Γ Q Dd : V} (htbl : TableOK tbl N) (hP : ProTable tbl) (hWp : W = proPieces)
    (hBt : ∀ j < len tbl, formulaLen LAct (rowB tbl.[j]) ≤ B) (hΓ : IsFormulaSet LAct Γ)
    (hiE : i + 3 + 1 + 1 ≤ E) (hLay : Layout0 walkPieces Wc T Γ i) :
    costSum N E Γ (proShift0 W i) ≤ 11 *
      (costK N E B 8 Q Dd + (3 * ((8 : ℕ) : V) + 11) * (ctxBoundG (growK B E Q) Γ 11 + (fvOccS LAct Γ + 11 * growK B E Q))) := by
  have := costSum_proShift0_le htbl hP hWp hBt hΓ hiE hLay (Q := Q) (Dd := Dd)
  rwa [len_proShift0] at this
...
theorem costSum_proIns0_le' {tbl N N' B' B Wl Wc W T p i ip D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    ...
    (hE : 13 * D + 18 * ‖D‖ + 12 ≤ E) (hiE : i + 14 * D + 5 ≤ E) (hipE : ip + 8 * D + 4 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout0 walkPieces Wc T Γ i) (hDp : DossF walkPieces Γ 0 p ip) :
    costSum N E Γ (proIns0 walkPieces Wl Wc W T p i ip) ≤ (49 * D + 13) *
      (costK N E B 8 (layQ B B' D) (layD N' B' D) + (3 * ((8 : ℕ) : V) + 11) *
        (ctxBoundG (growK B E (layQ B B' D)) Γ (49 * D + 13) + (fvOccS LAct Γ + (49 * D + 13) * growK B E (layQ B B' D)))) :=
  le_trans (costSum_proIns0_le htbl hP htblN hWl hWc hWp hBt hPle hp hsD hE hiE hipE hΓ hLay hDp)
    (costShape_mono (le_trans (len_proIns0_le htbl hP hWl hWc walkPieces W T i ip hp) (lin_le hsD)))

/-! ### 18.13 The shift counts: `shiftsV ≤ len` uniformly, and the sharp values from the `_ok` equalities -/

theorem shiftsV_proAxL_le {Wc : V} (hWc : Wc = certPieces) (Ww T s i : V) {p : V} (hp : IsSemiformula LAct 0 p) :
    shiftsV (proAxL Ww Wc T s p i) + 4 ≤ 12 * formulaLen LAct p :=
  le_trans (add_le_add (shiftsV_le_len _) le_rfl) (len_proAxL_le hWc Ww T s i hp)

theorem shiftsV_proIns_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T i ip : V) {s p : V} (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 0 p) :
    shiftsV (proIns Ww Wl Wc W T s p i ip) ≤ 55 * setLen LAct (insert p s) + 12 :=
  le_trans (shiftsV_le_len _) (len_proIns_le htbl hP hWl hWc Ww W T i ip hs hp)

theorem shiftsV_proOr_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T i ip iq : V) {s p q : V} (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 0 p)
    (hq : IsSemiformula LAct 0 q) :
    shiftsV (proOr Ww Wl Wc W T s p q i ip iq) ≤ 110 * setLen LAct (insert p (insert q s)) + 25 :=
  le_trans (shiftsV_le_len _) (len_proOr_le htbl hP hWl hWc Ww W T i ip iq hs hp hq)

theorem shiftsV_proCutPre_le {Wc : V} (hWc : Wc = certPieces) (Ww T : V) {p : V} (hp : IsSemiformula LAct 0 p) :
    shiftsV (proCutPre Ww Wc T p) ≤ 64 * formulaLen LAct p :=
  le_trans (shiftsV_le_len _) (len_proCutPre_le hWc Ww T hp)

theorem shiftsV_proWk_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T s i : V) {c : V} (hc : IsFormulaSet LAct c) :
    shiftsV (proWk Ww Wl Wc W T s c i) ≤ 44 * setLen LAct c + 7 :=
  le_trans (shiftsV_le_len _) (len_proWk_le htbl hP hWl hWc Ww W T s i hc)

theorem shiftsV_proShift_le {Wc : V} (hWc : Wc = certPieces) (Ww Wl W T i : V) {s c : V} (hs : IsFormulaSet LAct s)
    (hc : IsFormulaSet LAct c) : shiftsV (proShift Ww Wl Wc W T s c i) ≤ 61 * setLen LAct c + setLen LAct s + 20 :=
  le_trans (shiftsV_le_len _) (len_proShift_le hWc Ww Wl W T i hs hc)

theorem shiftsV_proSS_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T i : V) {s : V} (hs : IsFormulaSet LAct s) :
    shiftsV (proSS Ww Wl Wc W T s i) ≤ 40 * setLen LAct (setShift LAct s) + 69 * setLen LAct s + 30 :=
  le_trans (shiftsV_le_len _) (len_proSS_le htbl hP hWl hWc Ww W T i hs)

theorem shiftsV_proAll_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T i : V) {s p D : V} (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 1 p) (hr : (^∀ p) ∈ s)
    (hsD : setLen LAct s ≤ D) (hcD : setLen LAct (insert (free LAct p) (setShift LAct s)) ≤ D)
    (hspD : formulaLen LAct (shift LAct p) ≤ D) :
    shiftsV (proAll Ww Wl Wc W T s p i) ≤ 450 * (D + 1) ^ 5 :=
  le_trans (shiftsV_le_len _) (len_proAll_le htbl hP hWl hWc Ww W T i hs hp hr hsD hcD hspD)

theorem shiftsV_proExs_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T i : V) {s p t D : V} (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 1 p)
    (hr : (^∃ p) ∈ s) (ht : IsSemiterm LAct 0 t) (hsD : setLen LAct s ≤ D)
    (hcD : setLen LAct (insert (substs1 LAct t p) s) ≤ D) (htD : termLen LAct t ≤ D) :
    shiftsV (proExs Ww Wl Wc W T s p t i) ≤ 310 * (D + 1) ^ 5 :=
  le_trans (shiftsV_le_len _) (len_proExs_le htbl hP hWl hWc Ww W T i hs hp hr ht hsD hcD htD)

theorem shiftsV_proIns0_le {tbl N : V} (htbl : TableOK tbl N) (hP : ProTable tbl) {Wl Wc : V} (hWl : Wl = layoutPieces)
    (hWc : Wc = certPieces) (Ww W T i ip : V) {p : V} (hp : IsSemiformula LAct 0 p) :
    shiftsV (proIns0 Ww Wl Wc W T p i ip) ≤ 49 * setLen LAct (insert p (0 : V)) + 13 :=
  le_trans (shiftsV_le_len _) (len_proIns0_le htbl hP hWl hWc Ww W T i ip hp)

/-- The sharp counts (each `_ok` gives the exact expression; `proSig ≤ 6D + 1`, `mShift ≤ 4|x|`). -/
theorem shiftsV_proIns_le' {tbl N N' B' Wl Wc W T s p i ip D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWl : Wl = layoutPieces) (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 0 p) (hk1 : 1 ≤ len (memberList s))
    (hsD : setLen LAct (insert p s) ≤ D) (hE : 13 * D + 18 * ‖D‖ + 12 ≤ E) (hiE : i + 14 * D + 5 ≤ E)
    (hipE : ip + 8 * D + 4 ≤ E) (hΓ : IsFormulaSet LAct Γ)
    (hLay : Layout walkPieces Wc T Γ s i) (hDp : DossF walkPieces Γ 0 p ip) :
    shiftsV (proIns walkPieces Wl Wc W T s p i ip) ≤ 6 * D + 2 := by
  have hcs : IsFormulaSet LAct (insert p s) := IsFormulaSet.insert_iff.mpr ⟨hp, hs⟩
  have hE8 : 13 * D + 18 * ‖D‖ + 8 ≤ E := le_trans (add_le_add le_rfl (by norm_num)) hE
  have hσ := proSig_le htbl hP hWc htblN hWl hWp hcs (one_le_len_memberList_insert p s) hsD hE8 hΓ
  rw [(proIns_ok htbl hP htblN hWl hWc hWp hs hp hk1 hsD hE hiE hipE hΓ hLay hDp).2.2.1]
  calc 1 + proSig walkPieces Wl Wc W T (insert p s) ≤ 1 + (6 * D + 1) := add_le_add le_rfl hσ
    _ = 6 * D + 2 := by ring

theorem shiftsV_proWk_le' {tbl N N' B' Wl Wc W T s c i D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWl : Wl = layoutPieces) (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hs : IsFormulaSet LAct s) (hc : c ⊆ s) (hk1 : 1 ≤ len (memberList c))
    (hsD : setLen LAct s ≤ D) (hcD : setLen LAct c ≤ D) (hE : 13 * D + 18 * ‖D‖ + 12 ≤ E) (hiE : i + 14 * D + 5 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout walkPieces Wc T Γ s i) :
    shiftsV (proWk walkPieces Wl Wc W T s c i) ≤ 6 * D + 1 := by
  have hcs : IsFormulaSet LAct c := fun x hx ↦ hs x (hc hx)
  have hE8 : 13 * D + 18 * ‖D‖ + 8 ≤ E := le_trans (add_le_add le_rfl (by norm_num)) hE
  rw [(proWk_ok htbl hP htblN hWl hWc hWp hs hc hk1 hsD hcD hE hiE hΓ hLay).2.2.1]
  exact proSig_le htbl hP hWc htblN hWl hWp hcs hk1 hcD hE8 hΓ

theorem shiftsV_proOr_le' {tbl N N' B' Wl Wc W T s p q i ip iq D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWl : Wl = layoutPieces) (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hs : IsFormulaSet LAct s) (hp : IsSemiformula LAct 0 p) (hq : IsSemiformula LAct 0 q) (hk1 : 1 ≤ len (memberList s))
    (hsD : setLen LAct (insert p (insert q s)) ≤ D) (hE : 13 * D + 18 * ‖D‖ + 12 ≤ E) (hiE : i + 14 * D + 5 ≤ E)
    (hipE : ip + 14 * D + 6 ≤ E) (hiqE : iq + 8 * D + 4 ≤ E) (hΓ : IsFormulaSet LAct Γ)
    (hLay : Layout walkPieces Wc T Γ s i) (hDp : DossF walkPieces Γ 0 p ip) (hDq : DossF walkPieces Γ 0 q iq) :
    shiftsV (proOr walkPieces Wl Wc W T s p q i ip iq) ≤ 12 * D + 4 := by
  have hqs : IsFormulaSet LAct (insert q s) := IsFormulaSet.insert_iff.mpr ⟨hq, hs⟩
  have hpqs : IsFormulaSet LAct (insert p (insert q s)) := IsFormulaSet.insert_iff.mpr ⟨hp, hqs⟩
  have hE8 : 13 * D + 18 * ‖D‖ + 8 ≤ E := le_trans (add_le_add le_rfl (by norm_num)) hE
  have hσ₁ := proSig_le htbl hP hWc htblN hWl hWp hqs (one_le_len_memberList_insert q s) (le_trans (setLen_le_insert p _) hsD) hE8 hΓ
  have hσ₂ := proSig_le htbl hP hWc htblN hWl hWp hpqs (one_le_len_memberList_insert p _) hsD hE8 hΓ
  rw [(proOr_ok htbl hP htblN hWl hWc hWp hs hp hq hk1 hsD hE hiE hipE hiqE hΓ hLay hDp hDq).2.2.1]
  calc 1 + proSig walkPieces Wl Wc W T (insert q s) + (1 + proSig walkPieces Wl Wc W T (insert p (insert q s)))
      ≤ 1 + (6 * D + 1) + (1 + (6 * D + 1)) := add_le_add (add_le_add le_rfl hσ₁) (add_le_add le_rfl hσ₂)
    _ = 12 * D + 4 := by ring

theorem shiftsV_proShift_le' {tbl N N' B' Wl Wc W T s c i D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWl : Wl = layoutPieces) (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hs : IsFormulaSet LAct s) (hc : IsFormulaSet LAct c) (hsc : s = setShift LAct c) (hk1' : 1 ≤ len (memberList c))
    (hsD : setLen LAct s ≤ D) (hcD : setLen LAct c ≤ D) (hE : 13 * D + 18 * ‖D‖ + 12 ≤ E) (hiE : i + 16 * D + 8 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout walkPieces Wc T Γ s i) :
    shiftsV (proShift walkPieces Wl Wc W T s c i) ≤ 7 * D + 3 := by
  have hE8 : 13 * D + 18 * ‖D‖ + 8 ≤ E := le_trans (add_le_add le_rfl (by norm_num)) hE
  have hσ := proSig_le htbl hP hWc htblN hWl hWp hc hk1' hcD hE8 hΓ
  have hk' := le_trans (len_memberList_le_setLen hc) hcD
  rw [(proShift_ok htbl hP htblN hWl hWc hWp hs hc hsc hk1' hsD hcD hE hiE hΓ hLay).2.2.1]
  calc 1 + (len (memberList c) + 1) + proSig walkPieces Wl Wc W T c ≤ 1 + (D + 1) + (6 * D + 1) :=
        add_le_add (add_le_add le_rfl (add_le_add hk' le_rfl)) hσ
    _ = 7 * D + 3 := by ring

theorem shiftsV_proCutPre_le' {tbl N N' B' Wc T p D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWc : Wc = certPieces)
    (hp : IsSemiformula LAct 0 p) (hpD : formulaLen LAct p ≤ D) (hnpD : formulaLen LAct (neg LAct p) ≤ D)
    (hE : 13 * D + 8 ≤ E) (hΓ : IsFormulaSet LAct Γ) :
    shiftsV (proCutPre walkPieces Wc T p) ≤ 8 * D := by
  rw [(proCutPre_ok htbl hP htblN hWc hp hpD hnpD hE hΓ).2.2.1]
  calc mShift walkPieces Wc T p + mShift walkPieces Wc T (neg LAct p) ≤ 4 * formulaLen LAct p + 4 * formulaLen LAct (neg LAct p) :=
        add_le_add (mShift_le htbl hP.walkTable hWc T hp) (mShift_le htbl hP.walkTable hWc T hp.neg)
    _ ≤ 4 * D + 4 * D := add_le_add (mul_le_mul_of_nonneg_left hpD zero_le) (mul_le_mul_of_nonneg_left hnpD zero_le)
    _ = 8 * D := by ring

theorem shiftsV_proIns0_le' {tbl N N' B' Wl Wc W T p i ip D E Γ : V} (htbl : TableOK tbl N) (hP : ProTable tbl)
    (htblN : NumTableOK T N' B') (hWl : Wl = layoutPieces) (hWc : Wc = certPieces) (hWp : W = proPieces)
    (hp : IsSemiformula LAct 0 p) (hsD : setLen LAct (insert p (0 : V)) ≤ D)
    (hE : 13 * D + 18 * ‖D‖ + 12 ≤ E) (hiE : i + 14 * D + 5 ≤ E) (hipE : ip + 8 * D + 4 ≤ E)
    (hΓ : IsFormulaSet LAct Γ) (hLay : Layout0 walkPieces Wc T Γ i) (hDp : DossF walkPieces Γ 0 p ip) :
    shiftsV (proIns0 walkPieces Wl Wc W T p i ip) ≤ 6 * D + 2 := by
  have hcs : IsFormulaSet LAct (insert p (0 : V)) := IsFormulaSet.insert_iff.mpr ⟨hp, fun x hx ↦ by simp at hx⟩
  have hE8 : 13 * D + 18 * ‖D‖ + 8 ≤ E := le_trans (add_le_add le_rfl (by norm_num)) hE
  have hσ := proSig_le htbl hP hWc htblN hWl hWp hcs (one_le_len_memberList_insert p 0) hsD hE8 hΓ
  rw [(proIns0_ok htbl hP htblN hWl hWc hWp hp hsD hE hiE hipE hΓ hLay hDp).2.2.1]
  calc 1 + proSig walkPieces Wl Wc W T (insert p (0 : V)) ≤ 1 + (6 * D + 1) := add_le_add le_rfl hσ
    _ = 6 * D + 2 := by ring
...
**2026-09-15, night — `all` and `exs` DELIVERED (§13–§15).** `proAll_ok` (§13.4) and `proExs_ok` (§14.5) discharge every
`nodeAll_ok`/`nodeExs_ok` hypothesis except the child's own goal (`postIns`, as for `and`): the child at `0`, the parent
transported, the certified facts at explicit indices (see the docstrings). DESIGN CHANGES against the plan above: (a) no
"unshift index" — `proSS` (§13.2) lays out `setShift s` and REUSES `proShift_ok` with the roles `(setShift s, s, 0)` (a second
layout of `s` at `0`), then identifies the fresh chain top with the parent's `S` by Loop E (`eqSteps` per member, both
`congMem` directions) + two `subChain`s + `subsetAntisymm` + row 158 `congSetShiftR`; (b) `proIns_ok` is REUSED for the
`insert` in both tags (`insert (free p) (setShift s)`, `insert (substs1 t p) s`) — the fresh dossiers of `free p` /
`substs1 t p` are walked BEFORE the layouts, so `certFree`/`certSubst` run between fresh dossiers and the child stays at `0`;
(c) `exs`'s length OBJECT: `eqTotal [bnum |t|]`, `eqSymm`, the re-indexed cert row `congTLenNum`, `leOfEqP` (159) —
`nodeExs_ok`'s `&ilt` is a variable, not the numeral. CAPS: `D` bounds the parent, the child, `|shift p|` (all) / `|t|`
(exs); besides `13D + 18‖D‖ + 12 ≤ E` the quadratic `2·QD·(D+1) + 4D + 11 ≤ E` and `i + 2·QD·D + 40D + 20 ≤ E` with
`QD = (1+D)(2+D)` (all: `subFPre_single` at `fvec`, `B = 1`) or `(1+D)(1+2D)` (exs: `B = |t|`). `certFreeDef` (§13.3) is the
Σ₁ graph `Cert` lacked. COSTS (§15): `sizeOK_proAll/proExs` at the layout class, `costSum_proAll/proExs_le` at cap `9`.
TRAPS: `PrologueRows.olean` was STALE when this session started (`leOfEqP` committed without a rebuild; `proExtraRowCount = 4`
in the olean — `#print proExtraRowCount` is the sentinel); `lake build` REMOVES `Prologue.olean` while it rebuilds (scratch
checks must wait); `cok_*` lemmas take `hWc` — passing `rfl` produces a step at `certPieces` that no longer matches a `Wc`
step; `dossV_succ (j := 0)` wants the vector's dossier stated with `0 + 1`; `proShift_ok`/`proSS_ok` return the parent
offset LEFT-associated (`i + a + b + (…)`) — re-associate with a `ring` equation before `exact`; `rw [finalCtx_single]`
picks the OUTERMOST list — `simp only [finalCtx_single]` then the `cx` rewrites. NOT DONE: `proIns0` (the empty parent of
`cut`), the `0`-producers' costs, `axm`, the `Verify` clause edit.

**2026-09-15, later — `proIns0` and the `0`-producers' costs DELIVERED (§16–§17).** `proIns0_ok`: the `insert` child of the
EMPTY parent (`cut` at `∅`), `identIns0` = Loop A + the child's `subChain` + `emptySubsetC [s'']` + row 157 `congSubsetL
[𝟎, S, s'']` (`S ⊆ s''` from `Layout0`'s `S = 𝟎`) + the `p` block + `insertSubset` + `subsetAntisymm`; takes `Layout0 … i`
and returns `Layout0 … (i + 1 + σ)`, the child at `0`, `insFact cp P S`, `eqFactB cp s''` (`S = &(i + 1 + σ + (len
(memberList 0) + 1))` — `len (memberList 0) = 0` is `len_memberList_zero`). `sizeOK_layoutSteps0/proWk0/proShift0` (every
class: tags `0/2` only) with `costSum_*_le` at cap `8`; `sizeOK_proIns0`/`costSum_proIns0_le` at the layout class. STILL NOT
DONE: `axm`, the `Verify` clause edit, the verify recursion carrying `1 ≤ k ∧ Layout ∨ k = 0 ∧ Layout0`. 
**2026-09-15, evening — §18 the LENGTH and SHIFT bounds DELIVERED** (the `len (pro…)` multipliers of every cost lemma made
concrete; see §18's head docstring for the list and THE FINDING: `all`/`exs` are degree 5 in the size bound here, degree 3
with `Pin`'s linear-in-`Q` `certSubst` bound — `VerifyKit'.cost`'s `len ≤ Ck·(dlen ρ + 1)` is not linear at those nodes).
`NodeSize.lean` (new, imports `DerivationLength` + `ProperV`) has the node laws: `setLen (fstIdx d) ≤ dlen d`, the
principal formulas/witness/children `≤ dlen d`. TRAPS: `formulaLen_le_setLen_of_mem` needs `(L := LAct)` when `L` is not
fixed by the membership; `isSemitermVec_fvec`/`substInv_fvec` need their type ascribed (`(… : IsSemitermVec LAct 1 0
(fvec : V))`); `lenFoldAux` is 6-ary — induct on the packed `lenFoldAuxP ⟪W, T, xs, os, k⟫`; `len_subFGraph_le`'s implicit
`Wd W iw i j` must be named when nothing downstream fixes them; `norm_num` turned a true `12 * 1 + 4 = 14` goal into
`False` inside a `calc` — use `le_of_eq (by ring)` and `le_of_add_le_add_right`.
-/
