...
for name in TABLE:
    ...
    E('')
E('/-! ### The table predicate and its per-row readings -/')
E('')
E('/-- The recognizer table: `indRecRowCount` rows at least, the `i`-th with the arity and matrix of `indRecRows[i]`. -/')
E('def IndRecTable (tbl : V) : Prop :=')
E('  (indRecRowCount : V) ≤ len tbl ∧')
E('  ∀ (i : ℕ) (h : i < indRecRows.length),')
E('    rowM tbl.[(i : V)] = ((indRecRows[i]).m : V) ∧ rowB tbl.[(i : V)] = ⌜Semiformula.lMap emb (indRecRows[i]).B⌝')
E('')
E('lemma IndRecTable.proAxmTable {tbl : V} (h : IndRecTable tbl) : ProAxmTable tbl := by')
E('  refine ⟨le_trans (by exact_mod_cast (show proAxmRowCount ≤ indRecRowCount by simp only [indRecRowCount]; omega)) h.1, ?_⟩')
E('  intro i hi')
E('  have hi\' : i < indRecRows.length := by')
E('    rw [indRecRows_length]; rw [proAxmRows_length] at hi; simp only [indRecRowCount]; omega')
E('  have := h.2 i hi\'')
E('  rwa [show indRecRows[i] = proAxmRows[i] from List.getElem_append_left hi] at this')
E('')
E('lemma IndRecTable.numIdTable {tbl : V} (h : IndRecTable tbl) : NumIdTable tbl := h.proAxmTable.numIdTable')
E('lemma IndRecTable.proTable {tbl : V} (h : IndRecTable tbl) : ProTable tbl := h.proAxmTable.proTable')
E('lemma IndRecTable.layoutTable {tbl : V} (h : IndRecTable tbl) : LayoutTable tbl := h.proAxmTable.layoutTable')
E('lemma IndRecTable.walkTable {tbl : V} (h : IndRecTable tbl) : WalkTable tbl := h.proAxmTable.walkTable')
E('')
E('/-- The reading of the `k`-th extra row. -/')
E('lemma IndRecTable.extra {tbl : V} (h : IndRecTable tbl) (k : ℕ) (hk : k < indRecExtraRowCount) :')
E('    ((proAxmRowCount + k : ℕ) : V) < len tbl ∧')
E('    rowM tbl.[((proAxmRowCount + k : ℕ) : V)] = ((indRecExtraRows[k]\'(by rw [indRecExtraRows_length]; exact hk)).m : V) ∧')
E('    rowB tbl.[((proAxmRowCount + k : ℕ) : V)] =')
E('      ⌜Semiformula.lMap emb (indRecExtraRows[k]\'(by rw [indRecExtraRows_length]; exact hk)).B⌝ := by')
E('  have hlt : proAxmRowCount + k < indRecRows.length := by rw [indRecRows_length]; simp only [indRecRowCount]; omega')
E('  have hr := h.2 (proAxmRowCount + k) hlt')
E('  have hk\' : k < indRecExtraRows.length := by rw [indRecExtraRows_length]; exact hk')
E('  have e\' : indRecRows[proAxmRowCount + k]? = indRecExtraRows[k]? := by')
E('    unfold indRecRows')
E('    rw [List.getElem?_append_right (by rw [proAxmRows_length]; omega), proAxmRows_length, Nat.add_sub_cancel_left]')
E('  have e : indRecRows[proAxmRowCount + k]\'hlt = indRecExtraRows[k]\'hk\' := by')
E('    rw [List.getElem?_eq_getElem hlt, List.getElem?_eq_getElem hk\'] at e\'')
E('    exact Option.some.inj e\'')
E('  rw [e] at hr')
E('  exact ⟨lt_of_lt_of_le (by exact_mod_cast hlt) (indRecRows_length ▸ h.1), hr.1, hr.2⟩')
E('')
for k, name in enumerate(TABLE):
    d = info[name]
    E(f'lemma IndRecTable.{name} {{tbl : V}} (h : IndRecTable tbl) :')
    E(f'    ((iIdx_{name} : ℕ) : V) < len tbl ∧')
    E(f'    rowM tbl.[((iIdx_{name} : ℕ) : V)] = (({d["m"]} : ℕ) : V) ∧')
    E(f'    rowB tbl.[((iIdx_{name} : ℕ) : V)] = impChainV LAct (vecOf row_{name}_as) row_{name}_c := by')
    E(f'  have := h.extra {k} (by decide)')
    E(f'  have e : ((iIdx_{name} : ℕ) : V) = ((proAxmRowCount + {k} : ℕ) : V) := rfl')
    E(f'  rw [e]')
    E(f'  refine ⟨this.1, this.2.1, ?_⟩')
    E(f'  rw [impChainV_vecOf, this.2.2]')
    E(f'  exact quote_row_{name}')
    E('')
E('end indRecRowsTable')
table_part = '\n'.join(o)

header = open(os.path.join(HERE, 'IndRecRows.lean.head')).read()
header = header.replace('import ArithS.Necessitation.ProAxmRows', 'import ArithS.Necessitation.ProAxm')
