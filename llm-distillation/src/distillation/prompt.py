...
opponent's name + source, ask for a single C/D action) is settled.
"""

from __future__ import annotations

# Placeholder prompt. Refine the framing / payoff matrix before running real
# experiments. ``{bot_name}`` and ``{bot_source}`` are filled by build_prompt.
PROMPT_TEMPLATE = """\
...
def build_prompt(bot_name: str, bot_source: str) -> str:
    """Fill the prompt template for a single opponent bot."""
    return PROMPT_TEMPLATE.format(bot_name=bot_name, bot_source=bot_source)
