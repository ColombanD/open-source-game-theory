...
It remains to say what ``40\% transparency'' means. The dial \emph{is} the
normalized mutual information $I(B;\sigma(B))/H(B)$ of the signalling
channel under a uniform prior over the zoo, with $t = 1$ full transparency
and $t = 0$ opaque. Whatever internal knob a signal family uses (a softmax
temperature, a mixing weight) is bisection-inverted against this scale and
never exposed. The payoff of this calibration discipline is comparability:
at matched $t$, every signal family carries the \emph{same information
rate}, so any difference between families at equal $t$ isolates the effect
of confusion \emph{structure} --- which bots get mistaken for which ---
rather than differences in how much information flows.
...
$\sigma$, and the sub-zoo the matrix is restricted to.
...
$d$, with the temperature $T$ calibrated onto the shared $t$ dial.
...
and a structured family at matched $t$ isolates the effect of confusion
structure itself.
...
is scale-free. Two conventions matter. Search \emph{budgets} are erased ---
$\cupod\,k$ and $\cupod\,(2k{+}64)$ are the same code to this observer,
because the budget is Critch's own \emph{depth} dial and folding it into
the code channel would conflate two orthogonal transparency axes. And
referenced bots are opaque labelled leaves: an observer glimpsing
\texttt{.bot DefectBot} sees the \emph{name}, not the body. The syntactic
...
\zoobot{TitForTatBot}) is the clean instrument. It is behaviorally
\emph{twin-free} --- no two members share an action row --- so its
transparency scale reaches exactly $1.0$ and every bot is identifiable
from behavior alone. It carries two stipulated (unproven) cells involving
...
contributes no cooperation mass. And the zoo has two behavioral twin
groups, $\{$\zoobot{CooperateBot}, \zoobot{LegibleBot}$\}$ and
$\{$\dupoc, \zoobot{JustBot}$\}$, which cap the behavioral transparency
ceiling at $0.938$: on this zoo, ``$t = 1$'' means \emph{the most
informative behavioral signal possible}, not a true point mass. As we will
see, this ceiling is not a nuisance parameter --- it produces one of the
more instructive results.
...
at $t = 1$, $\alpha = 0.5$, zero cells differ from base.

On the enlarged zoo, three cells differ from base \emph{even at full
transparency}, and both underlying mechanisms are findings rather than
bugs. The first is that \textbf{twins make the base matrix unrecoverable
for any bot that treats twins differently}. A signal that the opponent is
\dupoc{} is \emph{necessarily} the fifty-fifty mixture
$\{\dupoc: 0.5,\ \zoobot{JustBot}: 0.5\}$: the two have identical action
rows, so no behavioral signal, at any blur level, can separate them.
\dimcid{} happens to be the unique zoo member that plays differently
against the pair ($D$ against \dupoc, $C$ against \zoobot{JustBot}), so
its cooperation mass against a ``\dupoc'' signal is exactly $0.50$ --- and
no caution threshold avoids distortion, since $\alpha \le 0.5$ flips its
\dupoc{} cell to $C$ while $\alpha > 0.5$ flips its \zoobot{JustBot} cell
to $D$. The abstract twin ceiling of $0.938$ is thus made concrete in a
single bot: \dimcid{} can never behave as its base self on this zoo, at
any transparency.

The second failure is that \textbf{the $N$ cell is unreproducible by
construction}. Tau agents output only $C$ or $D$; \zoobot{MirrorBot}'s
proven-\texttt{none} self-play has no image in that codomain, and under
the pessimistic reading its self-mass is $0$, so tau-\zoobot{MirrorBot}
defects against itself at every $\alpha > 0$ --- where the base
\zoobot{MirrorBot} \emph{diverges} against itself.

The honest statement of the anchor theorem is therefore: \emph{full
transparency reproduces the base matrix on twin-free zoos with no $N$
cells}. That is exactly the default zoo, and provably not the enlarged
one.
...
anchor at $t=1$ & exact & 3 cells off (twins $+$ $N$) \\
...
practice). The $N$ cell is read pessimistically, and the
\zoobot{MirrorBot} conclusions should be re-checked under the optimistic
convention before anything is built on them. Grids are $t$ in steps of
