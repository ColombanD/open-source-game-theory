...
\begin{algorithm}[h]
\caption{TauDupocBot: a concrete example of modified Def.~3}
\begin{algorithmic}
\Procedure{$\Tau$DupocBot$(\alpha)$}{$\Tau B_1,\dots,\Tau B_n; p_1,\dots,p_n$}
  \State $\mathit{sumCs} \gets 0$
  \For{$i = 1, \dots, n$}
    \If{proofSearch$(\Tau B_i(\alpha)($TauDupocBot$; 1) = C)$}
      \State $\mathit{sumCs} \mathrel{+}= p_i$
    \EndIf
  \EndFor
  \If{$\mathit{sumCs} \ge \alpha$} \Return $C$ \Else{} \Return $D$ \EndIf
...
\end{algorithm}
\begin{algorithm}[h]
\caption{TauTitForTatBot: a concrete example of modified Def.~3}
\begin{algorithmic}
\Procedure{$\Tau$TitForTatBot$(\alpha)$}{$\Tau B_1,\dots,\Tau B_n; p_1,\dots,p_n$}
  \State $\mathit{sumCs} \gets 0$
  \For{$i = 1, \dots, n$}
    \If{proofSearch$(\Tau B_i(\alpha)($TauCooperateBot$; 1) = C)$}
      \State $\mathit{sumCs} \mathrel{+}= p_i$
    \EndIf
  \EndFor
  \If{$\mathit{sumCs} \ge \alpha$} \Return $C$ \Else{} \Return $D$ \EndIf
...
\end{algorithm}
\begin{algorithm}[h]
\caption{TauCooperateBot: a concrete example of modified Def.~3}
\begin{algorithmic}
\Procedure{$\Tau$CooperateBot$(\alpha)$}{$\Tau B_1,\dots,\Tau B_n; p_1,\dots,p_n$}
\Return $C$
\EndProcedure
\end{algorithmic}
\end{algorithm}
\begin{algorithm}[h]
\caption{TauDefectBot: a concrete example of modified Def.~3}
\begin{algorithmic}
\Procedure{$\Tau$DefectBot$(\alpha)$}{$\Tau B_1,\dots,\Tau B_n; p_1,\dots,p_n$}
\Return $D$
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
You will find the full results here: \href{https://claude.ai/code/artifact/0db08fb2-3379-4136-ab78-3673a3bf6621}{Enlarged Zoo} - \href{https://claude.ai/code/artifact/5eec0ef0-eba7-4a48-847b-f5c0ee9b0e5a}{Default Zoo}. The following subsections summarize the main findings.
...
$0.01$ (default) and $0.02$ (enlarged).
...
Between these two edges lies \textbf{a dead zone in which $\alpha$ has no
effect at all}. The reason is structural: a cooperation mass is a sum of
signal weights, so at any fixed $t$ only finitely many masses are
achievable across the whole tournament, and moving $\alpha$ changes
nothing until it crosses one of them. Caution is not a continuous dial but
a staircase, and the interesting question is where its steps fall. At
$t = 0.5$ the staircase has a wide landing in the default zoo: no
achievable mass falls in $(0.41, 0.65)$, so an agent demanding $45\%$ of
its cooperation mass and one demanding $60\%$ play \emph{identically} ---
the entire moderate-caution band collapses to a single behavior. The
enlarged zoo narrows this gap to $(0.55, 0.65)$ and offers 158 achievable
masses against the default zoo's 98: in the richer zoo, caution genuinely
differentiates agents that the smaller zoo cannot tell apart. One
practical corollary for anyone extending these sweeps: sample $\alpha$
\emph{between} consecutive masses, never on one --- an $\alpha$ that lands
exactly on an achievable mass leaves the comparison
$\mathit{mass} \ge \alpha$ to floating-point residue.
...
\subsection{Distinguishing mutual cooperation, exploitation, and mutual defection}
...
exploitation, and mutual defection, for $\alpha = 0.5$ and the behavioral channel.
