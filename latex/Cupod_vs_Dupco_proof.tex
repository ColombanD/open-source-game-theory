...
For a budget $k\in\NN$ we write $T\vdash_k\varphi$ (\emph{bounded provability}) if some such proof has character length at most~$k$; this is exactly what the agents' $\ps$ queries (Definition~\ref{def:ps}) decide, and what the box $\bx{k}\varphi$ names \emph{inside} $S$. Three levels are kept apart throughout. The turnstiles $\vdash$, $\vdash_k$ always denote derivability in the object theory; $\NN\models\varphi$ denotes truth in the standard model; and claims \emph{about} $S$---soundness, the theorems of this note, the Lean verification---are stated in the metalanguage in prose, never with a turnstile. In particular $\nvdash_k\varphi$ (no $S$-proof of length $\le k$ exists) is a metastatement, distinct from the object refutation $\vdash_k\neg\varphi$ and from the internal $\vdash\neg\bx{k}\varphi$.
...
\[
\vdash_k\varphi\quad\Longleftrightarrow\quad \vdash_k\varphi^{\tr},\qquad\text{and}\qquad S\proves\bx{k}(\varphi)\leftrightarrow\bx{k}(\varphi^{\tr}).
\]
...
\begin{align*}
a=\Cact \ &\iff\ \vdash_k\rho_1\\
b=\Dact \ &\iff\ \vdash_k\rho_2
\end{align*}
with the respective default actions $\Dact$ and $\Cact$ otherwise (each agent acts by default unless its search succeeds, i.e.\ unless the bounded provability on the right holds).
...
\end{theorem}
\begin{proof}
By Theorem~\ref{thm:auto} (the bounded equivalence) and $\rho_1=\rho_2^{\tr}$ (Proposition~\ref{label}), $\vdash_k\rho_1\Leftrightarrow\vdash_k\rho_2$; call the common truth value $P$. Suppose $P$ holds. Then $\vdash_k\rho_1$ makes $a=\Cact$, while $\vdash_k\rho_2$ together with soundness gives $\NN\models\rho_2$, i.e.\ $\DUPOC(k)(\CUPOD(k)\source)=\Dact$, i.e.\ $a=\Dact$. As agents are deterministic, $a$ cannot be both; contradiction. Hence $P$ is false: $\nvdash_k\rho_1$, so $\DUPOC$ falls through to its default $a=\Dact$, and $\nvdash_k\rho_2$, so $\CUPOD$ falls through to its default $b=\Cact$. The classical unexploitability of Dupoc argument excludes $(a,b)=(\Cact, \Dact)$. Thus $(a,b)=(\Dact,\Cact)$.
\end{proof}
