...
instance : ProgramModel Program where -- Provide how `Program` chooses an action against an opponent.
  action -- Define the required `action` method from the `ProgramModel` typeclass.
    | Program.cooperate, _ => Action.C -- Cooperator ignores opponent and plays `C`.
    | Program.defect, _ => Action.D -- Defector ignores opponent and plays `D`.
...
@[simp] theorem cooperate_vs_any (opp : Program) :
  ...
  cases opp <;> rfl -- Check both possible opponents; both reduce by computation.
