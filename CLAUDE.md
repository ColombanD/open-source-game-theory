...
| **Outcome theorems** | `Theorems/<LeftBot>/vs_<RightBot>.lean` (per-pair layout, FULLY MIGRATED 2026-07-27) | The headline results: `outcome_X_vs_Y = some (a,b)` (and `∃k₂,∀k>k₂,…` families). One file per ordered matchup, sharded into per-bot directories; dir-local shared lemmas (play/guard/probe/floor machinery) in `Theorems/<Bot>/Helpers.lean`; reusable rules go to `LlmLemmas`. Hand-written bots keep a tiny UMBRELLA `Theorems/<Bot>.lean` re-exporting their directory (so `import …Theorems.<Bot>` still works — the Decidability.lean pattern); LLM-generated bots are indexed via `Theorems/LlmGenerations.lean` instead. LLM-written theorems keep the `llm_outcome_` prefix. |
