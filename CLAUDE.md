...
| **Meta-theorems** | `Base/` (`Asymptotics`, `AtomCerts`, `Soundness`, `Exclusion`, `Loeb`); `BaseTheorems.lean` is the re-exporting umbrella | Split 2026-07-09 (absorbed the former `SizeLemmas.lean` into `Base/Asymptotics`). Soundness (`Derivation.sound`, `sound_upto`, `proofSearch_sound`), monotonicity, `atom_complete_searchfree`, log₂ arithmetic, the internalized Löb engines (`bloeb_engine`, `pblt_engine`, `mutual_pblt_*`), and the NEGATIVE direction (`Exclusion`: the Derivation census `tail_plays_readable` + the generalized floor bound `no_provable_probeFirst_tail` (+`_botOpp` for `.bot`-wrapped searchers), plus `no_provable_searcherPlay_tail` for the searcher's OWN else-play, which resolved ALL SEVEN floor tombstones into honest outcomes: `outcome_DupocBot_vs_DBot`, `outcome_DupocBot_vs_EBot`, `outcome_PrudentBot_vs_EBot`, `outcome_JustBot_vs_DBot`, `outcome_JustBot_vs_EBot` — all `(D, C)` — `outcome_CupodBot_vs_OBot = (C, D)` (the defection-detector exploited), and the same-`k` `outcome_PrudentBot_vs_PrudentBot = (D, D)` (single-tier prudence is self-defeating; `PrudentBot2` is the escape)). All names still in `PD.BaseTheorems` (arithmetic in `PD`). The bridge from provability to real plays. |
| **Unified proof terms** | `Pf.lean` | **`Pf` (2026-07-14): the unified proof-term type** — `Derivation`+`Provable` merged into ONE `Prop`-valued budget-indexed inductive (22 constructors vs 25; no `struct` glue, one `mp`, one `implTrans`), NON-mutual so named `induction … with` works. Connected by the EXACT transfer theorem `pf_iff_provable : Pf k φ ↔ Provable k φ` (`[propext]` only) + `Pf_sound` + ported Löb/PBLT engines (`bloeb_engine_pf`, `pblt_engine_id_pf`). COEXISTENCE layer: the engine/oracle still speak `Provable`; write new proofs in `Pf`, ship via the iff. Demos (CIMCIC exclusion in ONE flat induction; DupocBot self-coop end-to-end): `Research/Spikes/unified_pf/PfEngineSpike.lean`. Full-replacement analysis: `Research/Notes/PF_REPLACEMENT_ASSESSMENT.md` (the mutualization regression + its `Pf.induct` cure are machine-checked in `Research/Spikes/unified_pf/PfMutualInductSpike.lean`). **DECIDED 2026-07-14: full replacement is GO** — `Pf` becomes the only system `S`; the phased migration plan (~10–17 sessions, engine Pf-only after Phase 3) is `Research/Notes/PF_ONLY_ROADMAP.md`. |
