"""Native tau players — MaxConfidenceBot, the MAX aggregator over Dupoc's test (2026-08-27).

A tau player is a (per-hypothesis TEST, AGGREGATOR) pair. Lifts aggregate by
`sum ≥ α`; MaxConfidenceBot by `max ≥ α`. Two facts are load-bearing and tested here:

* in the HYPOTHESIS role MaxConfidenceBot IS DupocBot (the aggregator is invisible at
...
* as a PLAYER it is the lift of no base bot: the max is not a threshold of any
  linear mass (`maxconfidence_not_linear` in Lean; the same three-signal witness here).
"""
...
def test_zoo_registers_the_native(matrix) -> None:
    ...
    assert matrix.aggregator("MinConfidenceBot") == "min"
    assert matrix.aggregator("DupocBot") == "sum"
    assert matrix.source_bot("MaxConfidenceBot") == "DupocBot"
    assert matrix.source_bot("DupocBot") == "DupocBot"
    assert NATIVE_PLAYERS["MaxConfidenceBot"].base == "DupocBot"
    assert NATIVE_PLAYERS["MinConfidenceBot"].base == "DupocBot"


def test_native_cells_are_clones_of_the_base(matrix) -> None:
    """In the hypothesis role MaxConfidenceBot is Dupoc's instance: every cell
    ...
    for other in matrix.bots:
        ...
        row = matrix.cell("MaxConfidenceBot", other)
        ...
        col = matrix.cell(other, "MaxConfidenceBot")
...
def test_native_base_need_not_be_in_the_zoo() -> None:
    ...
    m = load_tau_matrix(bots=("CooperateBot", "DefectBot", "MaxConfidenceBot"), hypothetical_cells={})
    assert m.action("MaxConfidenceBot", "CooperateBot") == "C"
    assert m.action("MaxConfidenceBot", "DefectBot") == "D"
    assert m.action("MaxConfidenceBot", "MaxConfidenceBot") == "C"   # Dupoc's Löbian self-play


def test_anchor_pointwise_for_the_native(matrix) -> None:
    """At point mass the aggregator is invisible: MaxConfidenceBot plays what Dupoc
    plays, for every α ∈ (0, 1] — the t = 1 anchor."""
    for hyp in matrix.bots:
        signal = Signal.point_mass(hyp)
        assert max_mass(matrix, "MaxConfidenceBot", signal) in (0.0, 1.0)
        for alpha in (1e-9, 0.25, 0.5, 0.75, 1.0):
            assert tau_play(matrix, "MaxConfidenceBot", alpha, signal) == matrix.action("DupocBot", hyp)
...
def test_max_is_not_the_sum(matrix) -> None:
    ...
    assert max_mass(matrix, "MaxConfidenceBot", signal) == pytest.approx(0.5)
    assert decision_mass(matrix, "MaxConfidenceBot", signal) == pytest.approx(0.5)
    assert tau_play(matrix, "DupocBot", 0.6, signal) == "C"
    assert tau_play(matrix, "MaxConfidenceBot", 0.6, signal) == "D"
    assert tau_play(matrix, "MaxConfidenceBot", 0.5, signal) == "C"   # `≥ α`
...
def test_native_is_not_a_threshold_of_any_mass(matrix) -> None:
    """The Python restatement of Lean's `maxconfidence_not_linear`: on the signals
    ...
    plays = tuple(tau_play(matrix, "MaxConfidenceBot", 1.0, w) for w in (w1, w2, w3))
    assert plays == ("C", "D", "C")
...
def test_native_needs_maxconfidence_not_expectation(matrix) -> None:
    """The headline mechanism: under blur, MaxConfidenceBot vs τ(Dupoc). Both see the
    ...
    for col in matrix.bots:
        out_native = tau_match(matrix, "MaxConfidenceBot", col, 0.6, channel)
...
def test_native_never_reads_the_N_state() -> None:
    ...
    m = load_tau_matrix(bots=("MirrorBot", "MaxConfidenceBot", "DupocBot"), hypothetical_cells={})
    assert m.action("MaxConfidenceBot", "MirrorBot") == m.action("DupocBot", "MirrorBot") == "C"
