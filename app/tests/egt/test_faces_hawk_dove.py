...
from pd_runner.egt.faces.classify import build_row
...
def test_hawk_dove_full_support_asymp_stable():
    ...
    # x_H* = V/C. Compared with a tolerance, not exactly: x* comes out of a
    # `scipy.linalg.solve` on the bordered block system, so the last ULP is
    # BLAS-version-dependent (observed 0.5000000000000001 under scipy 1.18).
    assert row["x_H"] == pytest.approx(0.5, abs=1e-12)
    assert row["x_D"] == pytest.approx(0.5, abs=1e-12)
