...
from pd_runner.egt.invasion.analysis import compute_readouts
from pd_runner.egt.invasion.cross_check import compare_against_ess
from pd_runner.egt.invasion.graph import SUSPECT_CELLS, build_strict, build_weak
from pd_runner.egt.invasion.loader import load_numeric_A
from pd_runner.egt.invasion.reporting import (
...
@pytest.mark.skip(
    reason="Rebased in step 2: this asserted against the standalone repo's "
    "hand-transcribed 8-bot CSV + config.json ingest path, both deleted. The "
    "end-to-end coverage it provides is worth keeping, so it returns driven by "
    "`egt.ingest` over a real zoo — see test_ingest.py."
)
...
@pytest.mark.skip(
    reason="Rebased in step 2: exercises the defence-in-depth cross-check via "
    "the deleted CSV+config rebuild path. The CHECK survives (it catches "
    "zoo-ordering bugs) but is repointed at egt.ingest — see test_ingest.py."
)
