...
from pd_runner.egt.nash.classification import classify_all
from pd_runner.egt.nash.game_construction import build_bimatrix
from pd_runner.egt.nash.method2_lrsnash import enumerate_lrsnash, lrsnash_available
...
def _enumerate_both(A_np: np.ndarray):
    """Enumerate extreme NE with both solvers, or pygambit alone.

    lrsnash is an optional system binary (see `method2_lrsnash`), so these
    tests exercise the pygambit path unconditionally and the cross-check only
    where lrslib is installed. The equilibrium-set assertions below are on
    `merged`, which is identical either way when the solvers agree — what is
    lost without lrsnash is the agreement CHECK, not the coverage.
    """
    ...
    if not lrsnash_available():
        return A_frac, pg, True, []
