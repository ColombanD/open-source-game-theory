...
from pd_runner.egt.nash.classification import ClassifiedNE
from pd_runner.egt.nash.dominance import DominanceResult
from pd_runner.egt.nash.payoff_shift import ShiftRecord
from pd_runner.egt.nash.pure_ne import PureNE
from pd_runner.egt.nash.reconcile import ReconciledNE
from pd_runner.egt.nash.verification import VerifiedNE
...
def compute_run_id(
    ...
    `config` is the `PayoffMatrix.assumptions()` block. The hash covers every
    input that can change the answer — the cells (`input_csv_sha`), the PD
    parameters, the zoo, and the `(t, α)` dials — so two runs share a hash iff
    they are the same analysis.
    """
    pd_payoffs = config.get("pd_payoffs", {})
    payload = json.dumps(
        {
            ...
            "zoo": config.get("zoo"),
            "tau": config.get("tau"),
            "non_termination": config.get("non_termination"),
            "stipulated_cells": config.get("stipulated_cells"),
...
def write_provenance_json(
    ...
    *,
    ...
    cross_check_performed: bool = True,
) -> None:
    payload = {
        "run_id": run_id,
        ...
        # `performed` distinguishes "the two solvers agreed" from "there was
        # no second solver to disagree". Without it, an absent lrsnash would
        # be indistinguishable from a failed cross-check in the record.
