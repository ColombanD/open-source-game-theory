...
from pd_runner.egt.nash.extreme_ne import ExtremeNE, make_extreme_ne
...
def lrsnash_available(lrsnash_bin: str = "lrsnash") -> bool:
    """True if the `lrsnash` binary can be executed.

    lrsnash ships as a conda-forge/system package (lrslib), NOT as a Python
    distribution, so `uv sync` cannot provide it. It is the SECONDARY solver:
    its only job is to cross-check pygambit. The pipeline therefore treats it
    as optional and records `cross_checked: false` when it is missing, rather
    than failing the run — but absence is always REPORTED, never silently
    taken for agreement.
    """
    return shutil.which(lrsnash_bin) is not None
...
def enumerate_lrsnash(
    ...
    Raises FileNotFoundError if the binary is absent; callers that treat the
    cross-check as optional should gate on `lrsnash_available()` first.
    """
    m = len(A_shifted)
