...
def write_assumptions(
    ...
    `max_support_size` must be passed whenever the enumeration was bounded:
    the `enumeration` field otherwise asserts that EVERY support was covered,
    and a truncated run must not read as a complete one.
    """
    ...
    payload = {
        ...
        "enumeration_is_complete": max_support_size is None,
        "enumeration": (
            ...
            if max_support_size is None
                ...
                f"TRUNCATED: only supports with |S| <= {max_support_size} were "
                "enumerated. Larger faces were NOT examined, so absence of a "
                "stable equilibrium among them is unproven, not evidence."
            )
        ),
...
def write_summary_md(
    ...
    Pass `max_support_size` when the enumeration was bounded so the summary
    says so up front — every "no stable equilibrium" reading below is then
    scoped to the faces actually examined.
    """
    out_path = Path(out_path)
    n = len(names)
    n_rows = len(df)
    ...
    body = [
        ...
        *(
            ...
            if max_support_size is None
                ...
                f"> **Enumeration truncated at |S| <= {max_support_size}.** Larger "
                "faces were not examined; the counts below describe only the "
                "faces enumerated, and absence of a stable equilibrium among "
                "them is not evidence of absence overall.",
                "",
            ]
        ),
