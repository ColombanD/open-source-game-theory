...
theorem wv_sound_upto (S S' : Prog → Prog → Prop)
    ...
    -- Kill obligation for `.tvote`: no census subject is ever a tau player, so every
    -- instantiation discharges this trivially. (Its `.tsearch` twin died with that
  ...
  | _ B IH =>
    ...
    have hplays : ∀ me opponent body a n, PlaysProof me opponent body a n → n ≤ B →
      ...
      refine PlaysProof.rec
        ...
        ?vapNil ?vapCons
        ?atomMk
        ?pfAtom ?pfAtomNeg ?pfSearchBranch ?pfSimStep ?pfBotSimStep ?pfBotSearchStep ?pfBotSysSearchStep ?pfBotSysSimStep ?pfBotSysSTS
    ...
    have hpf : ∀ k φ, Pf k φ →
      ...
      refine Pf.rec
        ...
        ?pAtom ?pAtomNeg ?pSearchBranch ?pSimStep ?pBotSimStep ?pBotSearchStep ?pBotSysSearchStep ?pBotSysSimStep ?pBotSysSTS
      ...
      case pBotSysSimStep =>
        intro k0 defs i j a me opponent hme hget hle
        constructor
        · intro _hB
          subst hme
          rintro ⟨np, hp⟩
          -- the premise gives component `j`'s SELF play; the copy is frozen on
          -- both sides, so the frame is irrelevant and the play forwards
          refine ⟨np + 3, ?_⟩
          show eval (np + 3) (.bot (.sys defs i)) opponent (.bot (.sys defs i)) = some a
          rw [eval]
          rw [eval_sys_some (np + 1) hget]
        ...
        · intro hs
          ...
          · exact Or.inr ((hs _ _ (Pf.botSysSimStep defs i j a me opponent hme hget hle))
              hint)
      ...
      case pBotSysSTS =>
        intro k0 defs i k₁ k₂ m0 ψ₁ ψ₂ c0 c1 q me opponent hme hget hprud hmk hle _ih
        constructor
        · intro _hB
          subst hme
          intro hbox
          have hps₁ : proofSearch k₁ ((ψ₁.sysClose defs).subst (.bot (.sys defs i)) opponent)
              = true := (proofSearch_spec _ _).2 hbox
          have hps₂ : proofSearch k₂ ((ψ₂.sysClose defs).subst (.bot (.sys defs i)) opponent)
              = true := (proofSearch_spec _ _).2 (Pf_mono hprud hmk)
          refine ⟨5, ?_⟩
          show eval 5 (.bot (.sys defs i)) opponent (.bot (.sys defs i)) = some c0
          rw [eval]
          rw [eval_sys_some 3 hget]
          simp only [Prog.sysClose]
          rw [eval]
          simp only [hps₁, if_true]
          rw [eval]
        ...
        · intro hs
          ...
          intro hbox
          rw [WV_plays]
          exact Or.inr ((hs _ _ (Pf.botSysSearchThenSearch defs i k₁ k₂ m0 ψ₁ ψ₂ c0 c1 q me
            opponent hme hget hprud hmk hle)) hbox)
      case pSTS =>
