...
/-! ## The trust floor, generic in the opponent (2026-08-21)

CupodBot's C is its ELSE-slot: any certificate of it crosses `search_f` and pays
the full failed budget. Structural — no hypothesis about the guard at all, and the
opponent is arbitrary. Predicted by the tau layer's entangled closure
(`ps_botSys_mismatch_false`), transplanted here to the base shape. -/

/-- No proof of ≤ k characters concludes any formula whose guarded spine tail is
    "CupodBot plays C against O": `search_t` concludes the then-constant `.D`
    (action mismatch), `search_f` pays the floor. -/
theorem no_provable_CupodBot_C_tail (k : Nat) (O : Prog) :
    ∀ K φ, Pf K φ → K ≤ k →
  ...
  · intro z a' g ψ c0 c1 q oppo hS
    injection hS with h1 h2 h3
    unfold CupodBot at h1; simp at h1
  ...
  · intro me oppo c hS L hme
    ...
    subst h1; subst h3
    cases L with
    | nil => unfold CupodBot at hme; simp [searchPlug] at hme
    | cons hd tl =>
        obtain ⟨g, ψ, e⟩ := hd
        unfold CupodBot at hme
  ...
  · intro me oppo c hS hd L hme
    ...
    | iteL z aT other => unfold CupodBot at hme; simp [ctxPlug] at hme
  · -- polarity plug: CupodBot's C IS its else-slot — the matching `elseL`
    -- decomposition pays CupodBot's own floor `k`; the `thenL` route dead-ends
    -- at the then-slot `.const .D ≠` any C-plug
    intro me oppo c hS hd L hme
    injection hS with h1 h2 h3; subst h1; subst h3
    cases hd with
    | thenL g ψ e =>
        simp only [plug2, CupodBot, Prog.search.injEq] at hme
        obtain ⟨-, -, hplug, -⟩ := hme
        exfalso
        cases L with
