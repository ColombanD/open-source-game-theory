...
theorem no_provable_JustBot_D_tail (k : Nat) (O : Prog) :
  ...
  -- (2026-08-25; formerly a 93-line hand-rolled kernel instantiation — the SAME
  -- census the tau layer's `.bot`/`.sys` editions run, one library for both).
  intro K φ hp hK ht
  exact no_provable_searcherElse_tail k (.plays .opp (.bot (DupocBot k)) .C) .C .D (by decide) O K φ hp hK
    (by simpa only [JustBot] using ht)
