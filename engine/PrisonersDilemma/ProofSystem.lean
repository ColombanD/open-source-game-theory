  ...
  inductive Pf : Nat → Formula → Prop where
    ...
    | botSysSearchStep (defs : ProgList) (i : Nat) (g : Nat) (ψ : Formula)
        ...
        Pf k (.impl (.box g ((ψ.sysClose defs).subst me opponent))
                    (.plays me opponent a))
    /-- `S` can read a `.bot`-wrapped SYSTEM COMPONENT that is a bare COPY of its
        partner — the `.sys` twin of `botSimStep`, as `botSysSearchStep` is the twin
        of `botSearchStep` (2026-08-24; replaced the `.ite`-shaped `botSysRunStep`
        the same day, once τ(Mirror) became a real forwarder). Component `i` is
        `.sim (.bot (.selfIdx j)) (.bot (.selfIdx j))`; `sysClose` sends the frozen
        `.selfIdx j` operands to `.sys defs j`, so the copy runs component `j`
        against itself and plays whatever that yields.

        ACTION-GENERIC — one rule, both polarities — which is the whole point of a
        forwarder over a threshold test: `mirror × cupod` is a Löb fixpoint on
        DEFECTION and `mirror × dupoc` one on cooperation, and the same rule reads
        both. Sound for exactly the reason `botSimStep` is: both operands of the
        `.sim` are `.bot`-frozen, so the copied play does not depend on `opponent`
        and the conclusion may quantify over it. -/
