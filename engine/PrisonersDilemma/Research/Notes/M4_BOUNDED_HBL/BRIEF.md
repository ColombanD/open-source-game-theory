...
| U2 | Property 1 in `V`: `L a (imp φ ψ) → L b φ → L (a + b + c₁(‖φ‖+‖ψ‖) + c₀) ψ` (`CutV.lean`) | V-generic lemma | **DONE 2026-09-11** (`CutV.lean`, f4a1a0b): `lenDerivable_cut_V` unconditional (`5|φ|+10|ψ|+9`); `lenProvableV_cut_V` under named `ProperV V T` (theorem at ℕ); `pa_proves_cutSentence`/`tact_proves_cutSentence` |
