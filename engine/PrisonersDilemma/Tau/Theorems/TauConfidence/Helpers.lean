...
/-!
# Tau/Theorems/TauConfidence/Helpers — the `confidence × dupoc` system

ConfidenceBot's test is Dupoc's, so its instances are Dupoc's by `rfl`
...
ConfidenceBot at point mass on Dupoc cooperates, and Dupoc at point mass on
ConfidenceBot cooperates, both past a budget threshold. The probe-level bit
(`ps_probe_inst_confidence_dupoc`) is what the δ_L column and τ(Just)'s row consume.
-/
...
theorem confidence_dupoc_mutual :
    ∃ k₂, ∀ k, k₂ < k →
...
/-- **ConfidenceBot at point mass on Dupoc COOPERATES** (Löb-gated). -/
theorem confidence_dupoc_plays_C :
    ∃ k₂, ∀ k, k₂ < k →
      ∃ N, eval N (.bot (inst (tauZoo k) .confidence .dupoc))
        (.bot (inst (tauZoo k) .confidence .dupoc)) (inst (tauZoo k) .confidence .dupoc)
        = some Action.C := by
  obtain ⟨kL, hLb⟩ := confidence_dupoc_mutual
  refine ⟨kL, fun k hk => ?_⟩
  obtain ⟨m, hm⟩ := hLb k hk
  have hint : (probe (.sys (cfdSys k) 0)).interp := Pf_sound m _ hm
  rw [inst_confidence_dupoc_eq k]
  exact entry_C_of_interp hint

/-- **Dupoc at point mass on ConfidenceBot COOPERATES** — the same term. -/
theorem dupoc_confidence_plays_C :
    ∃ k₂, ∀ k, k₂ < k →
      ∃ N, eval N (.bot (inst (tauZoo k) .dupoc .confidence))
...
/-- **THE LÖB BIT of the pair**: past a threshold the probe of `inst .confidence .dupoc`
    fires AT THE PROBING BUDGET. Consumed by the δ_L column's `.confidence` slot and by
    τ(Just)'s row. -/
theorem ps_probe_inst_confidence_dupoc :
    ∃ k₂, ∀ k, k₂ < k →
      proofSearch k (probe (inst (tauZoo k) .confidence .dupoc)) = true := by
  obtain ⟨kL, hLp⟩ := confidence_dupoc_plays_C
  ...
  rw [inst_confidence_dupoc_eq k] at hplay
  have hfired := sysSearcher_fired_of_plays (by decide) _ _ (cfdSys_get0 k) hplay
  rw [sysClose_subst_botSelfIdx] at hfired
  rw [probe, inst_confidence_dupoc_eq k]
  exact (proofSearch_spec _ _).2 (pf_A0_of_A1_cd hkk ((proofSearch_spec _ _).1 hfired))

/-- τ(Just) at ConfidenceBot: its third-party probe is aimed at the system, and fires
    exactly when the system's Löb bit does. -/
theorem ps_probe_just_confidence {k : Nat} (hkk : c_guard k + 3 ≤ k)
    (hconf : proofSearch k (probe (inst (tauZoo k) .confidence .dupoc)) = true) :
    proofSearch k (probe (inst (tauZoo k) .just .confidence)) = true :=
