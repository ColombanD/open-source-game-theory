...
/-!
# ConfidenceBot — the ambiguity-averse Löbian cooperator (native, 2026-08-27)
...
the signal reaches the threshold). ConfidenceBot aggregates by **`max ≥ θ`**:

    ConfidenceBot(w, θ) = C   iff   ∃ T, θ ≤ w T ∧ (the T-instance, facing me, provably cooperates)
...
**Why it cannot be a lift** (`confidence_not_linear`, `Tau/Theorems/TauConfidence/
...
**Its test is Dupoc's**, so `tauConfidenceSpec = tauDupocSpec` and in the hypothesis
role it is Dupoc (`Zoo.lean`'s `inst_confidence_eq_dupoc` bridges, all `rfl`). The
one new cell is `confidence × dupoc`: two Dupoc-spec self-probers, a symmetric `.sys`
system closed by mutual Löb (`TauConfidence/Helpers.lean`). Its player is
`ConfidenceBotZ` (`Zoo.lean`), the `maxPlayer` chain over the same decision vector
`TauBotZ k .confidence` would sum.
...
/-- ConfidenceBot's per-hypothesis TEST — Dupoc's: provably cooperates with me? then C,
    else D. (Definitionally `tauDupocSpec`; spelled out so the file reads alone.) -/
def tauConfidenceSpec : Spec Tmpl :=
  .search .prove .self .C (.const .C) (.const .D)

theorem tauConfidenceSpec_eq_dupoc : tauConfidenceSpec = tauDupocSpec := rfl
